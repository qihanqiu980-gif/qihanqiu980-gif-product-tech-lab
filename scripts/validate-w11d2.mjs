...
const lesson = getDailyCourse('W11D2')
const plan = getDay01FrameworkPlan('W11D2')
const implementation = getDayImplementation('W11D2')
assert.ok(lesson && plan && implementation, 'W11D2 必须同时存在完整课程、七章计划与真实实现')
assert.equal(validateDailyCourse(lesson).valid, true, 'W11D2 必须通过完整 DailyCourse 内容合同')
assert.equal(getDailyCourseRouteState('W11D2').status, 'available', 'W11D2 只能在内容、实现、门禁与 reviewed 共同满足后开放')
assert.equal(getDailyCourseRouteState('W11D2').access.requiresPriorDayCompletion, false, 'W11D2 不得依赖 W11D1 完成或掌握状态')
assert.equal(implementation.reviewed, true, 'W11D2 通过人工审阅和专项门禁后才可标为 reviewed')
assert.equal(implementation.renderer.key, 'day01-framework-w11d2')
assert.equal(implementation.experimentAdapter.key, 'tool-call-gate-observer-day01-v2')
assert.equal(implementation.evidenceAdapter.key, 'w11d2-evidence-v2')

assert.equal(lesson.title, '工具调用', 'W11D2 正式主题必须是工具调用')
assert.equal(lesson.deliverable.title, 'AI 调用图', 'W11D2 正式成果必须是 AI 调用图')
assert.equal(lesson.nextLesson?.id, 'W11D3', '下一课只能说明 W11D3，不得创建或替代其内容')
assert.equal(plan.chapters.length, 7, 'W11D2 主目录必须恰好七章')
...
for (const chapter of plan.chapters) {
  assert.ok(chapter.title.length >= 8 && chapter.lead.length >= 20, chapter.id + ' 缺少专属连续教材内容')
...
assert.equal(lessonConceptIds.size, 8, 'W11D2 必须有八个独立首次教学概念')

const serialized = JSON.stringify({ lesson, plan })
for (const phrase of [
  '工具调用的动作边界', '工具目录与参数合同', '工具选择与调用提案', '工具参数校验',
  '工具身份与权限校验', '风险分级与人工确认', '工具执行器与副作用', '工具结果回执与交付审计',
  '工具未注册', '工具参数缺失', '工具权限拒绝', '缺少人工确认', '工具执行失败', '工具教学成功',
  'executorCalled=false', 'TOOL_TIMEOUT_UNKNOWN', 'SIM_CASE_CREATED', '教学模拟', '不能证明',
]) assert.ok(serialized.includes(phrase), 'W11D2 缺少正式工具调用主题或实验边界：' + phrase)
for (const forbidden of ['RAG 检索增强生成', '检索查询', '资料源与候选片段', '基于资料的生成与引用', 'RAG 命中', 'RAG 无命中', '年龄门槛', 'DOM 沙盒', '活动报名', 'NXDOMAIN', '教学寻址台', '20 条评测样本评分']) {
  assert.ok(!serialized.includes(forbidden), 'W11D2 不得混入 RAG 首教、复制旧课或提前教学 W11D3：' + forbidden)
}

const pageSource = read('../src/views/W11D2Page.vue')
const viewSource = read('../src/components/W11D2Day01CourseView.vue')
...
for (const shellClass of ['w1-global-sidebar', 'w1-window', 'w1-shell', 'w1-main', 'w1-route', 'w1-mobile-route']) {
  assert.ok(visibleTemplate.includes(shellClass), 'W11D2 缺少正式 Day 01 外壳 ' + shellClass)
}
...
assert.match(viewSource, /tool-unsupported[\s\S]*?tool-missing-argument[\s\S]*?tool-permission-denied[\s\S]*?tool-confirmation-missing[\s\S]*?tool-execution-failed[\s\S]*?tool-success/, '工具调用观察器必须覆盖六种专属路径')
assert.match(viewSource, /不调用真实模型、工具、账号、Token、公司数据或生产系统/, '实验必须明确本地教学模拟资源边界')
assert.match(viewSource, /executorCalled=false[\s\S]*?TOOL_TIMEOUT_UNKNOWN\+callId[\s\S]*?SIM_CASE_CREATED\+callId/, '执行前失败、执行后未知与成功路径必须有不同回执证据')
...
const attemptBase = { dayId: 'W11D2', activityId: 'daily-guided-lab', conceptIds: [...lessonConceptIds], kind: 'practical-operation', passed: true, verification: 'system', evidence: '工具调用六路径教学模拟证据' }
let state = addAttempt(createEvidenceState(), { ...attemptBase, id: 'w11d2-guided-1', attemptedAt: '2026-08-22T11:00:00.000Z' })
state = addAttempt(state, { ...attemptBase, id: 'w11d2-guided-2', attemptedAt: '2026-08-22T12:00:00.000Z' })
assert.equal(state.attempts.length, 2, 'W11D2 修订提交必须追加，不能覆盖历史尝试')
state = scheduleReviewTasks(state, { dayId: 'W11D2', conceptIds: [...lessonConceptIds], learnedAt: '2026-08-22T12:00:00.000Z', sourceAttemptId: 'w11d2-guided-2' })
...
assert.ok(inventory.available >= 7, '后续推进不得撤销 W11D2 的 7/72 冻结基线')
...
else assert.ok(inventory.missing.includes('W12D1'), 'W12D1 尚未获准制作时 inventory 必须继续记为 missing')
