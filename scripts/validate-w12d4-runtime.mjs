...
const url = process.env.W12D4_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W12D4'
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w12d4-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['risk-too-vague', 'probability-missing', 'impact-missing', 'prevention-missing', 'trigger-fallback-missing', 'qualified-risk-register']) {
    ...
    assert.equal(value.title, '风险登记册编写', `${width}: W12D4 深链未打开完整课程`)
    ...
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['risk-too-vague', 'probability-missing', 'impact-missing', 'prevention-missing', 'trigger-fallback-missing', 'qualified-risk-register'], `${width}: 六路径顺序或名称错误`)
    return value
  } finally {
    chrome.kill('SIGKILL')
