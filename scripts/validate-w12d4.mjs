...
const lesson = getDailyCourse('W12D4')
const plan = getDay01FrameworkPlan('W12D4')
const implementation = getDayImplementation('W12D4')
assert.ok(lesson && plan && implementation, 'W12D4 必须同时存在完整课程、七章计划与真实实现')
assert.equal(validateDailyCourse(lesson).valid, true, 'W12D4 必须通过完整 DailyCourse 内容合同')
assert.equal(getDailyCourseRouteState('W12D4').status, 'available', 'W12D4 只能在内容、实现、门禁与 reviewed 共同满足后开放')
assert.equal(getDailyCourseRouteState('W12D4').access.requiresPriorDayCompletion, false, 'W12D4 不得依赖 W12D3 完成、复习或掌握状态')
assert.equal(implementation.reviewed, true, 'W12D4 通过审阅与专项门禁后才可标为 reviewed')
assert.equal(implementation.renderer.key, 'day01-framework-w12d4')
assert.equal(implementation.experimentAdapter.key, 'risk-register-gate-observer-day01-v1')
assert.equal(implementation.evidenceAdapter.key, 'w12d4-evidence-v2')

assert.equal(lesson.title, '风险登记册编写', 'W12D4 正式主题必须是风险登记册编写')
assert.equal(lesson.deliverable.title, '风险登记册', 'W12D4 正式成果必须是风险登记册')
assert.equal(lesson.nextLesson?.id, 'W12D5', 'W12D4 只能声明下一课 W12D5，不得创建或替代其内容')
assert.equal(plan.chapters.length, 7, 'W12D4 主目录必须恰好七章')
...
assert.equal(lessonConceptIds.size, 8, 'W12D4 必须有八个独立首次教学概念')
...
assert.deepEqual([...lessonConceptIds].sort(), requiredConcepts.sort(), 'W12D4 首教概念必须精确匹配风险登记册主题')

const serialized = JSON.stringify({ lesson, plan })
for (const phrase of [
  'risk-register.csv', 'risk_id', 'evidence_ref', 'risk_statement', 'probability', 'impact', 'prevention_action',
  'monitoring_signal', 'trigger_condition', 'fallback_plan', 'owner', 'review_cadence', 'status', 'evidence_limit',
  'risk-too-vague', 'probability-missing', 'impact-missing', 'prevention-missing', 'trigger-fallback-missing', 'qualified-risk-register',
  '教学模拟', '不能证明', 'W12D3', 'W12D5',
]) assert.ok(serialized.includes(phrase), 'W12D4 缺少风险登记册主题、字段或实验边界：' + phrase)
for (const forbidden of ['评审纪要已完成', '作品集已完成', '答辩已通过', '真实系统已上线', '真实数据已接入', '生产稳定性已证明', '真实业务价值已证明', '真实概率已证明', '真实影响已证明', 'owner已承诺', '评审已通过', '风险已关闭']) {
  assert.ok(!serialized.includes(forbidden), 'W12D4 不得提前教授 W12D5+ 或越界声明：' + forbidden)
}

assert.equal(lesson.learningPaths['30'].guidedStepIndices.length, 6, '30 分钟路径不得删除六条风险登记册路径')
assert.equal(lesson.learningPaths['45'].guidedStepIndices.length, 6, '45 分钟路径必须覆盖六条风险登记册路径')
assert.equal(lesson.exercises.length, 4, 'W12D4 必须有四道针对性练习')
assert.deepEqual(lesson.memory.reviewStages.map((item) => item.stage), ['D1', 'D3', 'D7', 'D14', 'D30', 'D60'])

const pageSource = read('../src/views/W12D4Page.vue')
const viewSource = read('../src/components/W12D4Day01CourseView.vue')
...
for (const shellClass of ['w1-global-sidebar', 'w1-window', 'w1-shell', 'w1-main', 'w1-route', 'w1-mobile-route']) {
  assert.ok(visibleTemplate.includes(shellClass), 'W12D4 缺少正式 Day 01 外壳 ' + shellClass)
}
...
assert.match(viewSource, /risk-too-vague[\s\S]*?probability-missing[\s\S]*?impact-missing[\s\S]*?prevention-missing[\s\S]*?trigger-fallback-missing[\s\S]*?qualified-risk-register/, '风险登记册观察器必须覆盖六条专属路径')
assert.match(viewSource, /不调用真实模型、工具、账号、Token、公司数据、生产日志或合规系统/, '实验必须明确本地教学模拟资源边界')
assert.match(viewSource, /validationFields[\s\S]*?risk_id[\s\S]*?evidence_ref[\s\S]*?risk_statement[\s\S]*?probability[\s\S]*?impact[\s\S]*?prevention_action[\s\S]*?monitoring_signal[\s\S]*?trigger_condition[\s\S]*?fallback_plan[\s\S]*?owner[\s\S]*?review_cadence[\s\S]*?status[\s\S]*?evidence_limit/, '成果必须机械校验 risk-register.csv 13 个字段')
assert.match(viewSource, /deliverableGate\.value\.valid[\s\S]*?saveAttempt\('deliverable'\)/, '成果只有通过风险登记册门禁才可追加保存')
...
const attemptBase = { dayId: 'W12D4', activityId: 'daily-guided-lab', conceptIds: [...lessonConceptIds], kind: 'practical-operation', passed: true, verification: 'system', evidence: '风险登记册六路径教学模拟证据' }
let state = addAttempt(createEvidenceState(), { ...attemptBase, id: 'w12d4-guided-1', attemptedAt: '2026-08-25T10:00:00.000Z' })
state = addAttempt(state, { ...attemptBase, id: 'w12d4-guided-2', attemptedAt: '2026-08-25T11:00:00.000Z' })
assert.equal(state.attempts.length, 2, 'W12D4 修订提交必须追加，不能覆盖历史尝试')
state = scheduleReviewTasks(state, { dayId: 'W12D4', conceptIds: [...lessonConceptIds], learnedAt: '2026-08-25T11:00:00.000Z', sourceAttemptId: 'w12d4-guided-2' })
...
assert.ok(inventory.available >= 15, 'W12D4 完成后 inventory 至少为 15/72')
assert.ok(!inventory.missing.includes('W12D4'), 'inventory 不得继续把 W12D4 记为 missing')
