...
const url = process.env.W3D1_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W3D1'
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d1-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['wrong-directory', 'source-overwritten', 'sqlite-missing', 'setup-not-run', 'preview-overclaim', 'qualified-sqlite-environment']) {
    ...
    assert.equal(value.title, '打开 SQLite 实验环境', `${width}: W3D1 深链未打开完整课程`)
    ...
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['wrong-directory', 'source-overwritten', 'sqlite-missing', 'setup-not-run', 'preview-overclaim', 'qualified-sqlite-environment'], `${width}: 六路径顺序或名称错误`)
    return value
  } finally {
    chrome.kill('SIGKILL')
