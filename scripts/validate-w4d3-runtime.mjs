...
const url = process.env.W4D3_RUNTIME_URL ?? 'http://127.0.0.1:4317/#/lesson/W4D3'
const courseUrl = process.env.W4D3_COURSE_ROUTE_URL ?? new URL('#/course', url).href
const viewports = [1440, 1024, 860, 390, 320]
const metricScenarios = ['base-events-cte', 'active-users', 'same-day-conversion', 'rolling-24h-conversion', 'unique-paid-orders', 'aov-and-guards']
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w4d3-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        setRetell('W4D3 要先把 base CTE、metric_ref 和 sql_snippet 固定下来，再分别算 active_users、转化率、unique_paid_orders 和 AOV。这样每个结果都有分母、去重键、窗口和边界，数字才可复核，但仍然不能证明真实生产或 W4D4 已完成。')
        retellButton?.click()
        await new Promise((resolve) => setTimeout(resolve, 80))
        ...
        setRetell('W4D3 先用 base CTE 统一输入，再分别写 active_users、exposure_to_pay、checkout_to_pay_24h、unique_paid_orders 和 aov_amount。每个结果都要说明分母、去重键、时间窗口与 sample_check，且不能越界成真实生产结论。')
        retellButton?.click()
        await new Promise((resolve) => setTimeout(resolve, 80))
        ...
        return {
          ...
          hasMetricSample: bodyText.includes('metric-calculation.md') && bodyText.includes('metric_name') && bodyText.includes('metric_ref') && bodyText.includes('aggregation_ref') && bodyText.includes('base_cte') && bodyText.includes('sql_snippet') && bodyText.includes('denominator') && bodyText.includes('numerator') && bodyText.includes('distinct_key') && bodyText.includes('time_window') && bodyText.includes('result_value') && bodyText.includes('sample_check') && bodyText.includes('can_prove') && bodyText.includes('cannot_prove') && bodyText.includes('next_review_task') && bodyText.includes('active_users') && bodyText.includes('exposure_to_pay') && bodyText.includes('checkout_to_pay_24h') && bodyText.includes('unique_paid_orders') && bodyText.includes('aov_amount') && bodyText.includes('median_amount') && bodyText.includes('max_amount') && bodyText.includes('TEST') && bodyText.includes('U03') && bodyText.includes('O05') && bodyText.includes('O06') && bodyText.includes('W4D4'),
          ariaStates: qa('.w1-route-state').every((item) => item.getAttribute('aria-label')?.match(/学习|练习|复述/)),
        }
      })()`,
    })
    const value = result.result.value ?? {}
    assert.equal(String(value.title ?? ''), '活跃、转化与客单价', `${width}: W4D3 深链未打开完整课程`)
    ...
    assert.match(value.retellAfterSecondWrong?.answer ?? '', /参考答案[\s\S]*base CTE[\s\S]*指标/, `${width}: 第二次错误后的参考答案缺少 W4D3 核心要点`)
    ...
    assert.equal(value.hasMetricSample, true, `${width}: W4D3 没有展示指标字段、样例或边界`)
    assert.equal(value.consoleCommands.length, metricScenarios.length, `${width}: 六条指标路径未全部追加`)
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), metricScenarios, `${width}: 六条指标路径顺序或名称错误`)
    return value
  } finally {
...
async function validateCourseRouteEntry(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w4d3-course-route-${width}-`))
  const chrome = spawn(chromePath, [
    '--headless=new',
    '--disable-gpu',
    '--no-first-run',
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        const entry = qa('button').find((button) => isVisible(button) && (text(button).includes('W4D3') || text(button).includes('活跃、转化与客单价')))
        entry?.click()
        await new Promise((resolve) => setTimeout(resolve, 350))
        const bodyText = text(document.body)
        return {
    ...
    assert.equal(value.entryFound, true, `${width}: 课程路线中找不到 W4D3 入口`)
    assert.equal(String(value.title ?? ''), '活跃、转化与客单价', `${width}: 课程路线点击后未打开 W4D3`)
    assert.equal(value.navCurrent, '课程', `${width}: 左侧全局学习栏课程未激活`)
    assert.equal(value.chapterCount, 7, `${width}: 课程路线打开后主正文不是七章`)
    assert.ok(bodyText.includes('metric-calculation.md') && bodyText.includes('W4D4'), `${width}: 课程路线打开后缺少 W4D3 样例或下一步边界`)
    return value
  } finally {
    await stopChrome(chrome, userDataDir)
  }
...
console.log(JSON.stringify({ dayId: 'W4D3', viewports, ok: true }, null, 2))
