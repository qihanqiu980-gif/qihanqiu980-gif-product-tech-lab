...
const url = process.env.W12D2_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W12D2'
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w12d2-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['boundary-missing', 'entity-missing', 'source-unknown', 'contract-missing', 'state-missing', 'qualified-plan']) {
        ...
        return {
          ...
          hasToolbar: Boolean(q('.daily-course-toolbar, .duration-switch')) || document.body.textContent.includes('返回课程路线'),
    ...
    assert.equal(value.title, '系统与数据方案', `${width}: W12D2 深链未打开完整课程`)
    ...
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['boundary-missing', 'entity-missing', 'source-unknown', 'contract-missing', 'state-missing', 'qualified-plan'], `${width}: 六路径顺序或名称错误`)
    return value
  } finally {
    chrome.kill('SIGKILL')
