...
const url = process.env.W12D3_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W12D3'
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w12d3-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['unknown-too-broad', 'question-missing', 'fixture-missing', 'criterion-missing', 'log-incomplete', 'qualified-validation']) {
    ...
    assert.equal(value.title, '最大未知的可运行验证日志', `${width}: W12D3 深链未打开完整课程`)
    ...
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['unknown-too-broad', 'question-missing', 'fixture-missing', 'criterion-missing', 'log-incomplete', 'qualified-validation'], `${width}: 六路径顺序或名称错误`)
    return value
  } finally {
    chrome.kill('SIGKILL')
