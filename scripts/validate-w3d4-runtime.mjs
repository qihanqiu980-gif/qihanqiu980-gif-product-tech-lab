...
const url = process.env.W3D4_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W3D4'
const courseUrl = process.env.W3D4_COURSE_ROUTE_URL ?? new URL('#/course', url).href
const viewports = [1440, 1024, 860, 390, 320]
const scenarioIds = ['null-user-id', 'null-paid-at', 'duplicate-order-id', 'test-order-included', 'zero-or-null-amount', 'boundary-cross-day']
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d4-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        return {
          ...
          hasQualityElements: bodyText.includes('业务问题') && bodyText.includes('SQL') && bodyText.includes('预期留下哪些行') && bodyText.includes('实际看到什么') && bodyText.includes('能证明什么') && bodyText.includes('不能证明什么'),
          hasDeliverableFields: bodyText.includes('check_id') && bodyText.includes('suspect_field') && bodyText.includes('check_sql') && bodyText.includes('observed_rows') && bodyText.includes('risk_type') && bodyText.includes('affected_query') && bodyText.includes('reviewer_note'),
          ariaStates: qa('.w1-route-state').every((item) => item.getAttribute('aria-label')?.match(/学习|练习|复述/)),
        }
      })()`,
    })
    const value = result.result.value
    assert.equal(value.title, '查询为何不可信', `${width}: W3D4 深链未打开完整课程`)
    ...
    assert.equal(value.hasOrdersQualityRows, true, `${width}: W3D4 没有展示 orders 表质量风险行`)
    assert.equal(value.hasQualityElements, true, `${width}: 6 条质量检查没有呈现问题/SQL/预期/实际/能证明/不能证明`)
    assert.equal(value.hasDeliverableFields, true, `${width}: 数据质量检查表字段缺失`)
    assert.equal(value.coverageCount, 6, `${width}: 六条质量检查台未全部追加`)
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), scenarioIds, `${width}: 六条质量检查顺序或名称错误`)
    return value
  } finally {
...
async function validateCourseRouteEntry(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d4-course-route-${width}-`))
  const chrome = spawn(chromePath, [
    '--headless=new',
    '--disable-gpu',
    '--no-first-run',
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        const entry = qa('button').find((button) => isVisible(button) && (text(button).includes('W3D4') || text(button).includes('查询为何不可信')))
        entry?.click()
        await new Promise((resolve) => setTimeout(resolve, 350))
        const bodyText = text(document.body)
        return {
          ...
          hasQualityRows: bodyText.includes('O1018') && bodyText.includes('TEST1') && bodyText.includes('data-quality-checklist.md'),
        }
      })()`,
    })
    const value = result.result.value
    assert.equal(value.entryFound, true, `${width}: 课程路线找不到 W3D4 入口`)
    assert.ok(value.href.includes('#/lesson/W3D4'), `${width}: 课程路线没有进入 W3D4 深链`)
    assert.equal(value.title, '查询为何不可信', `${width}: 课程路线没有打开 W3D4`)
    assert.equal(value.navCurrent, '课程', `${width}: 应用工作台课程栏目未保持活动态`)
    assert.equal(value.lockedOrMissing, false, `${width}: W3D4 被前一天完成状态或缺课壳阻断`)
    assert.equal(value.chapterCount, 7, `${width}: 路线进入后不是七章课程`)
    assert.ok(value.routeItems >= 14, `${width}: 路线进入后七章目录缺失`)
    assert.equal(value.hasQualityRows, true, `${width}: 路线进入后没有完整数据质量检查内容`)
    return value
  } finally {
    await stopChrome(chrome, userDataDir)
  }
