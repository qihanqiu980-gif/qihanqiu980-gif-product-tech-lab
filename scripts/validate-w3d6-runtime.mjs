...
const url = process.env.W3D6_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W3D6'
const courseUrl = process.env.W3D6_COURSE_ROUTE_URL ?? new URL('#/course', url).href
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d6-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['bq01-paid-app-orders', 'bq02-20260831-paid-orders', 'bq03-boundary-after-midnight', 'bq04-recent-paid-orders', 'bq05-app-nonpaid-orders', 'bq06-distinct-channel-values', 'bq07-test-order-risk', 'bq08-duplicate-order-risk', 'bq09-amount-review-risk', 'bq10-paid-time-alias', 'bq11-null-user-risk', 'bq12-review-ready-app-paid']) {
        ...
        return {
          ...
          hasOrdersSample: bodyText.includes('order_id') && bodyText.includes('user_id') && bodyText.includes('O1001') && bodyText.includes('TEST1'),
          hasTwelveQueryElements: bodyText.includes('业务问题') && bodyText.includes('SQL') && bodyText.includes('预期留下哪些行') && bodyText.includes('实际看到什么') && bodyText.includes('能证明什么') && bodyText.includes('不能证明什么') && bodyText.includes('quality_flags') && bodyText.includes('handoff_note') && bodyText.includes('next_review_question'),
          hasBoundaryContent: bodyText.includes('business-query-set.sql') && bodyText.includes('business-query-log.md') && bodyText.includes('BQ01') && bodyText.includes('BQ12') && bodyText.includes('TEST1') && bodyText.includes('O1018') && bodyText.includes('NULL') && bodyText.includes('0 元') && bodyText.includes('W4D1') && bodyText.includes('教学模拟') && bodyText.includes('不能证明'),
          ariaStates: qa('.w1-route-state').every((item) => item.getAttribute('aria-label')?.match(/学习|练习|复述/)),
        }
      })()`,
    })
    const value = result.result.value
    assert.equal(value.title, '回答真实业务问题', `${width}: W3D6 深链未打开完整课程`)
    ...
    assert.equal(value.hasOrdersSample, true, `${width}: W3D6 没有展示 orders 表字段和风险样本行`)
    assert.equal(value.hasTwelveQueryElements, true, `${width}: 12 道查询没有呈现问题/SQL/预期/实际/能证明/不能证明/交接字段`)
    assert.equal(value.hasBoundaryContent, true, `${width}: W3D6 缺少文件名、BQ01-BQ12、质量风险或 W4D1 边界`)
    assert.equal(value.coverageCount, 12, `${width}: 十二道查询练习台未全部追加`)
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['bq01-paid-app-orders', 'bq02-20260831-paid-orders', 'bq03-boundary-after-midnight', 'bq04-recent-paid-orders', 'bq05-app-nonpaid-orders', 'bq06-distinct-channel-values', 'bq07-test-order-risk', 'bq08-duplicate-order-risk', 'bq09-amount-review-risk', 'bq10-paid-time-alias', 'bq11-null-user-risk', 'bq12-review-ready-app-paid'], `${width}: 十二道查询顺序或名称错误`)
    return value
  } finally {
...
async function validateCourseRouteEntry(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d6-course-route-${width}-`))
  const chrome = spawn(chromePath, [
    '--headless=new',
    '--disable-gpu',
    '--no-first-run',
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        const entry = qa('button').find((button) => isVisible(button) && (text(button).includes('W3D6') || text(button).includes('回答真实业务问题')))
        entry?.click()
        await new Promise((resolve) => setTimeout(resolve, 350))
        const bodyText = text(document.body)
        return {
    ...
    const value = result.result.value
    assert.equal(value.entryFound, true, `${width}: 课程路线中必须能找到 W3D6 入口`)
    assert.match(value.href, /#\/lesson\/W3D6$/, `${width}: 点击课程路线入口后未进入 W3D6 深链`)
    assert.equal(value.title, '回答真实业务问题', `${width}: 课程路线入口未打开 W3D6 完整课程`)
    assert.equal(value.navCurrent, '课程', `${width}: 从课程路线进入 W3D6 后课程导航未激活`)
    assert.equal(value.chapterCount, 7, `${width}: 从课程路线进入 W3D6 后不是七章正文`)
    assert.equal(value.lockedOrMissing, false, `${width}: W3D6 入口被缺课或锁定页替代`)
    assert.ok(value.routeItems >= 14, `${width}: 从课程路线进入 W3D6 后目录未完整渲染`)
    assert.equal(value.horizontalOverflow, false, `${width}: 课程路线进入 W3D6 后页面级横向溢出`)
    return value
  } finally {
    await stopChrome(chrome, userDataDir)
  }
