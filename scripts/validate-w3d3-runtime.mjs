...
const url = process.env.W3D3_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W3D3'
const courseUrl = process.env.W3D3_COURSE_ROUTE_URL ?? new URL('#/course', url).href
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d3-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['q1-paid-orders', 'q2-app-channel-orders', 'q3-orders-on-20260831', 'q4-recent-paid-orders', 'q5-paid-time-alias', 'q6-distinct-channels']) {
        ...
        return {
          ...
          hasOrdersSample: bodyText.includes('order_id') && bodyText.includes('user_id') && bodyText.includes('O1001') && bodyText.includes('O1005'),
          hasSixQueryElements: bodyText.includes('业务问题') && bodyText.includes('SQL') && bodyText.includes('预期留下哪些行') && bodyText.includes('实际看到什么') && bodyText.includes('能证明什么') && bodyText.includes('不能证明什么'),
          ariaStates: qa('.w1-route-state').every((item) => item.getAttribute('aria-label')?.match(/学习|练习|复述/)),
        }
      })()`,
    })
    const value = result.result.value
    assert.equal(value.title, '用户与订单基础查询', `${width}: W3D3 深链未打开完整课程`)
    ...
    assert.equal(value.hasOrdersSample, true, `${width}: W3D3 没有展示 orders 表字段和样本行`)
    assert.equal(value.hasSixQueryElements, true, `${width}: 6 道查询没有呈现问题/SQL/预期/实际/能证明/不能证明`)
    assert.equal(value.coverageCount, 6, `${width}: 六道查询练习台未全部追加`)
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['q1-paid-orders', 'q2-app-channel-orders', 'q3-orders-on-20260831', 'q4-recent-paid-orders', 'q5-paid-time-alias', 'q6-distinct-channels'], `${width}: 六道查询顺序或名称错误`)
    return value
  } finally {
...
async function validateCourseRouteEntry(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w3d3-course-route-${width}-`))
  const chrome = spawn(chromePath, [
    '--headless=new',
    '--disable-gpu',
    '--no-first-run',
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        const entry = qa('button').find((button) => isVisible(button) && (text(button).includes('W3D3') || text(button).includes('用户与订单基础查询')))
        entry?.click()
        await new Promise((resolve) => setTimeout(resolve, 350))
        const bodyText = text(document.body)
        return {
    ...
    const value = result.result.value
    assert.equal(value.entryFound, true, `${width}: 课程路线中必须能找到 W3D3 入口`)
    assert.match(value.href, /#\/lesson\/W3D3$/, `${width}: 点击课程路线入口后未进入 W3D3 深链`)
    assert.equal(value.title, '用户与订单基础查询', `${width}: 课程路线入口未打开 W3D3 完整课程`)
    assert.equal(value.navCurrent, '课程', `${width}: 从课程路线进入 W3D3 后课程导航未激活`)
    assert.equal(value.chapterCount, 7, `${width}: 从课程路线进入 W3D3 后不是七章正文`)
    assert.equal(value.lockedOrMissing, false, `${width}: W3D3 入口被缺课或锁定页替代`)
    assert.ok(value.routeItems >= 14, `${width}: 从课程路线进入 W3D3 后目录未完整渲染`)
    assert.equal(value.horizontalOverflow, false, `${width}: 课程路线进入 W3D3 后页面级横向溢出`)
    return value
  } finally {
    await stopChrome(chrome, userDataDir)
  }
