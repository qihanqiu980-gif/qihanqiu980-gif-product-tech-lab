...
const lesson = getDailyCourse('W11D6')
const plan = getDay01FrameworkPlan('W11D6')
const implementation = getDayImplementation('W11D6')
assert.ok(lesson && plan && implementation, 'W11D6 必须同时存在完整课程、七章计划与真实实现')
assert.equal(validateDailyCourse(lesson).valid, true, 'W11D6 必须通过完整 DailyCourse 内容合同')
assert.equal(getDailyCourseRouteState('W11D6').status, 'available', 'W11D6 只能在内容、实现、门禁与 reviewed 共同满足后开放')
assert.equal(getDailyCourseRouteState('W11D6').access.requiresPriorDayCompletion, false, 'W11D6 不得依赖 W11D5 完成、复习或掌握状态')
assert.equal(implementation.reviewed, true, 'W11D6 通过审阅与专项门禁后才可标为 reviewed')
assert.equal(implementation.renderer.key, 'day01-framework-w11d6')
assert.equal(implementation.experimentAdapter.key, 'ai-service-eval-report-observer-day01-v1')
assert.equal(implementation.evidenceAdapter.key, 'w11d6-evidence-v2')

assert.equal(lesson.title, '评测 AI 客服', 'W11D6 正式主题必须是评测 AI 客服')
assert.equal(lesson.deliverable.title, 'AI 客服综合评测报告', 'W11D6 正式成果必须是 AI 客服综合评测报告')
assert.equal(lesson.nextLesson?.id, 'W12D1', 'W11D6 只能声明下一课 W12D1，不得创建或替代其内容')
assert.equal(plan.chapters.length, 7, 'W11D6 主目录必须恰好七章')
...
assert.equal(lessonConceptIds.size, 8, 'W11D6 必须有八个独立首次教学概念')

const serialized = JSON.stringify({ lesson, plan })
for (const phrase of [
  '评测报告范围', '目标版本锁定', '评测结果表', '报告门槛', '失败合成', '风险合成', '有限发布决策', '报告证据边界',
  'scope_version', 'result_summary', 'failure_analysis', 'risk_recommendation', 'launch_decision', 'follow_up_plan',
  'section_id', 'report_part', 'evidence_used', 'conclusion', 'recommendation', 'evidence_limit',
  '教学模拟', '不能证明',
]) assert.ok(serialized.includes(phrase), 'W11D6 缺少综合评测报告主题或实验边界：' + phrase)
for (const forbidden of ['真实生产表现已证明', '正式合规已证明', '上线审批已通过', '全量上线已批准', 'W12作品集已完成']) {
  assert.ok(!serialized.includes(forbidden), 'W11D6 不得越界声明或提前创建后续成果：' + forbidden)
}

assert.equal(lesson.learningPaths['30'].guidedStepIndices.length, 6, '30 分钟路径不得删除六段综合报告路径')
assert.equal(lesson.learningPaths['45'].guidedStepIndices.length, 6, '45 分钟路径必须覆盖六段综合报告路径')
assert.equal(lesson.exercises.length, 4, 'W11D6 必须有四道针对性练习')
assert.deepEqual(lesson.memory.reviewStages.map((item) => item.stage), ['D1', 'D3', 'D7', 'D14', 'D30', 'D60'])

const pageSource = read('../src/views/W11D6Page.vue')
const viewSource = read('../src/components/W11D6Day01CourseView.vue')
...
for (const shellClass of ['w1-global-sidebar', 'w1-window', 'w1-shell', 'w1-main', 'w1-route', 'w1-mobile-route']) {
  assert.ok(visibleTemplate.includes(shellClass), 'W11D6 缺少正式 Day 01 外壳 ' + shellClass)
}
...
assert.match(viewSource, /scope-version[\s\S]*?result-summary[\s\S]*?failure-analysis[\s\S]*?risk-recommendation[\s\S]*?launch-decision[\s\S]*?follow-up-plan/, '综合报告观察器必须覆盖六段专属路径')
assert.match(viewSource, /不调用真实模型、工具、账号、Token、公司数据、生产账单、生产日志或合规系统/, '实验必须明确本地教学模拟资源边界')
assert.match(viewSource, /lines\.length !== 6[\s\S]*?section_id[\s\S]*?reportSections[\s\S]*?launchDecisionRows !== 1[\s\S]*?followUpRows !== 1[\s\S]*?boundaryRows < 6/, '成果必须机械校验 6 行、六段、发布决策、补证计划和证据边界')
assert.match(viewSource, /deliverableGate\.value\.valid[\s\S]*?saveAttempt\('deliverable'\)/, '成果只有通过综合评测报告门禁才可追加保存')
...
const attemptBase = { dayId: 'W11D6', activityId: 'daily-guided-lab', conceptIds: [...lessonConceptIds], kind: 'practical-operation', passed: true, verification: 'system', evidence: '综合评测报告六段教学模拟证据' }
let state = addAttempt(createEvidenceState(), { ...attemptBase, id: 'w11d6-guided-1', attemptedAt: '2026-08-23T14:00:00.000Z' })
state = addAttempt(state, { ...attemptBase, id: 'w11d6-guided-2', attemptedAt: '2026-08-23T15:00:00.000Z' })
assert.equal(state.attempts.length, 2, 'W11D6 修订提交必须追加，不能覆盖历史尝试')
state = scheduleReviewTasks(state, { dayId: 'W11D6', conceptIds: [...lessonConceptIds], learnedAt: '2026-08-23T15:00:00.000Z', sourceAttemptId: 'w11d6-guided-2' })
...
assert.ok(inventory.available >= 11, '后续推进不得撤销 W11D6 的 11/72 冻结基线')
...
else assert.ok(inventory.missing.includes('W12D1'), 'W11D6 单日完成时 inventory 必须继续把 W12D1 记为 missing')
