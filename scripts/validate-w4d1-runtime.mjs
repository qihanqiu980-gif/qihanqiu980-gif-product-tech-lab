...
const url = process.env.W4D1_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W4D1'
const courseUrl = process.env.W4D1_COURSE_ROUTE_URL ?? new URL('#/course', url).href
const viewports = [1440, 1024, 860, 390, 320]
const metricScenarios = ['purpose-missing', 'signal-missing', 'numerator-only', 'denominator-missing', 'grain-ambiguous', 'time-window-ambiguous', 'exclusion-missing', 'qualified-metric-contract']
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w4d1-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        setRetell('W4D1 要先定义指标口径，因为 W3D6 只能提供行级明细证据边界，不能直接跳到指标数字或提前计算。先把口径和边界写清楚，再到 W4D2 计算。')
        retellButton?.click()
        await new Promise((resolve) => setTimeout(resolve, 80))
        ...
        return {
          ...
          hasMetricSample: bodyText.includes('metric-contract-table.md') && bodyText.includes('metric_id') && bodyText.includes('decision_purpose') && bodyText.includes('signal') && bodyText.includes('numerator') && bodyText.includes('denominator') && bodyText.includes('grain') && bodyText.includes('time_window') && bodyText.includes('exclusion_rules') && bodyText.includes('quality_risks') && bodyText.includes('source_query_ref') && bodyText.includes('can_prove') && bodyText.includes('cannot_prove') && bodyText.includes('next_sql_task') && bodyText.includes('app_paid_order_count') && bodyText.includes('app_payment_conversion_rate') && bodyText.includes('W3D6') && bodyText.includes('W4D2') && bodyText.includes('教学模拟') && bodyText.includes('不能证明'),
    ...
    assert.equal(title, '指标不是一个数字', `${width}: W4D1 深链未打开完整课程。当前正文：${bodyText.slice(0, 400)}`)
    ...
    assert.match(value.retellAfterSecondWrong?.answer ?? '', /参考答案[\s\S]*指标口径[\s\S]*行级/, `${width}: 第二次错误后的参考答案缺少 W4D1 核心要点`)
    ...
    assert.equal(value.hasMetricSample, true, `${width}: W4D1 没有展示指标口径样例、字段或边界`)
    assert.equal(value.coverageCount, metricScenarios.length, `${width}: 八条指标口径练习未全部追加`)
    assert.deepEqual((value.consoleCommands ?? []).map((item) => item.split('|')[1]), metricScenarios, `${width}: 八条指标口径练习顺序或名称错误`)
    return value
  } finally {
...
async function validateCourseRouteEntry(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w4d1-course-route-${width}-`))
  const chrome = spawn(chromePath, [
    '--headless=new',
    '--disable-gpu',
    '--no-first-run',
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        const entry = qa('button').find((button) => isVisible(button) && (text(button).includes('W4D1') || text(button).includes('指标不是一个数字')))
        entry?.click()
        await new Promise((resolve) => setTimeout(resolve, 350))
        const bodyText = text(document.body)
        return {
    ...
    const value = result.result.value ?? {}
    const bodyText = String(value.bodyText ?? '')
    assert.equal(value.entryFound, true, `${width}: 课程路线中找不到 W4D1 入口`)
    assert.equal(String(value.title ?? ''), '指标不是一个数字', `${width}: 课程路线点击后未打开 W4D1`)
    assert.equal(value.navCurrent, '课程', `${width}: 左侧全局学习栏课程未激活`)
    assert.equal(value.chapterCount, 7, `${width}: 课程路线打开后主正文不是七章`)
    assert.ok(bodyText.includes('metric-contract-table.md') && bodyText.includes('W4D2'), `${width}: 课程路线打开后缺少 W4D1 样例或下一步边界`)
    return value
  } finally {
    await stopChrome(chrome, userDataDir)
  }
...
console.log(JSON.stringify({ dayId: 'W4D1', viewports, ok: true }, null, 2))
