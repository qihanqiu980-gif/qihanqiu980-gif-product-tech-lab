...
const lesson = getDailyCourse('W12D1')
const plan = getDay01FrameworkPlan('W12D1')
const implementation = getDayImplementation('W12D1')
assert.ok(lesson && plan && implementation, 'W12D1 必须同时存在完整课程、七章计划与真实实现')
assert.equal(validateDailyCourse(lesson).valid, true, 'W12D1 必须通过完整 DailyCourse 内容合同')
assert.equal(getDailyCourseRouteState('W12D1').status, 'available', 'W12D1 只能在内容、实现、门禁与 reviewed 共同满足后开放')
assert.equal(getDailyCourseRouteState('W12D1').access.requiresPriorDayCompletion, false, 'W12D1 不得依赖 W11D6 完成、复习或掌握状态')
assert.equal(implementation.reviewed, true, 'W12D1 通过审阅与专项门禁后才可标为 reviewed')
assert.equal(implementation.renderer.key, 'day01-framework-w12d1')
assert.equal(implementation.experimentAdapter.key, 'project-brief-gate-observer-day01-v1')
assert.equal(implementation.evidenceAdapter.key, 'w12d1-evidence-v2')

assert.equal(lesson.title, '定义值得解决的问题', 'W12D1 正式主题必须是定义值得解决的问题')
assert.equal(lesson.deliverable.title, '项目 Brief', 'W12D1 正式成果必须是项目 Brief')
assert.equal(lesson.nextLesson?.id, 'W12D2', 'W12D1 只能声明下一课 W12D2，不得创建或替代其内容')
assert.equal(plan.chapters.length, 7, 'W12D1 主目录必须恰好七章')
...
assert.equal(lessonConceptIds.size, 8, 'W12D1 必须有八个独立首次教学概念')

const serialized = JSON.stringify({ lesson, plan })
for (const phrase of [
  '问题陈述', '目标用户', '利益相关方与影响', '证据清单', '缺失证据', '成功指标', '范围与非目标', 'Brief 决策',
  'project_title', 'user_problem', 'target_user', 'stakeholders', 'evidence_used', 'missing_evidence', 'success_metric',
  'scope', 'non_goals', 'assumptions', 'largest_unknown', 'next_step', 'evidence_limit', 'project-brief.md',
  'problem-too-broad', 'user-missing', 'evidence-missing', 'metric-not-measurable', 'scope-creep', 'qualified-brief',
  '教学模拟', '不能证明',
]) assert.ok(serialized.includes(phrase), 'W12D1 缺少项目 Brief 主题、字段或实验边界：' + phrase)
for (const forbidden of ['系统与数据方案已完成', '验证日志已完成', '风险登记册已完成', '评审纪要已完成', '作品集已完成', '答辩已通过', '真实业务价值已证明', '真实生产表现已证明']) {
  assert.ok(!serialized.includes(forbidden), 'W12D1 不得提前教授 W12D2+ 或越界声明：' + forbidden)
}

assert.equal(lesson.learningPaths['30'].guidedStepIndices.length, 6, '30 分钟路径不得删除六条项目 Brief 路径')
assert.equal(lesson.learningPaths['45'].guidedStepIndices.length, 6, '45 分钟路径必须覆盖六条项目 Brief 路径')
assert.equal(lesson.exercises.length, 4, 'W12D1 必须有四道针对性练习')
assert.deepEqual(lesson.memory.reviewStages.map((item) => item.stage), ['D1', 'D3', 'D7', 'D14', 'D30', 'D60'])

const pageSource = read('../src/views/W12D1Page.vue')
const viewSource = read('../src/components/W12D1Day01CourseView.vue')
...
for (const shellClass of ['w1-global-sidebar', 'w1-window', 'w1-shell', 'w1-main', 'w1-route', 'w1-mobile-route']) {
  assert.ok(visibleTemplate.includes(shellClass), 'W12D1 缺少正式 Day 01 外壳 ' + shellClass)
}
...
assert.match(viewSource, /problem-too-broad[\s\S]*?user-missing[\s\S]*?evidence-missing[\s\S]*?metric-not-measurable[\s\S]*?scope-creep[\s\S]*?qualified-brief/, '项目 Brief 观察器必须覆盖六条专属路径')
assert.match(viewSource, /不调用真实模型、工具、账号、Token、公司数据、生产账单、生产日志或合规系统/, '实验必须明确本地教学模拟资源边界')
assert.match(viewSource, /briefFields[\s\S]*?project_title[\s\S]*?evidence_used[\s\S]*?missing_evidence[\s\S]*?success_metric[\s\S]*?next_step[\s\S]*?evidence_limit/, '成果必须机械校验项目 Brief 13 个字段')
assert.match(viewSource, /deliverableGate\.value\.valid[\s\S]*?saveAttempt\('deliverable'\)/, '成果只有通过项目 Brief 门禁才可追加保存')
...
const attemptBase = { dayId: 'W12D1', activityId: 'daily-guided-lab', conceptIds: [...lessonConceptIds], kind: 'practical-operation', passed: true, verification: 'system', evidence: '项目 Brief 六路径教学模拟证据' }
let state = addAttempt(createEvidenceState(), { ...attemptBase, id: 'w12d1-guided-1', attemptedAt: '2026-08-23T14:00:00.000Z' })
state = addAttempt(state, { ...attemptBase, id: 'w12d1-guided-2', attemptedAt: '2026-08-23T15:00:00.000Z' })
assert.equal(state.attempts.length, 2, 'W12D1 修订提交必须追加，不能覆盖历史尝试')
state = scheduleReviewTasks(state, { dayId: 'W12D1', conceptIds: [...lessonConceptIds], learnedAt: '2026-08-23T15:00:00.000Z', sourceAttemptId: 'w12d1-guided-2' })
...
const frozenHashes = {
  ...
  '../src/course/w11d4.ts': 'fda3d58019337e16bfb686dab362d19ddc7187d0149d15d78a85461c1812d035',
  '../src/views/W11D4Page.vue': '06d70062cef33d0785754b83052a9bb1966b6082fad6e356e00b388233b76733',
  '../src/components/W11D4Day01CourseView.vue': '4ec114061ecbb016ecb38a68fda39b635f62fa45e7005746e1e004175574abe8',
  '../src/course/w11d5.ts': '88311c02aea822f723c7e3a8aeaadbff5e3884cb8eec174fbdc2dd35a78d3289',
  '../src/views/W11D5Page.vue': '77f063a9c500b97f82e287cad2e98bd7938103eece9032d5e605609284834320',
  '../src/components/W11D5Day01CourseView.vue': 'c63c2b095e1e2f6bc5f539f6b63ff69611e136ac673dc0084d1027f3eae10cae',
...
assert.ok(inventory.available >= 12, '后续推进不得撤销 W12D1 的 12/72 冻结基线')
...
else assert.ok(inventory.missing.includes('W12D2'), 'inventory 必须继续把 W12D2 记为 missing')
