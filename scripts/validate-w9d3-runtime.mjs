...
const url = process.env.W9D3_RUNTIME_URL ?? 'http://127.0.0.1:4318/course-source.html#/lesson/W9D3'
...
async function validateViewport(width, index) {
  const port = 9920 + index
  const userDataDir = await mkdtemp(join(tmpdir(), `w9d3-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        if (retell) {
          retell.value = '输入契约先决定入口，质量体检先找脏点，问题标记再把行分到 clean 与 quarantine。'
        ...
        const prediction = guidedPanel?.querySelector('textarea[aria-label="订单清洗实验预测"]')
        if (prediction) {
          prediction.value = '先看输入契约，再看六维质量、问题标记、分流和对账。'
        ...
        saveButton?.click()
        await waitFrame()
        const deliverableArea = q('textarea[aria-label="清洗脚本 Markdown 草稿"]')
        if (deliverableArea) {
          deliverableArea.value = [
            '# clean_orders.py',
            'script_id: clean_orders.py',
            'source_file: orders_dirty.csv',
            'quality_rules: completeness, uniqueness, validity, consistency, timeliness, accuracy',
            'issue_flags: is_missing_order_id, is_duplicate_order_id, is_invalid_amount, is_unknown_status, is_bad_paid_at',
            'clean_output: outputs/orders_clean.csv',
            'quarantine_output: outputs/orders_quarantine.csv',
            'rule_log: missing_order_id=2, invalid_amount=4, duplicate_order_id=3',
            'reconciliation_summary: raw_rows=24; clean_rows=19; quarantine_rows=5; raw = clean + quarantine = true',
            'can_prove: local pandas pipeline reproduces the same split and report for the same input',
            'cannot_prove: real production data is already approved and completely correct',
            'next_step: W9D4',
            'orders_dirty.csv',
            'orders_clean.csv',
        ...
        const deliverableButton = [...(q('.deliverable-panel')?.querySelectorAll('button') ?? [])].find((button) => text(button).includes('校验并保存订单清洗脚本'))
    ...
    const value = result.result.value ?? {}
    assert.ok(String(value.headline ?? '').includes('订单清洗首教'))
    assert.ok(String(value.cover ?? '').includes('两时段'))
    assert.ok(String(value.deliverableText ?? '').includes('清洗脚本'))
