...
const lesson = getDailyCourse('W11D5')
const plan = getDay01FrameworkPlan('W11D5')
const implementation = getDayImplementation('W11D5')
assert.ok(lesson && plan && implementation, 'W11D5 必须同时存在完整课程、七章计划与真实实现')
assert.equal(validateDailyCourse(lesson).valid, true, 'W11D5 必须通过完整 DailyCourse 内容合同')
assert.equal(getDailyCourseRouteState('W11D5').status, 'available', 'W11D5 只能在内容、实现、门禁与 reviewed 共同满足后开放')
assert.equal(getDailyCourseRouteState('W11D5').access.requiresPriorDayCompletion, false, 'W11D5 不得依赖 W11D4 完成、复习或掌握状态')
assert.equal(implementation.reviewed, true, 'W11D5 通过审阅与专项门禁后才可标为 reviewed')
assert.equal(implementation.renderer.key, 'day01-framework-w11d5')
assert.equal(implementation.experimentAdapter.key, 'cost-latency-safety-risk-observer-day01-v1')
assert.equal(implementation.evidenceAdapter.key, 'w11d5-evidence-v2')

assert.equal(lesson.title, '成本、延迟与安全', 'W11D5 正式主题必须是成本、延迟与安全')
assert.equal(lesson.deliverable.title, '成本延迟安全风险建议表', 'W11D5 正式成果必须是成本延迟安全风险建议表')
assert.equal(lesson.nextLesson?.id, 'W11D6', '下一课只能说明 W11D6，不得创建或替代其内容')
assert.equal(plan.chapters.length, 7, 'W11D5 主目录必须恰好七章')
...
assert.equal(lessonConceptIds.size, 9, 'W11D5 必须有九个独立首次教学概念')

const serialized = JSON.stringify({ lesson, plan })
for (const phrase of [
  'token 与生成成本驱动', '单位预算', '延迟预算', '关键路径', '安全边界', '安全拒答与安全完成', '降级兜底', '人工升级', '成本延迟安全风险建议表',
  'token_budget_exceeded', 'tool_retry_cost', 'latency_budget_breach', 'timeout_degradation', 'safety_policy_block', 'human_escalation',
  'cost_driver', 'latency_driver', 'safety_risk', 'decision', 'product_action', 'evidence_limit',
  '教学模拟', '不能证明',
]) assert.ok(serialized.includes(phrase), 'W11D5 缺少正式风险建议主题或实验边界：' + phrase)
for (const forbidden of ['Prompt injection 架构', '完整综合评测报告已经完成', '真实模型运行结果', '真实账单已证明', '生产P95已证明', '正式合规已证明']) {
  assert.ok(!serialized.includes(forbidden), 'W11D5 不得提前教授 W11D6 或越界声明：' + forbidden)
}

assert.equal(lesson.learningPaths['30'].guidedStepIndices.length, 6, '30 分钟路径不得删除六条风险建议路径')
assert.equal(lesson.learningPaths['45'].guidedStepIndices.length, 6, '45 分钟路径必须覆盖六条风险建议路径')
assert.equal(lesson.exercises.length, 4, 'W11D5 必须有四道针对性练习')
assert.deepEqual(lesson.memory.reviewStages.map((item) => item.stage), ['D1', 'D3', 'D7', 'D14', 'D30', 'D60'])

const pageSource = read('../src/views/W11D5Page.vue')
const viewSource = read('../src/components/W11D5Day01CourseView.vue')
...
for (const shellClass of ['w1-global-sidebar', 'w1-window', 'w1-shell', 'w1-main', 'w1-route', 'w1-mobile-route']) {
  assert.ok(visibleTemplate.includes(shellClass), 'W11D5 缺少正式 Day 01 外壳 ' + shellClass)
}
...
assert.match(viewSource, /token-budget[\s\S]*?tool-retry-cost[\s\S]*?latency-budget[\s\S]*?timeout-degradation[\s\S]*?safety-policy-block[\s\S]*?human-escalation/, '风险建议观察器必须覆盖六种专属路径')
assert.match(viewSource, /不调用真实模型、工具、账号、Token、公司数据或生产系统/, '实验必须明确本地教学模拟资源边界')
assert.match(viewSource, /lines\.length !== 6[\s\S]*?cost_driver[\s\S]*?riskDrivers[\s\S]*?escalationRows < 1/, '成果必须机械校验 6 行、风险驱动配额、成本延迟安全和人工升级')
assert.match(viewSource, /deliverableGate\.value\.valid[\s\S]*?saveAttempt\('deliverable'\)/, '成果只有通过成本延迟安全风险建议表门禁才可追加保存')
...
const attemptBase = { dayId: 'W11D5', activityId: 'daily-guided-lab', conceptIds: [...lessonConceptIds], kind: 'practical-operation', passed: true, verification: 'system', evidence: '风险建议六路径教学模拟证据' }
let state = addAttempt(createEvidenceState(), { ...attemptBase, id: 'w11d5-guided-1', attemptedAt: '2026-08-22T14:00:00.000Z' })
state = addAttempt(state, { ...attemptBase, id: 'w11d5-guided-2', attemptedAt: '2026-08-22T15:00:00.000Z' })
assert.equal(state.attempts.length, 2, 'W11D5 修订提交必须追加，不能覆盖历史尝试')
state = scheduleReviewTasks(state, { dayId: 'W11D5', conceptIds: [...lessonConceptIds], learnedAt: '2026-08-22T15:00:00.000Z', sourceAttemptId: 'w11d5-guided-2' })
...
assert.ok(inventory.available >= 10, '后续推进不得撤销 W11D5 的 10/72 冻结基线')
