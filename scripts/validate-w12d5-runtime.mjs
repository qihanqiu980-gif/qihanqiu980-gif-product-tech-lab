...
const url = process.env.W12D5_RUNTIME_URL ?? 'http://127.0.0.1:4318/#/lesson/W12D5'
...
async function validateViewport(width, index) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), `w12d5-runtime-${width}-`))
  ...
  try {
    ...
    const result = await page.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        for (const value of ['role-missing', 'question-vague', 'evidence-missing', 'decision-missing', 'revision-owner-missing', 'qualified-review-notes']) {
    ...
    assert.equal(value.title, '跨职能评审与修订', `${width}: W12D5 深链未打开完整课程`)
    ...
    assert.deepEqual(value.consoleCommands.map((item) => item.split('|')[1]), ['role-missing', 'question-vague', 'evidence-missing', 'decision-missing', 'revision-owner-missing', 'qualified-review-notes'], `${width}: 六路径顺序或名称错误`)
    return value
  } finally {
    chrome.kill('SIGKILL')
