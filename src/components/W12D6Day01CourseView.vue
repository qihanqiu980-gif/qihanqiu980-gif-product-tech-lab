...
type PortfolioScenario = 'artifact-missing' | 'evidence-thread-broken' | 'pitch-too-long' | 'demo-boundary-missing' | 'qna-unprepared' | 'qualified-portfolio-package'
...
type W12D6Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W12D6Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W12D6Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '作品集装配六路径观察台', domDraft: '', domEdits: 0, ageInput: 'artifact-missing', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W12D6Evidence>): W12D6Evidence {
...
const evidence = ref<W12D6Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W12D6Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W12D6Evidence }
...
const portfolioScenarioValues = ['artifact-missing', 'evidence-thread-broken', 'pitch-too-long', 'demo-boundary-missing', 'qna-unprepared', 'qualified-portfolio-package'] as const
const currentPortfolioScenario = computed<PortfolioScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return portfolioScenarioValues.includes(value as PortfolioScenario) ? value as PortfolioScenario : 'artifact-missing'
})
const currentPortfolioRun = computed(() => {
  const scenario = currentPortfolioScenario.value
  const common = { target: 'portfolio-checklist.md', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的课程作品集装配质量，不能证明真实上线、真实业务价值、真实答辩通过、招聘结果或真实跨职能批准' }
  if (scenario === 'evidence-thread-broken') return { ...common, label: '证据线断裂', metric: 'evidence_thread=broken; validation_or_risk=missing', field: 'evidence_thread', conclusion: 'evidence_thread_broken', action: '补问题→方案→验证→风险→评审修订证据线' }
  if (scenario === 'pitch-too-long') return { ...common, label: '答辩超时', metric: 'ten_minute_pitch=over_10min; boundary_slot=missing', field: 'ten_minute_pitch', conclusion: 'pitch_too_long', action: '压缩为 10 分钟并保留边界/Q&A 时间' }
  if (scenario === 'demo-boundary-missing') return { ...common, label: '演示边界缺失', metric: 'demo_boundary=empty; simulation_label=missing', field: 'demo_boundary', conclusion: 'demo_boundary_missing', action: '补教学模拟、脱敏和不能证明事项' }
  if (scenario === 'qna-unprepared') return { ...common, label: '追问无准备', metric: 'reviewer_qna=empty; evidence_limit=missing', field: 'reviewer_qna', conclusion: 'qna_unprepared', action: '补高概率追问、证据回答、缺口和下一步' }
  if (scenario === 'qualified-portfolio-package') return { ...common, label: '合格作品集包', metric: 'fields=13; terminal=true; next=W13D1=false', field: 'portfolio_package', conclusion: 'course_portfolio_ready', action: '作为 12 周核心课程终点作品集，不声明外部业务成功' }
  return { ...common, label: '成果缺失', metric: 'artifact_index=incomplete; W12D1-W12D5 coverage<5', field: 'artifact_index', conclusion: 'artifact_missing', action: '补 W12D1-W12D5 成果索引、版本和用途' }
})
const portfolioCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    artifactMissing: commands.some((item) => item.includes('artifact-missing')),
    evidenceThreadBroken: commands.some((item) => item.includes('evidence-thread-broken')),
    pitchTooLong: commands.some((item) => item.includes('pitch-too-long')),
    demoBoundaryMissing: commands.some((item) => item.includes('demo-boundary-missing')),
    qnaUnprepared: commands.some((item) => item.includes('qna-unprepared')),
    qualifiedPortfolioPackage: commands.some((item) => item.includes('qualified-portfolio-package')),
  }
})
function runPortfolioObserver() {
  const run = currentPortfolioRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `portfolio-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentPortfolioScenario.value}`, output: `${run.field} → ${run.conclusion}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-portfolio-run'] = `${runId} · ${run.label} · field=${run.field} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成作品集装配字段判断，不证明真实上线、真实业务价值、真实答辩通过、招聘结果或真实跨职能批准。`; publish()
}
function resetPortfolioObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '作品集装配观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(portfolioCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六条作品集装配路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '作品集装配实操证据已追加保存；它只证明本地教学模拟中的作品集合同质量。'
}
...
const validationFields = ['portfolio_id', 'artifact_index', 'problem_story', 'solution_summary', 'evidence_thread', 'risk_review_summary', 'revision_log', 'ten_minute_pitch', 'demo_boundary', 'reviewer_qna', 'retrospective_insight', 'redaction_boundary', 'evidence_limit'] as const
const deliverableGate = computed(() => {
  ...
  const requiredBoundaryPhrases = ['教学模拟', '不能证明', 'W12D1', 'W12D5']
  for (const phrase of requiredBoundaryPhrases) if (!text.includes(phrase)) errors.push(`必须写明边界或下一步：${phrase}。`)
  const forbiddenClaims = [
    ['真实上线', '已完成'], ['答辩', '已通过'], ['真实业务价值', '已证明'],
    ['真实跨职能', '已批准'], ['招聘', '成功'], ['W13D1', '继续'], ['录用', '已确定'],
  ]
  for (const [subject, claim] of forbiddenClaims) {
    if (text.includes(subject) && text.includes(claim)) errors.push(`不得越界声明：${subject}${claim}。`)
  }
  const artifactReady = /artifact_index[\s\S]{0,420}(W12D1|project-brief)[\s\S]{0,420}(W12D5|review-notes)/i.test(text)
  const storyReady = /problem_story[\s\S]{0,260}(用户|问题|目标|成功指标|值得解决)/i.test(text)
  const solutionReady = /solution_summary[\s\S]{0,260}(系统|数据|方案|边界|非目标|取舍)/i.test(text)
  const threadReady = /evidence_thread[\s\S]{0,520}(W12D1|Brief)[\s\S]{0,520}(W12D3|validation-log)[\s\S]{0,520}(W12D5|review-notes)/i.test(text)
  const riskReady = /risk_review_summary[\s\S]{0,320}(risk-register|review-notes|风险|评审|修订|缺口)/i.test(text)
  const revisionReady = /revision_log[\s\S]{0,300}(修订|更新|补充|change|status|ready_for_course_review)/i.test(text)
  const pitchReady = /ten_minute_pitch[\s\S]{0,360}(10|十|分钟|问题|方案|证据|风险|复盘|下一步)/i.test(text)
  const demoReady = /demo_boundary[\s\S]{0,360}(教学模拟|脱敏|不能证明|真实|样本)/i.test(text)
  const qnaReady = /reviewer_qna[\s\S]{0,360}(问|追问|答|证据|缺口|下一步|不能证明)/i.test(text)
  const retroReady = /retrospective_insight[\s\S]{0,360}(我|判断|变化|错误|下次|提前|验证|复盘)/i.test(text)
  const redactionReady = /redaction_boundary[\s\S]{0,360}(脱敏|隐藏|替换|Token|生产地址|真实用户|公司数据|公开)/i.test(text)
  const boundaryReady = /evidence_limit[\s\S]{0,520}(不能证明|教学模拟|真实上线|真实业务价值|真实答辩|招聘|W13D1|真实跨职能)/i.test(text)
  if (!artifactReady) errors.push('artifact_index 必须覆盖 W12D1–W12D5 成果。')
  if (!storyReady) errors.push('problem_story 必须写出用户、问题或成功指标。')
  if (!solutionReady) errors.push('solution_summary 必须写出系统/数据方案取舍。')
  if (!threadReady) errors.push('evidence_thread 必须串起 Brief、验证日志和评审纪要。')
  if (!riskReady) errors.push('risk_review_summary 必须引用风险登记或评审修订。')
  if (!revisionReady) errors.push('revision_log 必须写出修订动作或当前状态。')
  if (!pitchReady) errors.push('ten_minute_pitch 必须有 10 分钟答辩结构。')
  if (!demoReady) errors.push('demo_boundary 必须标注教学模拟、脱敏和不能证明。')
  if (!qnaReady) errors.push('reviewer_qna 必须包含追问、证据回答、缺口或下一步。')
  if (!retroReady) errors.push('retrospective_insight 必须写出判断变化或下次提前验证动作。')
  if (!redactionReady) errors.push('redaction_boundary 必须写出脱敏、隐藏、替换或公开限制。')
  if (!boundaryReady) errors.push('evidence_limit 必须写出教学模拟边界和不能证明的真实上线/业务价值/答辩/招聘事项。')
...
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过作品集包字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '完整作品集包已作为待审核成果追加保存；真实上线、真实业务价值、真实答辩通过和招聘结果仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W12D6</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>artifact_index</span><i>→</i><span>evidence_thread</span><i>→</i><span>problem_story</span><i>→</i><span>ten_minute_pitch</span><i>→</i><span>demo_boundary</span><i>→</i><span>reviewer_qna</span><i>→</i><span>evidence_limit</span><p>先建立 W12D1–W12D5 成果索引，再把问题、方案、验证、风险和评审修订串成证据线，压缩成 10 分钟答辩，并在演示、Q&A、复盘和脱敏处保留证据边界；任何本地教学模拟作品集都不代表真实上线、真实业务价值、真实答辩通过、招聘结果或真实跨职能批准。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="作品集装配 实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个作品集装配质量路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="作品集装配 教学场景" @change="publish"><option value="artifact-missing">成果缺失</option><option value="evidence-thread-broken">证据线断裂</option><option value="pitch-too-long">答辩超时</option><option value="demo-boundary-missing">演示边界缺失</option><option value="qna-unprepared">追问无准备</option><option value="qualified-portfolio-package">合格作品集包</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentPortfolioRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentPortfolioRun.mode }}</dd></div><div><dt>Field signal</dt><dd>{{ currentPortfolioRun.metric }}</dd></div><div><dt>日志 field</dt><dd>{{ currentPortfolioRun.field }}</dd></div><div><dt>Conclusion</dt><dd>{{ currentPortfolioRun.conclusion }}</dd></div><div><dt>Revision action</dt><dd>{{ currentPortfolioRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentPortfolioRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>作品集字段：</b>{{ currentPortfolioRun.field }} → {{ currentPortfolioRun.conclusion }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runPortfolioObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetPortfolioObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="作品集装配 教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="作品集装配 实验覆盖"><span :class="{ done: portfolioCoverage.artifactMissing }">成果缺失</span><span :class="{ done: portfolioCoverage.evidenceThreadBroken }">证据线断裂</span><span :class="{ done: portfolioCoverage.pitchTooLong }">答辩超时</span><span :class="{ done: portfolioCoverage.demoBoundaryMissing }">演示边界缺失</span><span :class="{ done: portfolioCoverage.qnaUnprepared }">追问无准备</span><span :class="{ done: portfolioCoverage.qualifiedPortfolioPackage }">合格作品集包</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W12D6 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
