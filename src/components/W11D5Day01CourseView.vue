...
type RiskScenario = 'token-budget' | 'tool-retry-cost' | 'latency-budget' | 'timeout-degradation' | 'safety-policy-block' | 'human-escalation'
...
type W11D5Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W11D5Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W11D5Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '成本延迟安全六路径观察台', domDraft: '', domEdits: 0, ageInput: 'token-budget', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W11D5Evidence>): W11D5Evidence {
...
const evidence = ref<W11D5Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W11D5Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W11D5Evidence }
...
const currentRiskScenario = computed<RiskScenario>(() => {
  ...
  return ['token-budget', 'tool-retry-cost', 'latency-budget', 'timeout-degradation', 'safety-policy-block', 'human-escalation'].includes(value) ? value as RiskScenario : 'token-budget'
})
const currentRiskRun = computed(() => {
  const scenario = currentRiskScenario.value
  const common = { target: 'cost-latency-safety-sim-v1', mode: '教学模拟', evidenceLimit: '不能证明真实账单、线上 P95/P99、正式合规结论或上线审批' }
  if (scenario === 'tool-retry-cost') return { ...common, label: '工具重试成本', metric: 'tool_calls=3; retry_count=2', driver: 'tool_retry_cost', decision: 'watch', action: '设置工具重试上限并记录失败回执' }
  if (scenario === 'latency-budget') return { ...common, label: '延迟超预算', metric: 'total_latency_ms=9800; latency_budget_ms=6000', driver: 'latency_budget_breach', decision: 'degrade', action: '改为流式首答或后台补全' }
  if (scenario === 'timeout-degradation') return { ...common, label: '超时降级', metric: 'receipt=TOOL_TIMEOUT_UNKNOWN; callId=sim-305', driver: 'timeout_degradation', decision: 'degrade', action: '提示后台处理中并保留 callId' }
  if (scenario === 'safety-policy-block') return { ...common, label: '安全阻断', metric: 'safety_level=high; policy_boundary=private_data', driver: 'safety_policy_block', decision: 'block', action: '拒绝泄露隐私并提供安全替代路径' }
  if (scenario === 'human-escalation') return { ...common, label: '人工升级', metric: 'high_value=true; tool_receipt_unknown=true', driver: 'human_escalation', decision: 'escalate', action: '携带证据转人工处理并说明 SLA' }
  return { ...common, label: 'token 超预算', metric: 'input_tokens=9000; output_tokens=2200', driver: 'token_budget_exceeded', decision: 'watch', action: '压缩上下文并设置单次 token budget' }
})
const riskCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    tokenBudget: commands.some((item) => item.includes('token-budget')),
    toolRetryCost: commands.some((item) => item.includes('tool-retry-cost')),
    latencyBudget: commands.some((item) => item.includes('latency-budget')),
    timeoutDegradation: commands.some((item) => item.includes('timeout-degradation')),
    safetyPolicyBlock: commands.some((item) => item.includes('safety-policy-block')),
    humanEscalation: commands.some((item) => item.includes('human-escalation')),
  }
})
function runRiskObserver() {
  ...
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentRiskScenario.value}`, output: `${run.driver} → ${run.decision}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-risk-run'] = `${runId} · ${run.label} · driver=${run.driver} · decision=${run.decision}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成风险建议，不证明真实账单、线上延迟或正式合规结论。`; publish()
}
function resetRiskObserver() {
  ...
  setSectionComplete('guided-lab', false); announcement.value = '风险建议观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(riskCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六类成本/延迟/安全风险路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '风险建议实操证据已追加保存；它只证明本地教学模拟中的风险建议合同质量。'
}
...
const riskDrivers = ['token_budget_exceeded', 'tool_retry_cost', 'latency_budget_breach', 'timeout_degradation', 'safety_policy_block', 'human_escalation'] as const
const allowedDecisions = ['allow', 'watch', 'degrade', 'block', 'escalate']
const deliverableGate = computed(() => {
  const lines = evidence.value.deliverable.draft.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const rows: Record<string, unknown>[] = []
  const driverCounts = Object.fromEntries(riskDrivers.map((driver) => [driver, 0])) as Record<(typeof riskDrivers)[number], number>
  ...
  const requiredText = ['case_id', 'cost_driver', 'latency_driver', 'safety_risk', 'decision', 'product_action', 'evidence_limit']
  const ids: string[] = []
  let costRows = 0
  let latencyRows = 0
  let safetyRows = 0
  let escalationRows = 0
  let teachingRows = 0
  for (const [index, row] of rows.entries()) {
    ...
    for (const driver of riskDrivers) if (rowText.includes(driver)) driverCounts[driver] += 1
    if (typeof row.decision !== 'string' || !allowedDecisions.includes(row.decision)) errors.push(`第 ${index + 1} 行 decision 必须是 ${allowedDecisions.join('/')}。`)
    if (typeof row.case_id === 'string') ids.push(row.case_id.trim().toLowerCase())
    if (rowText.includes('token_budget') || rowText.includes('tool_retry_cost') || rowText.includes('manual_review')) costRows += 1
    if (rowText.includes('latency_budget') || rowText.includes('timeout_degradation') || rowText.includes('human_queue')) latencyRows += 1
    if (rowText.includes('safety_policy_block') || rowText.includes('privacy') || rowText.includes('human_escalation') || rowText.includes('uncertain_tool_receipt')) safetyRows += 1
    if (rowText.includes('human_escalation') && rowText.includes('escalate')) escalationRows += 1
    if (rowText.includes('教学') || rowText.includes('teaching')) teachingRows += 1
    const overclaimPhrases = [['真实账单', '已证明'], ['生产p95', '已证明'], ['正式合规', '已证明'], ['可以', '全量上线']]
    if (overclaimPhrases.some(([left, right]) => rowText.includes(left) && rowText.includes(right))) errors.push(`第 ${index + 1} 行 evidence_limit 不能声称生产风险已被教学模拟完全证明。`)
  }
  if (new Set(ids).size !== ids.length) errors.push('case_id 存在重复。')
  for (const driver of riskDrivers) if (driverCounts[driver] < 1) errors.push(`${driver} 至少出现 1 次，当前 ${driverCounts[driver]} 次。`)
  if (costRows < 2) errors.push('至少 2 行必须体现成本驱动。')
  if (latencyRows < 2) errors.push('至少 2 行必须体现延迟驱动。')
  if (safetyRows < 2) errors.push('至少 2 行必须体现安全风险。')
  if (escalationRows < 1) errors.push('至少 1 行必须体现 human_escalation 且 decision=escalate。')
  if (teachingRows < 6) errors.push('每行 evidence_limit 都必须标注本地教学模拟边界。')
  return { valid: errors.length === 0, errors, lineCount: lines.length, driverCounts, costRows, latencyRows, safetyRows, escalationRows }
})
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过 6 行风险建议表门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '成本延迟安全风险建议表已作为待审核成果追加保存；真实账单、线上延迟和正式合规仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W11D5</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 11 · DAY 5 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>eval case</span><i>→</i><span>cost driver</span><i>→</i><span>latency driver</span><i>→</i><span>safety boundary</span><i>→</i><span>decision / product action</span><p>先固定样本，再分别判断成本、延迟和安全；任何一行风险建议通过都不代表真实账单、线上 P95 或正式合规结论已被证明。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="风险建议实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个风险条件</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="成本延迟安全教学场景" @change="publish"><option value="token-budget">token 超预算</option><option value="tool-retry-cost">工具重试成本</option><option value="latency-budget">延迟超预算</option><option value="timeout-degradation">超时降级</option><option value="safety-policy-block">安全阻断</option><option value="human-escalation">人工升级</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentRiskRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentRiskRun.mode }}</dd></div><div><dt>Metric</dt><dd>{{ currentRiskRun.metric }}</dd></div><div><dt>Risk driver</dt><dd>{{ currentRiskRun.driver }}</dd></div><div><dt>Decision</dt><dd>{{ currentRiskRun.decision }}</dd></div><div><dt>Product action</dt><dd>{{ currentRiskRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentRiskRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>风险建议：</b>{{ currentRiskRun.driver }} → {{ currentRiskRun.decision }}</p>
                    ...
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="风险建议教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="风险建议实验覆盖"><span :class="{ done: riskCoverage.tokenBudget }">token 成本</span><span :class="{ done: riskCoverage.toolRetryCost }">工具重试</span><span :class="{ done: riskCoverage.latencyBudget }">延迟预算</span><span :class="{ done: riskCoverage.timeoutDegradation }">超时降级</span><span :class="{ done: riskCoverage.safetyPolicyBlock }">安全阻断</span><span :class="{ done: riskCoverage.humanEscalation }">人工升级</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W11D5 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
