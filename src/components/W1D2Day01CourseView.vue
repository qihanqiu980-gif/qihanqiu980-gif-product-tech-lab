...
type W1D2FrameworkEvidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W1D2FrameworkEvidence>
  durationMode?: 30 | 45
  readonly?: boolean
}>(), {
  durationMode: 45,
...
function blankEvidence(): W1D2FrameworkEvidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: {
      ...
      previewHeadline: 'api.example.test',
      domDraft: '',
      domEdits: 0,
      ageInput: 'https://api.example.test:8443/catalog?week=1',
      buttonState: 'idle',
      buttonClicks: 0,
      consoleHistory: [],
    },
...
function mergeEvidence(source?: DeepPartial<W1D2FrameworkEvidence>): W1D2FrameworkEvidence {
  ...
  return {
    ...
    sandbox: { ...base.sandbox, ...source.sandbox, consoleHistory: Array.isArray(source.sandbox?.consoleHistory) ? source.sandbox.consoleHistory as W1D2FrameworkEvidence['sandbox']['consoleHistory'] : [] },
    guidedLab: {
      ...base.guidedLab,
      ...source.guidedLab,
...
const evidence = ref<W1D2FrameworkEvidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W1D2FrameworkEvidence {
  return JSON.parse(JSON.stringify(evidence.value)) as W1D2FrameworkEvidence
}
...
const parsedAddress = computed(() => {
  const raw = evidence.value.sandbox.ageInput.trim()
  try {
    ...
    return {
      ...
      domain: url.hostname,
      port: url.port || (url.protocol === 'https:' ? '443（默认）' : '80（默认）'),
      path: url.pathname,
      query: url.search || '无',
    }
  } catch {
    return { valid: false, protocol: '—', domain: '—', port: '—', path: '—', query: '—' }
  }
})

const dnsResult = computed(() => {
  const domain = parsedAddress.value.domain
  if (!parsedAddress.value.valid) return { status: '格式错误', address: '—', ttl: '—', entry: '未运行' }
  if (domain.includes('missing')) return { status: 'NXDOMAIN', address: '无', ttl: '—', entry: '名称层停止' }
  if (domain.includes('staging')) return { status: 'NOERROR', address: '203.0.113.20', ttl: '60s', entry: '8443 拒绝连接' }
  return { status: 'NOERROR', address: '203.0.113.18', ttl: '120s', entry: '入口接受连接（教学模拟）' }
})

function runAddressLab() {
  if (!parsedAddress.value.valid) return
  ...
  evidence.value.sandbox.consoleHistory.push({
    command: `resolve ${parsedAddress.value.domain}`,
    output: `${dnsResult.value.status} · ${dnsResult.value.address} · TTL ${dnsResult.value.ttl} · ${dnsResult.value.entry}`,
    tone: dnsResult.value.status === 'NOERROR' ? 'success' : 'error',
  })
  evidence.value.guidedLab.observations['address-result'] = `${parsedAddress.value.domain} → ${dnsResult.value.address}；${dnsResult.value.entry}`
  publish()
}

function resetAddressLab() {
  evidence.value.sandbox.buttonState = 'idle'
  evidence.value.sandbox.buttonClicks = 0
  evidence.value.sandbox.consoleHistory = []
  evidence.value.guidedLab = blankEvidence().guidedLab
  setSectionComplete('guided-lab', false)
  announcement.value = '教学寻址台已清空，可重新预测、操作和记录。'
  publish()
}
...
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || evidence.value.sandbox.buttonState !== 'success' || !stepsReady || !recordsReady) {
    announcement.value = '先完成预测、寻址操作、全部步骤和实验记录。'
    return
  }
  setSectionComplete('guided-lab', true)
  saveAttempt('guided-lab')
  announcement.value = '引导实验已形成实操证据；它仍只证明教学寻址操作。'
}
...
function completeDeliverable() {
  ...
  announcement.value = '寻址记录已作为待审核工作成果保存。'
}
...
function observeChapters() {
  ...
  }, { root: null, rootMargin: '-80px 0px -55% 0px', threshold: [0, .08, .25] })
  ...
  >
    ...
    <div class="w1-window">
      <header class="w1-appbar">
        ...
        <span class="w1-appbar-meta">COURSE / W1D2</span>
      </header>
      ...
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div>
              <p class="eyebrow">WEEK 1 · DAY 2 · 双时段课程</p>
          ...
          <details ref="mobileRoute" class="w1-mobile-route">
            ...
            <nav class="w1-route-list" aria-label="W1D2 移动端七章目录">
              <button v-for="chapter in plan.chapters" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)">
                ...
                <span class="w1-route-states" aria-label="本章状态">
                  <span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]">学习</span>
                  <span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]">练习</span>
                  <span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]">复述</span>
                </span>
              </button>
            </nav>
          </details>

          <div class="course-protocol"><span>连续教材流</span><p>每章按“工作问题 → 精讲 → URL／寻址模型 → 完整示范 → 练习／实验 → 证据边界 → 自主复述”推进。可提前观看全文，前置不足只提示补学。</p></div>
        ...
        >
          ...
          <template v-if="[2, 3, 5].includes(chapter.number)">
            <div class="concept-ledger">
              <article v-for="concept in conceptsFor(chapter)" :key="concept.id">
          ...
          <figure v-if="chapter.number === 3" class="relationship-map">
          ...
          <div v-if="chapter.number === 4" class="understanding-map">
            <span>用户动作</span><i>→</i><span>完整 URL</span><i>→</i><span>域名</span><i>→</i><span>DNS</span><i>→</i><span>服务器地址</span><i>→</i><span>端口入口</span>
            <p>到这里仍未证明 HTTP 成功、页面内容正确或数据库已写入。</p>
          </div>
          ...
          <template v-if="chapter.number === 5">
            <div class="demonstration">
              ...
              <div class="limited-conclusion"><p><b>有限结论：</b>{{ lesson.demonstration.finalConclusion }}</p><p><b>结论限制：</b>{{ lesson.demonstration.conclusionLimit }}</p></div>
            </div>
            <div class="lab-panel">
              ...
              <div class="address-console">
                <label><span>域名或完整 URL</span><input v-model="evidence.sandbox.ageInput" :readonly="readonly" @input="evidence.sandbox.buttonState = 'idle'; publish()"></label>
                <div class="url-parts"><p><span>协议</span><b>{{ parsedAddress.protocol }}</b></p><p><span>域名</span><b>{{ parsedAddress.domain }}</b></p><p><span>端口</span><b>{{ parsedAddress.port }}</b></p><p><span>路径</span><b>{{ parsedAddress.path }}</b></p><p><span>查询</span><b>{{ parsedAddress.query }}</b></p></div>
                <div class="console-actions"><button type="button" :disabled="readonly || !parsedAddress.valid" @click="runAddressLab">解析并检查入口</button><button type="button" class="secondary" :disabled="readonly" @click="resetAddressLab">清空实验</button></div>
                <dl v-if="evidence.sandbox.buttonState === 'success'" class="dns-result"><div><dt>DNS 状态</dt><dd>{{ dnsResult.status }}</dd></div><div><dt>地址</dt><dd>{{ dnsResult.address }}</dd></div><div><dt>TTL</dt><dd>{{ dnsResult.ttl }}</dd></div><div><dt>入口观察</dt><dd>{{ dnsResult.entry }}</dd></div></dl>
              </div>
              ...
              <div class="panel-actions"><button type="button" :disabled="readonly" @click="completeGuidedLab">保存引导实验尝试</button><button type="button" class="secondary" :disabled="readonly" @click="resetAddressLab">清空并重做</button></div>
            </div>
          </template>
          ...
          <template v-if="chapter.number === 6">
            <div class="boundary-table">
              <h3>看到什么，最多判断到哪里</h3>
              <dl><div><dt>域名无解析</dt><dd>名称层停止；还没进入连接、HTTP、页面或数据。</dd></div><div><dt>地址有结果，端口拒绝</dt><dd>入口层失败；不能判断 HTTP 响应与页面代码。</dd></div><div><dt>入口接受连接</dt><dd>只说明连接向前；仍要下一课的 HTTP 证据。</dd></div><div><dt>页面内容错误</dt><dd>需要继续看请求、响应和业务数据，不能倒推 DNS 一定有错。</dd></div></dl>
            </div>
          ...
          <template v-if="chapter.number === 7">
            <div class="practice-workshop">
              ...
              <article v-for="(exercise, index) in exerciseSet" :key="exercise.id">
                ...
                <textarea v-else v-model="exerciseState(exercise).response" rows="4" :readonly="readonly || exerciseState(exercise).submitted" placeholder="先写你的推理与证据边界" @input="publish" />
            ...
            <div class="deliverable-panel">
              ...
              <textarea v-model="evidence.deliverable.draft" rows="14" :readonly="readonly" @input="evidence.deliverable.touched = true; publish()" />
          ...
          <div v-if="chapter.number === 7" class="course-completion">
            ...
            <button type="button" :disabled="readonly || !canCompleteCourse" @click="completeCourse">{{ evidence.completedAt ? '本课首学已完成' : '确认完成 W1D2 首学' }}</button>
        ...
        <aside class="w1-route" aria-label="W1D2 七章学习路线">
          <div class="w1-route-head"><strong>W1D2 学习路线</strong><span>7 章 · {{ visibleCourseMinutes }} 分钟 · 状态分开记录</span></div>
          <div class="w1-route-group">
            ...
            <div class="w1-route-list">
              <button v-for="chapter in sessionOneChapters" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)">
                <span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span>
                <span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]">复述</span></span>
              </button>
            </div>
          </div>
          <div class="w1-route-group">
...
.address-console { margin-top:22px; padding:20px; color:#fff; background:var(--navy); }.address-console label { display:grid; gap:7px; }.address-console input { color:var(--ink); }.url-parts { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); gap:1px; margin-top:14px; background:#285469; }.url-parts p { display:grid; gap:4px; min-width:0; margin:0; padding:10px; background:#0a3448; }.url-parts span { color:#a9c0c9; font-size:13px; }.url-parts b { overflow-wrap:anywhere; font-size:14px; }.console-actions,.panel-actions,.template-actions,.checkpoint-footer { display:flex; flex-wrap:wrap; gap:10px; margin-top:16px; }.day01-course button { min-height:42px; padding:9px 14px; color:#fff; background:var(--orange); border:1px solid var(--orange); border-radius:8px; font:inherit; font-weight:800; cursor:pointer; }.day01-course button.secondary,.reset-link { color:var(--navy); background:#fff; border-color:var(--line); }.day01-course button:disabled { cursor:not-allowed; opacity:.5; }.day01-course button:focus-visible,.day01-course input:focus-visible,.day01-course textarea:focus-visible,.day01-course select:focus-visible { outline:3px solid rgba(226,103,45,.35); outline-offset:2px; }.dns-result { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); margin:16px 0 0; border-top:1px solid #285469; border-left:1px solid #285469; }.dns-result div { padding:10px; border-right:1px solid #285469; border-bottom:1px solid #285469; }.dns-result dt { color:#a9c0c9; font-size:13px; }.dns-result dd { margin:5px 0 0; overflow-wrap:anywhere; }.lab-steps { padding:0; list-style:none; }.lab-steps li { padding:16px 0; border-bottom:1px solid var(--line); }.lab-steps label { display:flex; align-items:flex-start; gap:10px; }.lab-steps label span { line-height:1.7; }.lab-steps label b { display:block; }.lab-steps p,.lab-steps small { display:block; margin:8px 0 0 28px; line-height:1.65; }.record-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:14px; }.record-grid label { display:grid; gap:7px; }.record-grid span { font-weight:750; }
...
.framework-chapter { width:min(100%,840px); margin-inline:auto; padding:56px 0; scroll-margin-top:18px; border-bottom:1px solid var(--w1-line); }
...
@media (max-width:390px) {
  ...
  .day01-course[data-device="mobile"] .manuscript { padding-inline:15px; }
}
</style>
