...
import { getW8D6FrameworkPlan } from '../course/w8d6Framework'
...
type StructureScenario = 'input-contract' | 'normalization' | 'metric-snapshot' | 'template-assembly' | 'run-repeatability' | 'qualified-summary-script'
...
type W8D6Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W8D6Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
const plan = (() => {
  const resolved = getW8D6FrameworkPlan(props.lesson.id)
...
function blankEvidence(): W8D6Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '自动摘要脚本观察器', domDraft: '', domEdits: 0, ageInput: 'input-contract', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W8D6Evidence>): W8D6Evidence {
...
const evidence = ref<W8D6Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W8D6Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W8D6Evidence }
...
const currentStructureScenario = computed<StructureScenario>(() => {
  ...
  return ['input-contract', 'normalization', 'metric-snapshot', 'template-assembly', 'run-repeatability', 'qualified-summary-script'].includes(value) ? value as StructureScenario : 'input-contract'
})
const currentStructureRun = computed(() => {
  const scenario = currentStructureScenario.value
  const common = { target: 'daily-summary-script.py', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的输入清洗、统计和文本拼装路径，不能证明真实业务结果、生产调度或自动化上线' }
  if (scenario === 'normalization') return { ...common, label: '源记录归一化', metric: 'raw_record -> normalized_record', section: 'cleaning_rule', conclusion: 'normalized_path', action: '把字段整理和格式统一分开标注' }
  if (scenario === 'metric-snapshot') return { ...common, label: '指标快照', metric: 'count / total / missing', section: 'metric_snapshot', conclusion: 'snapshot_ready', action: '标出统计字段如何装配成快照' }
  if (scenario === 'template-assembly') return { ...common, label: '模板拼装', metric: 'snapshot -> summary text', section: 'summary_template', conclusion: 'template_filled', action: '标出快照如何填进固定模板' }
  if (scenario === 'run-repeatability') return { ...common, label: '重复运行', metric: 'command + same input + same output', section: 'run_command', conclusion: 'repeatable_run', action: '标出命令、输入和输出如何稳定复跑' }
  if (scenario === 'qualified-summary-script') return { ...common, label: '合格标注', metric: 'fields=13; evidence_limit=present', section: 'next_step', conclusion: 'ready_for_w9d1', action: '进入 W9D1 前先把自动摘要脚本补完整' }
  return { ...common, label: '输入契约', metric: 'source feed + first record', section: 'input_source', conclusion: 'entry_boundary', action: '先写输入来源和第一条原始记录' }
})
const structureCoverage = computed(() => {
  ...
  return {
    ...
    assignmentConversion: commands.some((item) => item.includes('normalization')),
    branchCondition: commands.some((item) => item.includes('metric-snapshot')),
    loopIteration: commands.some((item) => item.includes('template-assembly')),
    functionOutput: commands.some((item) => item.includes('run-repeatability')),
    qualifiedAnnotation: commands.some((item) => item.includes('qualified-summary-script')),
  }
})
function runStructureObserver() {
  ...
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成摘要路径判断，不证明真实业务结果或后续流程已完成。`; publish()
}
function resetStructureObserver() {
  ...
  setSectionComplete('guided-lab', false); announcement.value = '自动摘要流程观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '自动摘要脚本实操证据已追加保存；它只证明本地教学模拟中的路径质量。'
}
...
const structureFields = ['script_id', 'summary_id', 'input_source', 'cleaning_rule', 'metric_snapshot', 'summary_template', 'output_target', 'run_command', 'run_log', 'evidence_limit', 'can_prove', 'cannot_prove', 'next_step'] as const
const deliverableGate = computed(() => {
  ...
  const requiredBoundaryPhrases = ['daily-summary-script.py', '教学模拟', '不能证明', 'W9D1']
  ...
  const forbiddenClaims = [
    ...
    ['真实业务', '已证明'], ['生产结果', '已证明'], ['数据清洗', '已完成'],
  ]
  for (const [subject, claim] of forbiddenClaims) {
    if (text.includes(subject) && text.includes(claim)) errors.push(`不得越界声明：${subject}${claim}。`)
  }
  const roleReady = /input_source[\s\S]{0,220}(输入源|原始记录|清洗|统计|模板|输出)/i.test(text)
  const canProveReady = /can_prove[\s\S]{0,180}(清洗|统计|模板|命令|日志|教学模拟)/i.test(text)
  const cannotProveReady = /cannot_prove[\s\S]{0,220}(不能证明|真实|生产|自动化|数据清洗|结果正确|上线|调度)/i.test(text)
  const nextStepReady = /next_step[\s\S]{0,160}W9D1/i.test(text)
  if (!roleReady) errors.push('input_source 必须明确输入源、原始记录、清洗、统计、模板或输出。')
  if (!canProveReady) errors.push('can_prove 必须写出流程层能证明的有限事实。')
  if (!cannotProveReady) errors.push('cannot_prove 必须写出不能证明的真实生产或自动化边界。')
  if (!nextStepReady) errors.push('next_step 必须只指向 W9D1。')
...
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过自动摘要脚本字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '自动摘要脚本已作为待审核成果追加保存；真实自动化、生产调度和生产结果仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W8D6</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 8 · DAY 6 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>input contract</span><i>→</i><span>normalization</span><i>→</i><span>metric snapshot</span><i>→</i><span>template</span><i>→</i><span>run log</span><p>先固定脚本的输入契约、源记录归一化、指标快照、文本模板和重复运行命令，再把自动摘要脚本写成可复核的字段；任何本地教学模拟通过都不代表真实业务结果、生产调度或自动化上线。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="自动摘要实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实脚本、文件、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个结构路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="自动摘要教学场景" @change="publish"><option value="input-contract">输入契约</option><option value="normalization">源记录归一化</option><option value="metric-snapshot">指标快照</option><option value="template-assembly">模板拼装</option><option value="run-repeatability">重复运行</option><option value="qualified-summary-script">合格脚本</option></select></label>
                    ...
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="自动摘要脚本教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="自动摘要脚本实验覆盖"><span :class="{ done: structureCoverage.entryInput }">输入契约</span><span :class="{ done: structureCoverage.assignmentConversion }">源记录归一化</span><span :class="{ done: structureCoverage.branchCondition }">指标快照</span><span :class="{ done: structureCoverage.loopIteration }">模板拼装</span><span :class="{ done: structureCoverage.functionOutput }">重复运行</span><span :class="{ done: structureCoverage.qualifiedAnnotation }">合格脚本</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W8D6 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
