...
import { getW5D2FrameworkPlan } from '../course/w5d2Framework'
...
type W5D2Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W5D2Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
const plan = (() => {
  const resolved = getW5D2FrameworkPlan(props.lesson.id)
...
function blankEvidence(): W5D2Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: 'JOIN 对账练习台', domDraft: '', domEdits: 0, ageInput: 'inner-join-rule', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W5D2Evidence>): W5D2Evidence {
...
const evidence = ref<W5D2Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W5D2Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W5D2Evidence }
...
const joinSampleRows = [
  { join_case_id: 'J1-inner-orders-users', join_input_rows: 'orders + users', join_behavior: 'INNER JOIN', join_result: 'matched_rows=5', join_edge: 'O05 user U99 缺失' },
  { join_case_id: 'J2-left-orders-users', join_input_rows: 'orders + users', join_behavior: 'LEFT JOIN', join_result: 'unmatched_orders=1', join_edge: '右表补 NULL' },
  { join_case_id: 'J3-left-orders-items', join_input_rows: 'orders + order_items', join_behavior: 'LEFT JOIN', join_result: 'wrong_gmv=740', join_edge: 'O01/O02 被复制' },
] as const

const firstCompleteJoinRule = {
  join_case_id: 'J3-left-orders-items',
  join_ref: 'reconciliation.md / W5D2 JOIN 对照图',
  join_input_rows: 'orders JOIN order_items 前的订单粒度与明细粒度输入。',
  join_rule: 'LEFT JOIN orders o ON o.order_id = i.order_id，再比较 JOIN 前后行数。',
  join_behavior: '先看 rows_before / rows_after，再看 SUM(o.paid_amount) 是否被复制。',
  join_key: 'order_id。',
  join_result: 'correct_gmv=440; wrong_gmv=740。',
  join_edge: 'O05 user U99 缺失；I08 order_id=MISSING；O01/O02 被明细复制。',
  cannot_prove: '不能证明真实生产 GMV、原因、趋势、分析报告或 W5D3 已完成。',
} as const
...
const joinTasks = [
  {
    id: 'inner-join-rule',
    label: 'INNER 保留',
    target: 'reconciliation.md + self_check.sql',
    mode: '教学模拟',
    businessQuestion: 'INNER JOIN 会留下谁。',
    sql: 'orders INNER JOIN users ON orders.user_id = users.user_id',
    expected: '只保留匹配行，任何一边缺失都会被过滤。',
    actual: '交集结果只剩匹配订单和用户。',
    field: 'join_type',
    metric: 'INNER JOIN',
    conclusion: 'inner_only_keeps_matches',
    action: '记录 join_key 和保留规则。',
    canProve: '能证明 INNER JOIN 只保留交集。',
    cannotProve: '不能证明缺口自动消失。',
    deliverableHint: '补 join_case_id、join_type、preserve_rule、join_key、rows_before、rows_after。',
  },
  {
    id: 'left-join-rule',
    label: 'LEFT 保留',
    target: 'reconciliation.md + self_check.sql',
    mode: '教学模拟',
    businessQuestion: 'LEFT JOIN 会保留哪一边。',
    sql: 'orders LEFT JOIN users ON orders.user_id = users.user_id',
    expected: '保留订单，右表缺失时补 NULL。',
    actual: 'orders 全量保留，用户字段缺失处显示 NULL。',
    field: 'preserve_rule',
    metric: 'LEFT JOIN',
    conclusion: 'left_keeps_orders',
    action: '记录保留哪一边、哪些字段补 NULL。',
    canProve: '能证明 LEFT JOIN 保留左表。',
    cannotProve: '不能证明用户维表已完整。',
    deliverableHint: '补 join_type、preserve_rule、unmatched_keys、can_prove、cannot_prove。',
  },
  {
    id: 'right-join-rule',
    label: 'RIGHT 保留',
    target: 'reconciliation.md + self_check.sql',
    mode: '教学模拟',
    businessQuestion: 'RIGHT JOIN 会保留哪一边。',
    sql: 'users RIGHT JOIN orders ON users.user_id = orders.user_id',
    expected: '保留订单侧，左表缺失时补 NULL。',
    actual: '订单全量保留，用户字段缺失处显示 NULL。',
    field: 'preserve_rule',
    metric: 'RIGHT JOIN',
    conclusion: 'right_keeps_orders',
    action: '记录保留哪一边、哪些字段补 NULL。',
    canProve: '能证明 RIGHT JOIN 和 LEFT JOIN 只是视角互换。',
    cannotProve: '不能证明更换方向会自动修复缺失。',
    deliverableHint: '补 join_type、preserve_rule、join_key、cannot_prove。',
  },
  {
    id: 'full-join-rule',
    label: 'FULL 保留',
    target: 'reconciliation.md + self_check.sql',
    mode: '教学模拟',
    businessQuestion: 'FULL JOIN 会保留什么。',
    sql: 'orders FULL JOIN users ON orders.user_id = users.user_id',
    expected: '左右两边都保留，双方缺口都变成 NULL。',
    actual: '订单缺用户、用户缺订单都能同时看见。',
    field: 'join_type',
    metric: 'FULL JOIN',
    conclusion: 'full_keeps_both_sides',
    action: '记录两边缺口如何同时显现。',
    canProve: '能证明 FULL JOIN 是双向缺口盘点。',
    cannotProve: '不能证明缺口原因。',
    deliverableHint: '补 unmatched_keys、join_type、preserve_rule、cannot_prove。',
  },
  {
    id: 'amplification-check',
    label: '一对多放大',
    target: 'reconciliation.md + self_check.sql',
    mode: '教学模拟',
    businessQuestion: 'orders JOIN order_items 后为什么金额会放大。',
    sql: 'SUM(orders.paid_amount) FROM orders JOIN order_items',
    expected: '订单级字段会被明细行复制，wrong_gmv 变大。',
    actual: 'wrong_gmv=740，而正确 GMV=440。',
    field: 'amount_gap',
    metric: 'grain amplification',
    conclusion: 'one_to_many_multiplies_orders',
    action: '记录哪几笔订单被复制了几次。',
    canProve: '能证明一对多会放大订单级金额。',
    cannotProve: '不能证明收入真的上涨。',
    deliverableHint: '补 amount_gap、unique_keys、rows_before、rows_after。',
  },
  {
    id: 'anti-join-gap',
    label: '缺口审查',
    target: 'reconciliation.md + self_check.sql',
    mode: '教学模拟',
    businessQuestion: '哪条 JOIN 最适合找未匹配订单和未匹配明细。',
    sql: 'LEFT JOIN ... WHERE right_key IS NULL',
    expected: '先保留左表，再把右表 NULL 缺口筛出来。',
    actual: 'O05 缺 user，I08 缺 order。',
    field: 'unmatched_keys',
    metric: 'Anti Join',
    conclusion: 'null_keys_are_gap',
    action: '记录缺口键而不是静默删掉。',
    canProve: '能证明缺口可以被单独列出。',
    cannotProve: '不能证明缺口原因。',
    deliverableHint: '补 unmatched_keys、join_key、cannot_prove。',
  },
...
const currentJoinScenario = computed<JoinScenario>(() => {
  ...
  return joinScenarioValues.includes(value as JoinScenario) ? value as JoinScenario : 'inner-join-rule'
})
const currentJoinRun = computed(() => {
  ...
  return { ...task, evidenceLimit: '只能证明本地 W5-JOIN对账 教学库中的 JOIN 对照可复核，不能证明真实生产 JOIN、JOIN 对账、归因、趋势、看板发布或 W5D3 已完成' }
})
...
function runJoinObserver() {
  ...
  const runId = `w5d2-join-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  ...
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成 JOIN 对账证据，不证明真实生产 JOIN、JOIN 对账、归因、趋势、看板发布或 W5D3 已完成。`; publish()
}
function resetJoinObserver() {
  ...
  setSectionComplete('guided-lab', false); announcement.value = 'JOIN 对账观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = 'JOIN 对账实操证据已追加保存；它只证明本地教学模拟中的 JOIN 保留规则可复核。'
}
...
const deliverableGate = computed(() => {
  ...
  const requiredBoundaryPhrases = ['reconciliation.md', 'JOIN 对账记录', 'W5D2', 'W5-JOIN对账', '440', '740', 'O05', 'I08', 'LEFT JOIN', 'FULL JOIN', '先聚合再连接', '不能证明']
  ...
  if (!joinCaseReady) errors.push('join_case_id 必须写出JOIN 对照编号。')
  ...
  if (!joinCanReady) errors.push('can_prove 必须限定到本地教学JOIN 对照可复核。')
...
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过 JOIN 对账字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = 'JOIN 对账记录已作为待审核成果追加保存；真实生产 JOIN、JOIN 对账、归因、趋势、看板发布和 W5D3 完成状态仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W5D2</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 5 · DAY 2 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
            ...
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <template v-if="chapter.number === 1">
                ...
                <div class="plain-section">
                  <h3>今天先看见对象：JOIN 对照图</h3>
                  <p>今天学的是 JOIN 保留规则。先不要做 JOIN 结论，先确认输入行集合和保留侧，再看 NULL 补位、一对多放大和未匹配键。</p>
	                  <div class="system-console" aria-label="JOIN 对照样本">
	                    <p class="simulation-label">来自 W5-JOIN对账 教学库的 JOIN 样本</p>
	                    <table>
                  ...
                  <p>先看一条能从头到尾解释的JOIN 对照，再进入 join_input_rows、join_rule、join_behavior、join_key、join_result 和 join_edge。这样学习者先知道“今天到底在计算什么”，再把字段拆开。</p>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="JOIN 对照 实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 只读取 W5-JOIN对账 教学库素材，不调用真实数据库、账号、Token、公司数据或生产系统</p>
                    <label><span>选择一道 JOIN 路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="JOIN 对照 教学场景" @change="publish"><option v-for="task in joinTasks" :key="task.id" :value="task.id">{{ task.label }}</option></select></label>
                    <dl class="stage-ledger">
                    ...
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="JOIN 对照 教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="JOIN 对照 实验覆盖"><span v-for="task in joinTasks" :key="task.id" :class="{ done: joinCoverage[task.id] }">{{ task.label }}</span></div>
                  </div>
                  <ol class="lab-steps">
        ...
        <aside class="w1-route" aria-label="W5D2 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
