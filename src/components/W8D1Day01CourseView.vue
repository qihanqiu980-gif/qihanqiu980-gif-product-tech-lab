...
import { getW8D1FrameworkPlan } from '../course/w8d1Framework'
...
type StructureScenario = 'entry-input' | 'assignment-conversion' | 'branch-condition' | 'loop-iteration' | 'function-output' | 'qualified-annotation'
...
type W8D1Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W8D1Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
const plan = (() => {
  const resolved = getW8D1FrameworkPlan(props.lesson.id)
...
function blankEvidence(): W8D1Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '代码结构标注观察器', domDraft: '', domEdits: 0, ageInput: 'entry-input', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W8D1Evidence>): W8D1Evidence {
...
const evidence = ref<W8D1Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W8D1Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W8D1Evidence }
...
const currentStructureScenario = computed<StructureScenario>(() => {
  ...
  return ['entry-input', 'assignment-conversion', 'branch-condition', 'loop-iteration', 'function-output', 'qualified-annotation'].includes(value) ? value as StructureScenario : 'entry-input'
})
const currentStructureRun = computed(() => {
  const scenario = currentStructureScenario.value
  const common = { target: 'code-structure-annotation.md', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的结构标注质量，不能证明真实自动化、JSON/CSV 清洗、报错诊断或文件批处理' }
  if (scenario === 'assignment-conversion') return { ...common, label: '赋值与转换', metric: 'raw_count -> count', section: 'statement_or_block', conclusion: 'conversion_boundary', action: '把赋值和类型转换分开标注' }
  if (scenario === 'branch-condition') return { ...common, label: '条件分支', metric: 'if/elif/else', section: 'role', conclusion: 'branch_path', action: '标出条件决定了哪条路径' }
  if (scenario === 'loop-iteration') return { ...common, label: '循环迭代', metric: 'for row in rows', section: 'output', conclusion: 'repeat_path', action: '标出重复处理的对象和次数' }
  if (scenario === 'function-output') return { ...common, label: '函数边界', metric: 'def build_line', section: 'boundary', conclusion: 'reuse_boundary', action: '标出函数输入、输出与复用范围' }
  if (scenario === 'qualified-annotation') return { ...common, label: '合格标注', metric: 'fields=9; evidence_limit=present', section: 'next_step', conclusion: 'ready_for_w8d2', action: '进入 W8D2 前准备完整输入到输出流程图' }
  return { ...common, label: '入口与输入', metric: 'python entry + first input', section: 'step_id', conclusion: 'entry_boundary', action: '先写入口和第一条输入语句' }
})
const structureCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    entryInput: commands.some((item) => item.includes('entry-input')),
    assignmentConversion: commands.some((item) => item.includes('assignment-conversion')),
    branchCondition: commands.some((item) => item.includes('branch-condition')),
    loopIteration: commands.some((item) => item.includes('loop-iteration')),
    functionOutput: commands.some((item) => item.includes('function-output')),
    qualifiedAnnotation: commands.some((item) => item.includes('qualified-annotation')),
  }
})
function runStructureObserver() {
  const run = currentStructureRun.value; evidence.value.sandbox.buttonClicks += 1
  ...
  evidence.value.guidedLab.observations['latest-structure-run'] = `${runId} · ${run.label} · section=${run.section} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成代码结构标注判断，不证明真实自动化或后续流程已完成。`; publish()
}
function resetStructureObserver() {
  ...
  setSectionComplete('guided-lab', false); announcement.value = '代码结构标注观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(structureCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六条结构路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '代码结构标注实操证据已追加保存；它只证明本地教学模拟中的结构合同质量。'
}
...
const structureFields = ['step_id', 'statement_or_block', 'role', 'input', 'output', 'boundary', 'can_prove', 'cannot_prove', 'next_step'] as const
const deliverableGate = computed(() => {
  ...
  const requiredBoundaryPhrases = ['code-structure-annotation.md', '教学模拟', '不能证明', 'W8D2']
  for (const phrase of requiredBoundaryPhrases) if (!text.includes(phrase)) errors.push(`必须写明边界或下一步：${phrase}。`)
  const forbiddenClaims = [
    ['JSON', '已完成'], ['CSV', '已完成'], ['报错诊断', '已完成'], ['文件批处理', '已完成'],
    ['真实自动化', '已上线'], ['生产结果', '已证明'], ['数据清洗', '已完成'],
  ]
  for (const [subject, claim] of forbiddenClaims) {
    if (text.includes(subject) && text.includes(claim)) errors.push(`不得越界声明：${subject}${claim}。`)
  }
  const roleReady = /role[\s\S]{0,220}(入口|输入|赋值|转换|分支|循环|函数|输出)/i.test(text)
  const canProveReady = /can_prove[\s\S]{0,180}(结构|步骤|入口|转换|分支|循环|函数|输出|教学模拟)/i.test(text)
  const cannotProveReady = /cannot_prove[\s\S]{0,220}(不能证明|真实|生产|自动化|数据清洗|结果正确|上线)/i.test(text)
  const nextStepReady = /next_step[\s\S]{0,160}W8D2/i.test(text)
  if (!roleReady) errors.push('role 必须明确入口、输入、赋值、转换、分支、循环、函数或输出。')
  if (!canProveReady) errors.push('can_prove 必须写出结构层能证明的有限事实。')
  if (!cannotProveReady) errors.push('cannot_prove 必须写出不能证明的真实生产或自动化边界。')
  if (!nextStepReady) errors.push('next_step 必须只指向 W8D2 准备。')
...
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过代码结构标注字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '代码结构标注已作为待审核成果追加保存；真实自动化、数据清洗和生产结果仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W8D1</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 8 · DAY 1 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>entry</span><i>→</i><span>input</span><i>→</i><span>assignment</span><i>→</i><span>conversion</span><i>→</i><span>branch / loop / function</span><i>→</i><span>output</span><p>先固定 Python 脚本的入口、输入、赋值、类型转换和控制流，再把结构标注成可复核的字段；任何本地教学模拟通过都不代表真实自动化、JSON/CSV 清洗或文件批处理已完成。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="代码结构标注实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实脚本、文件、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个结构路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="代码结构标注教学场景" @change="publish"><option value="entry-input">入口与输入</option><option value="assignment-conversion">赋值与转换</option><option value="branch-condition">条件分支</option><option value="loop-iteration">循环迭代</option><option value="function-output">函数边界</option><option value="qualified-annotation">合格标注</option></select></label>
                    ...
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="代码结构标注教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="代码结构标注实验覆盖"><span :class="{ done: structureCoverage.entryInput }">入口输入</span><span :class="{ done: structureCoverage.assignmentConversion }">赋值转换</span><span :class="{ done: structureCoverage.branchCondition }">条件分支</span><span :class="{ done: structureCoverage.loopIteration }">循环迭代</span><span :class="{ done: structureCoverage.functionOutput }">函数输出</span><span :class="{ done: structureCoverage.qualifiedAnnotation }">合格标注</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W8D1 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
