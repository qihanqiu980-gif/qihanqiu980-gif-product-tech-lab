...
type PlanScenario = 'boundary-missing' | 'entity-missing' | 'source-unknown' | 'contract-missing' | 'state-missing' | 'qualified-plan'
...
type W12D2Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W12D2Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W12D2Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '系统与数据方案六路径观察台', domDraft: '', domEdits: 0, ageInput: 'boundary-missing', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W12D2Evidence>): W12D2Evidence {
...
const evidence = ref<W12D2Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W12D2Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W12D2Evidence }
...
const currentPlanScenario = computed<PlanScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['boundary-missing', 'entity-missing', 'source-unknown', 'contract-missing', 'state-missing', 'qualified-plan'].includes(value) ? value as PlanScenario : 'boundary-missing'
})
const currentPlanRun = computed(() => {
  const scenario = currentPlanScenario.value
  const common = { target: 'system-data-plan.md', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的系统与数据方案合同质量，不能证明真实系统已开发、真实数据已接入、验证日志已完成或上线可行' }
  if (scenario === 'entity-missing') return { ...common, label: '数据对象缺失', metric: 'data_entities=empty; fields=unknown', section: 'data_entities', conclusion: 'entity_missing', action: '补 FailureSample/ReviewRecord 等对象和关键字段' }
  if (scenario === 'source-unknown') return { ...common, label: '来源未知', metric: 'data_sources=system_auto; owner=unknown', section: 'data_sources', conclusion: 'source_unknown', action: '写 source/owner/missing_reason/fallback' }
  if (scenario === 'contract-missing') return { ...common, label: '契约缺失', metric: 'required_inputs=missing; error_cases=missing', section: 'input_output_contracts', conclusion: 'contract_missing', action: '补 required_inputs/outputs/error_cases' }
  if (scenario === 'state-missing') return { ...common, label: '状态缺失', metric: 'state_lifecycle=success_or_fail_only', section: 'state_lifecycle', conclusion: 'state_missing', action: '补 imported/classified/review/export 状态' }
  if (scenario === 'qualified-plan') return { ...common, label: '合格方案', metric: 'fields=13; evidence_limit=present; validation_questions=W12D3', section: 'system_data_decision', conclusion: 'ready_for_w12d3', action: '进入 W12D3 前准备可运行验证问题' }
  return { ...common, label: '系统边界缺失', metric: 'system_boundary=all_platform; non_goals=empty', section: 'system_boundary', conclusion: 'boundary_missing', action: '限定系统负责/不负责的起止点' }
})
const planCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    boundaryMissing: commands.some((item) => item.includes('boundary-missing')),
    entityMissing: commands.some((item) => item.includes('entity-missing')),
    sourceUnknown: commands.some((item) => item.includes('source-unknown')),
    contractMissing: commands.some((item) => item.includes('contract-missing')),
    stateMissing: commands.some((item) => item.includes('state-missing')),
    qualifiedPlan: commands.some((item) => item.includes('qualified-plan')),
  }
})
function runPlanObserver() {
  const run = currentPlanRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `plan-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentPlanScenario.value}`, output: `${run.section} → ${run.conclusion}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-plan-run'] = `${runId} · ${run.label} · section=${run.section} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成系统与数据方案字段判断，不证明真实系统已开发、真实数据已接入或后续文档已完成。`; publish()
}
function resetPlanObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '系统与数据方案观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(planCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六条系统与数据方案路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '系统与数据方案实操证据已追加保存；它只证明本地教学模拟中的系统与数据方案合同质量。'
}
...
const planFields = ['brief_ref', 'system_goal', 'system_boundary', 'user_touchpoints', 'data_entities', 'data_sources', 'input_output_contracts', 'state_lifecycle', 'dependencies', 'privacy_access_boundary', 'validation_questions', 'non_goals', 'evidence_limit'] as const
const deliverableGate = computed(() => {
  const text = evidence.value.deliverable.draft
  const lower = text.toLowerCase()
  const lines = text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const fieldPresence = Object.fromEntries(planFields.map((field) => [field, lower.includes(field)])) as Record<(typeof planFields)[number], boolean>
  for (const field of planFields) if (!fieldPresence[field]) errors.push(`缺少字段：${field}。`)
  const requiredBoundaryPhrases = ['教学模拟', '不能证明', 'W12D3']
  for (const phrase of requiredBoundaryPhrases) if (!text.includes(phrase)) errors.push(`必须写明边界或下一步：${phrase}。`)
  const forbiddenClaims = [
    ['验证日志', '已完成'], ['风险登记册', '已完成'], ['评审纪要', '已完成'], ['作品集', '已完成'],
    ['答辩', '已通过'], ['真实系统', '已开发'], ['真实数据', '已接入'], ['验证', '已通过'], ['上线可行', '已证明'],
  ]
  for (const [subject, claim] of forbiddenClaims) {
    if (text.includes(subject) && text.includes(claim)) errors.push(`不得越界声明：${subject}${claim}。`)
  }
  const briefReady = /brief_ref[\s\S]{0,160}(W12D1|Brief|brief|项目 Brief)/i.test(text)
  const entityReady = /data_entities[\s\S]{0,260}(FailureSample|ReviewRecord|ClassificationSuggestion|字段|case_id)/i.test(text)
  const sourceReady = /data_sources[\s\S]{0,260}(owner|missing|教学模拟|真实|source|来源)/i.test(text)
  const contractReady = /input_output_contracts[\s\S]{0,260}(required_inputs|outputs|error_cases|输入|输出|错误)/i.test(text)
  const stateReady = /state_lifecycle[\s\S]{0,260}(imported|classified|review|required|exported|状态|待复核)/i.test(text)
  const validationReady = /validation_questions[\s\S]{0,180}W12D3/i.test(text)
  if (!briefReady) errors.push('brief_ref 必须引用 W12D1 Brief。')
  if (!entityReady) errors.push('data_entities 必须包含业务对象、关键字段或 case_id。')
  if (!sourceReady) errors.push('data_sources 必须写明来源、owner、教学模拟/真实缺口。')
  if (!contractReady) errors.push('input_output_contracts 必须包含 required_inputs、outputs 和 error_cases。')
  if (!stateReady) errors.push('state_lifecycle 必须包含 imported/classified/review/export 等状态路径。')
  if (!validationReady) errors.push('validation_questions 必须只指向 W12D3 的可运行验证问题。')
  return { valid: errors.length === 0, errors, lineCount: lines.length, fieldPresence, presentFields: planFields.filter((field) => fieldPresence[field]).length }
})
const deliverableFieldStatus = computed(() => planFields.map((field) => ({ field, done: deliverableGate.value.fieldPresence[field] })))
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过系统与数据方案 字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '系统与数据方案 已作为待审核成果追加保存；真实业务价值、方案有效性和作品集完成度仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W12D2</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 12 · DAY 2 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>brief</span><i>→</i><span>boundary</span><i>→</i><span>data</span><i>→</i><span>contract</span><i>→</i><span>state</span><i>→</i><span>validation</span><p>先从 W12D1 Brief 推出系统边界，再用数据对象、来源、输入输出和状态生命周期决定 W12D3 要验证什么；任何本地方案通过都不代表真实系统已开发、真实数据已接入或验证已通过。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="系统与数据方案 实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个系统与数据方案质量路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="系统与数据方案 教学场景" @change="publish"><option value="boundary-missing">系统边界缺失</option><option value="entity-missing">数据对象缺失</option><option value="source-unknown">来源未知</option><option value="contract-missing">契约缺失</option><option value="state-missing">状态缺失</option><option value="qualified-plan">合格方案</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentPlanRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentPlanRun.mode }}</dd></div><div><dt>Field signal</dt><dd>{{ currentPlanRun.metric }}</dd></div><div><dt>方案 field</dt><dd>{{ currentPlanRun.section }}</dd></div><div><dt>Conclusion</dt><dd>{{ currentPlanRun.conclusion }}</dd></div><div><dt>Revision action</dt><dd>{{ currentPlanRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentPlanRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>方案字段：</b>{{ currentPlanRun.section }} → {{ currentPlanRun.conclusion }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runPlanObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetPlanObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="系统与数据方案 教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="系统与数据方案 实验覆盖"><span :class="{ done: planCoverage.boundaryMissing }">系统边界缺失</span><span :class="{ done: planCoverage.entityMissing }">数据对象缺失</span><span :class="{ done: planCoverage.sourceUnknown }">来源未知</span><span :class="{ done: planCoverage.contractMissing }">契约缺失</span><span :class="{ done: planCoverage.stateMissing }">状态缺失</span><span :class="{ done: planCoverage.qualifiedPlan }">合格方案</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W12D2 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
