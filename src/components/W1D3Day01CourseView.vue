...
type W1D3FrameworkEvidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W1D3FrameworkEvidence>
  durationMode?: 30 | 45
  readonly?: boolean
}>(), {
  durationMode: 45,
...
function blankEvidence(): W1D3FrameworkEvidence {
  return {
    schemaVersion: 2,
...
function mergeEvidence(source?: DeepPartial<W1D3FrameworkEvidence>): W1D3FrameworkEvidence {
  ...
  return {
    ...
    sandbox: { ...base.sandbox, ...source.sandbox, consoleHistory: Array.isArray(source.sandbox?.consoleHistory) ? source.sandbox.consoleHistory as W1D3FrameworkEvidence['sandbox']['consoleHistory'] : [] },
    guidedLab: {
      ...base.guidedLab,
      ...source.guidedLab,
...
const evidence = ref<W1D3FrameworkEvidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W1D3FrameworkEvidence {
  return JSON.parse(JSON.stringify(evidence.value)) as W1D3FrameworkEvidence
}
...
const httpExchange = computed(() => {
  const method = evidence.value.sandbox.previewHeadline === 'GET' ? 'GET' : 'POST'
  const hasCookie = evidence.value.sandbox.domDraft !== 'no-cookie'
  const numericPayload = evidence.value.sandbox.ageInput !== 'string'
  const slow = evidence.value.sandbox.activePanel === 'console'
  const url = '/checkout/preview?currency=CNY&coupon=LAB20'
  const payload = numericPayload
    ? '{"sku":"LAB-HTTP-01","quantity":2}'
    : '{"sku":"LAB-HTTP-01","quantity":"2"}'

  if (method !== 'POST') {
    return { method, url, payload, status: 405, code: 'METHOD_NOT_ALLOWED', timing: '72 ms', size: '318 B', tone: 'error' as const }
  }
  if (!hasCookie) {
    return { method, url, payload, status: 401, code: 'SESSION_EXPIRED', timing: '84 ms', size: '352 B', tone: 'error' as const }
  }
  if (!numericPayload) {
    return { method, url, payload, status: 422, code: 'INVALID_TYPE', timing: '96 ms', size: '416 B', tone: 'error' as const }
  }
  return { method, url, payload, status: 200, code: 'PREVIEW_READY', timing: slow ? '2.4 s' : '120 ms', size: '1.2 KB', tone: 'success' as const }
})

function runHttpLab() {
  const result = httpExchange.value
  ...
  evidence.value.sandbox.consoleHistory.push({
    ...
    output: `${result.status} · ${result.code} · ${result.timing} · ${result.size}`,
    tone: result.tone,
  })
  evidence.value.guidedLab.observations['http-result'] = `${result.method} ${result.url} → ${result.status} ${result.code}；${result.timing}；${result.size}`
  publish()
}

function resetHttpLab() {
  ...
  announcement.value = '教学 HTTP 台已清空，可重新预测、发送和记录。'
  publish()
}
...
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || evidence.value.sandbox.buttonClicks < 2 || !stepsReady || !recordsReady) {
    announcement.value = '先完成预测、至少两次 HTTP 对照发送、全部步骤和实验记录。'
    return
  }
  setSectionComplete('guided-lab', true)
  saveAttempt('guided-lab')
  announcement.value = '引导实验已形成实操证据；它仍只证明教学 HTTP 模拟中的字段与响应关系。'
}
...
function completeDeliverable() {
  ...
  announcement.value = 'HTTP 观察记录已作为待审核工作成果保存。'
}
  ...
  >
    ...
    <div class="w1-window">
      <header class="w1-appbar">
        ...
        <span class="w1-appbar-meta">COURSE / W1D3</span>
      </header>
      ...
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div>
              <p class="eyebrow">WEEK 1 · DAY 3 · 双时段课程</p>
          ...
          <details ref="mobileRoute" class="w1-mobile-route">
            ...
            <nav class="w1-route-list" aria-label="W1D3 移动端七章目录">
          ...
          <div class="course-protocol"><span>连续教材流</span><p>每章按“工作问题 → 请求／响应精讲 → 字段关系 → 完整示范 → HTTP 对照实验 → 证据边界 → 自主复述”推进。可提前观看全文，前置不足只提示补学。</p></div>
        ...
        >
          ...
          <div v-if="chapter.number === 4" class="understanding-map">
            <span>用户动作</span><i>→</i><span>Request</span><i>→</i><span>Method + URL</span><i>→</i><span>Header / Cookie / Query / Payload</span><i>→</i><span>Response</span>
            <p>收到响应后还要联合 Status、正文、Timing 与 Size；任何单项都不能直接证明业务成功或持久化完成。</p>
          </div>
          ...
          <template v-if="chapter.number === 5">
            ...
            <div class="lab-panel">
              ...
              <div class="http-console">
                <p class="simulation-label">教学模拟 · 不发送真实请求或账号数据</p>
                <div class="request-controls">
                  <label><span>Method</span><select v-model="evidence.sandbox.previewHeadline" :disabled="readonly" @change="evidence.sandbox.buttonState = 'idle'; publish()"><option value="POST">POST</option><option value="GET">GET</option></select></label>
                  <label><span>Cookie</span><select v-model="evidence.sandbox.domDraft" :disabled="readonly" @change="evidence.sandbox.buttonState = 'idle'; publish()"><option value="session">教学会话已附带</option><option value="no-cookie">不附带会话</option></select></label>
                  <label><span>Payload.quantity</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" @change="evidence.sandbox.buttonState = 'idle'; publish()"><option value="number">数字 2</option><option value="string">字符串 “2”</option></select></label>
                  <label><span>Timing 模式</span><select v-model="evidence.sandbox.activePanel" :disabled="readonly" @change="evidence.sandbox.buttonState = 'idle'; publish()"><option value="elements">正常等待</option><option value="console">慢响应</option></select></label>
                </div>
                <div class="message-ledger"><p><span>Request URL</span><b>{{ httpExchange.url }}</b></p><p><span>Header</span><b>Content-Type: application/json</b></p><p><span>Query</span><b>currency=CNY · coupon=LAB20</b></p><p><span>Payload</span><b>{{ httpExchange.payload }}</b></p></div>
                <div class="console-actions"><button type="button" :disabled="readonly" @click="runHttpLab">发送教学请求</button><button type="button" class="secondary" :disabled="readonly" @click="resetHttpLab">清空实验</button></div>
                <dl v-if="evidence.sandbox.buttonState === 'success'" class="response-result"><div><dt>Status</dt><dd>{{ httpExchange.status }}</dd></div><div><dt>Response code</dt><dd>{{ httpExchange.code }}</dd></div><div><dt>Timing</dt><dd>{{ httpExchange.timing }}</dd></div><div><dt>Size</dt><dd>{{ httpExchange.size }}</dd></div></dl>
                <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="HTTP 对照历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
              </div>
              ...
              <div class="panel-actions"><button type="button" :disabled="readonly" @click="completeGuidedLab">保存引导实验尝试</button><button type="button" class="secondary" :disabled="readonly" @click="resetHttpLab">清空并重做</button></div>
            </div>
          </template>
          ...
          <template v-if="chapter.number === 6">
            <div class="boundary-table">
              <h3>看到什么，最多判断到哪里</h3>
              <dl><div><dt>Request 已出现</dt><dd>客户端表达了意图；不能据此断言服务端接受或业务成功。</dd></div><div><dt>Status 200</dt><dd>HTTP 得到了正常类别的响应；仍要读取 Response code 与业务证据。</dd></div><div><dt>Status 4xx / 5xx</dt><dd>说明拒绝或服务异常类别；责任归因仍需请求上下文、正文和服务记录。</dd></div><div><dt>Timing / Size 异常</dt><dd>说明性能或传输规模变化；不能替代结果正确性和持久化证明。</dd></div></dl>
            </div>
          ...
          <div v-if="chapter.number === 7" class="course-completion">
            ...
            <button type="button" :disabled="readonly || !canCompleteCourse" @click="completeCourse">{{ evidence.completedAt ? '本课首学已完成' : '确认完成 W1D3 首学' }}</button>
        ...
        <aside class="w1-route" aria-label="W1D3 七章学习路线">
          <div class="w1-route-head"><strong>W1D3 学习路线</strong><span>7 章 · {{ visibleCourseMinutes }} 分钟 · 状态分开记录</span></div>
