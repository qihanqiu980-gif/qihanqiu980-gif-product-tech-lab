...
type FailureScenario = 'knowledge-gap' | 'retrieval-gap' | 'source-conflict' | 'generation-overreach' | 'tool-chain' | 'expression-overcertainty'
...
type W11D4Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W11D4Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W11D4Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '失败分类六路径观察台', domDraft: '', domEdits: 0, ageInput: 'knowledge-gap', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W11D4Evidence>): W11D4Evidence {
...
const evidence = ref<W11D4Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W11D4Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W11D4Evidence }
...
const currentScenario = computed<FailureScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['knowledge-gap', 'retrieval-gap', 'source-conflict', 'generation-overreach', 'tool-chain', 'expression-overcertainty'].includes(value) ? value as FailureScenario : 'knowledge-gap'
})
const currentRun = computed(() => {
  const scenario = currentScenario.value
  const common = { target: 'support-failure-sim-v1', mode: '教学模拟', evidenceLimit: '不能证明真实生产根因、线上频率或修复效果' }
  if (scenario === 'retrieval-gap') return { ...common, label: '召回缺口', source: 'source_exists=true', context: 'context_missing=true', output: '答复缺少退款时限', category: 'retrieval_gap', firstStage: 'context_selection', action: '检查资料命中与上下文选择' }
  if (scenario === 'source-conflict') return { ...common, label: '资料冲突', source: 'source_a=7天 / source_b=15天', context: 'priority=none', output: '直接选择 7 天且未提示冲突', category: 'source_conflict', firstStage: 'source_arbitration', action: '确认权威源、有效期和优先级' }
  if (scenario === 'generation-overreach') return { ...common, label: '生成过度推断', source: 'context lacks exact date', context: 'must_not_include=具体到账日', output: '承诺明天到账', category: 'generation_overreach', firstStage: 'model_candidate', action: '收紧未知处理和禁止承诺' }
  if (scenario === 'tool-chain') return { ...common, label: '工具链失败', source: 'executorCalled=true', context: 'receipt=TOOL_TIMEOUT_UNKNOWN; callId=sim-204', output: '工单已创建成功', category: 'tool_chain', firstStage: 'tool_receipt_delivery', action: '按 callId 查询并修正回执转述' }
  if (scenario === 'expression-overcertainty') return { ...common, label: '表达过度确定', source: 'source says maybe', context: 'expected_behavior=qualified_answer', output: '保证 3 天内到', category: 'expression_overcertainty', firstStage: 'output_handoff', action: '降级确定性表达并补下一步' }
  return { ...common, label: '知识缺失', source: 'source_exists=false', context: 'policy_out_of_scope=true', output: '编造 2027 新政策日期', category: 'knowledge_gap', firstStage: 'knowledge_scope', action: '补充获准知识或转人工' }
})
const observerCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    knowledgeGap: commands.some((item) => item.includes('knowledge-gap')),
    retrievalGap: commands.some((item) => item.includes('retrieval-gap')),
    sourceConflict: commands.some((item) => item.includes('source-conflict')),
    generationOverreach: commands.some((item) => item.includes('generation-overreach')),
    toolChain: commands.some((item) => item.includes('tool-chain')),
    expressionCertainty: commands.some((item) => item.includes('expression-overcertainty')),
  }
})
function runFailureObserver() {
  const run = currentRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `failure-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentScenario.value}`, output: `${run.category} → ${run.firstStage}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-failure-run'] = `${runId} · ${run.label} · category=${run.category} · first_failed_stage=${run.firstStage}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只分类失败来源，不证明真实生产根因。`; publish()
}
function resetFailureObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '失败分类观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(observerCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六类失败来源路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '失败分类实操证据已追加保存；它只证明本地教学模拟中的分类合同质量。'
}
...
const failureCategories = ['knowledge_gap', 'retrieval_gap', 'source_conflict', 'generation_overreach', 'tool_chain', 'expression_overcertainty'] as const
const deliverableGate = computed(() => {
  const lines = evidence.value.deliverable.draft.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const rows: Record<string, unknown>[] = []
  const categoryCounts = Object.fromEntries(failureCategories.map((category) => [category, 0])) as Record<(typeof failureCategories)[number], number>
  ...
  if (lines.length !== 6) errors.push(`需要恰好 6 行，当前 ${lines.length} 行。`)
  const requiredText = ['case_id', 'expected_behavior', 'observed_output', 'evidence_seen', 'failure_category', 'first_failed_stage', 'product_action', 'evidence_limit']
  const ids: string[] = []
  let toolRows = 0
  let retrievalRows = 0
  let teachingRows = 0
  for (const [index, row] of rows.entries()) {
    ...
    if (typeof category === 'string' && failureCategories.includes(category as (typeof failureCategories)[number])) categoryCounts[category as (typeof failureCategories)[number]] += 1
    else errors.push(`第 ${index + 1} 行 failure_category 不在六类合同内。`)
    if (typeof row.case_id === 'string') ids.push(row.case_id.trim().toLowerCase())
    const rowText = JSON.stringify(row).toLowerCase()
    if (String(category) === 'tool_chain' && (rowText.includes('callid') || rowText.includes('receipt'))) toolRows += 1
    if (String(category) === 'retrieval_gap' && rowText.includes('source_exists=true') && rowText.includes('context_missing=true')) retrievalRows += 1
    if (rowText.includes('教学') || rowText.includes('teaching')) teachingRows += 1
    if (rowText.includes('生产根因已证明') || rowText.includes('proven_root_cause')) errors.push(`第 ${index + 1} 行 evidence_limit 不能声称生产根因已证明。`)
  }
  if (new Set(ids).size !== ids.length) errors.push('case_id 存在重复。')
  for (const category of failureCategories) if (categoryCounts[category] !== 1) errors.push(`${category} 应为 1 条，当前 ${categoryCounts[category]} 条。`)
  if (toolRows < 1) errors.push('tool_chain 行必须包含 callId 或 receipt 证据。')
  if (retrievalRows < 1) errors.push('retrieval_gap 行必须同时包含 source_exists=true 与 context_missing=true。')
  if (teachingRows < 6) errors.push('每行 evidence_limit 都必须标注本地教学模拟边界。')
  return { valid: errors.length === 0, errors, lineCount: lines.length, categoryCounts, toolRows, retrievalRows }
})
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过 6 行失败分类表门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '失败分类表已作为待审核成果追加保存；生产根因仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W11D4</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 11 · DAY 4 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>observed output</span><i>→</i><span>knowledge / retrieval / conflict</span><i>→</i><span>generation / tool / expression</span><i>→</i><span>first failed stage</span><i>→</i><span>product action</span><p>先固定输出事实，再按可见证据选唯一主分类；任何一行分类通过都不代表真实生产根因已被证明。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="失败分类实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个失败来源条件</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="失败分类教学场景" @change="publish"><option value="knowledge-gap">知识缺失</option><option value="retrieval-gap">召回缺口</option><option value="source-conflict">资料冲突</option><option value="generation-overreach">生成过度推断</option><option value="tool-chain">工具链失败</option><option value="expression-overcertainty">表达过度确定</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentRun.mode }}</dd></div><div><dt>Source</dt><dd>{{ currentRun.source }}</dd></div><div><dt>Context</dt><dd>{{ currentRun.context }}</dd></div><div><dt>Observed output</dt><dd>{{ currentRun.output }}</dd></div><div><dt>Category</dt><dd>{{ currentRun.category }}</dd></div><div><dt>First failed stage</dt><dd>{{ currentRun.firstStage }}</dd></div><div><dt>Product action</dt><dd>{{ currentRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>首个证据断点：</b>{{ currentRun.firstStage }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runFailureObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetFailureObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="失败分类教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="失败分类实验覆盖"><span :class="{ done: observerCoverage.knowledgeGap }">知识缺失</span><span :class="{ done: observerCoverage.retrievalGap }">召回缺口</span><span :class="{ done: observerCoverage.sourceConflict }">资料冲突</span><span :class="{ done: observerCoverage.generationOverreach }">生成越界</span><span :class="{ done: observerCoverage.toolChain }">工具链</span><span :class="{ done: observerCoverage.expressionCertainty }">表达过满</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W11D4 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
