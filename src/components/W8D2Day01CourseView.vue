...
import { getW8D2FrameworkPlan } from '../course/w8d2Framework'
...
type StructureScenario = 'raw-input' | 'state-flow' | 'decision-path' | 'flow-trace' | 'output-target' | 'qualified-flow-map'
...
type W8D2Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W8D2Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
const plan = (() => {
  const resolved = getW8D2FrameworkPlan(props.lesson.id)
...
function blankEvidence(): W8D2Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '执行流程图观察器', domDraft: '', domEdits: 0, ageInput: 'raw-input', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W8D2Evidence>): W8D2Evidence {
...
const evidence = ref<W8D2Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W8D2Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W8D2Evidence }
...
const currentStructureScenario = computed<StructureScenario>(() => {
  ...
  return ['raw-input', 'state-flow', 'decision-path', 'flow-trace', 'output-target', 'qualified-flow-map'].includes(value) ? value as StructureScenario : 'raw-input'
})
const currentStructureRun = computed(() => {
  const scenario = currentStructureScenario.value
  const common = { target: 'execution-flow-diagram.md', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的输入到输出路径质量，不能证明真实业务结果、JSON/CSV 处理、报错诊断或文件批处理' }
  if (scenario === 'state-flow') return { ...common, label: '变量接力', metric: 'raw_value -> state_variable', section: 'state_variable', conclusion: 'state_flow', action: '把变量接力和中间状态分开标注' }
  if (scenario === 'decision-path') return { ...common, label: '判断路径', metric: 'if/else path', section: 'decision_path', conclusion: 'branch_path', action: '标出条件决定了哪条路径' }
  if (scenario === 'flow-trace') return { ...common, label: '流程痕迹', metric: 'input -> transform -> output', section: 'flow_id', conclusion: 'trace_path', action: '标出输入、转换和出口之间的痕迹' }
  if (scenario === 'output-target') return { ...common, label: '输出契约', metric: 'print / return / file', section: 'output_target', conclusion: 'output_boundary', action: '标出最后交出去的形式' }
  if (scenario === 'qualified-flow-map') return { ...common, label: '合格标注', metric: 'fields=10; evidence_limit=present', section: 'next_step', conclusion: 'ready_for_w8d3', action: '进入 W8D3 前先把输入到输出流程图补完整' }
  return { ...common, label: '输入与原始值', metric: 'script entry + first input', section: 'input_source', conclusion: 'entry_boundary', action: '先写输入来源和第一条原始值' }
})
const structureCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    entryInput: commands.some((item) => item.includes('raw-input')),
    assignmentConversion: commands.some((item) => item.includes('state-flow')),
    branchCondition: commands.some((item) => item.includes('decision-path')),
    loopIteration: commands.some((item) => item.includes('flow-trace')),
    functionOutput: commands.some((item) => item.includes('output-target')),
    qualifiedAnnotation: commands.some((item) => item.includes('qualified-flow-map')),
  }
})
function runStructureObserver() {
  ...
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成输入到输出路径判断，不证明真实业务结果或后续流程已完成。`; publish()
}
function resetStructureObserver() {
  ...
  setSectionComplete('guided-lab', false); announcement.value = '输入到输出流程观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '输入到输出流程实操证据已追加保存；它只证明本地教学模拟中的路径质量。'
}
...
const structureFields = ['flow_id', 'input_source', 'raw_value', 'state_variable', 'transform_step', 'decision_path', 'output_target', 'can_prove', 'cannot_prove', 'next_step'] as const
const deliverableGate = computed(() => {
  ...
  const requiredBoundaryPhrases = ['execution-flow-diagram.md', '教学模拟', '不能证明', 'W8D3']
  ...
  const roleReady = /input_source[\s\S]{0,220}(入口|输入|原始值|变量|转换|判断|输出)/i.test(text)
  const canProveReady = /can_prove[\s\S]{0,180}(流程|路径|入口|转换|判断|输出|教学模拟)/i.test(text)
  const cannotProveReady = /cannot_prove[\s\S]{0,220}(不能证明|真实|生产|自动化|数据清洗|结果正确|上线)/i.test(text)
  const nextStepReady = /next_step[\s\S]{0,160}W8D3/i.test(text)
  if (!roleReady) errors.push('input_source 必须明确入口、输入、原始值、变量、转换、判断或输出。')
  if (!canProveReady) errors.push('can_prove 必须写出流程层能证明的有限事实。')
  if (!cannotProveReady) errors.push('cannot_prove 必须写出不能证明的真实生产或自动化边界。')
  if (!nextStepReady) errors.push('next_step 必须只指向 W8D3。')
...
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过执行流程图字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '执行流程图已作为待审核成果追加保存；真实自动化、数据清洗和生产结果仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W8D2</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 8 · DAY 2 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>input</span><i>→</i><span>raw value</span><i>→</i><span>state variable</span><i>→</i><span>decision / transform</span><i>→</i><span>output</span><p>先固定脚本的输入、原始值、变量接力、转换步骤和判断路径，再把执行流程图写成可复核的字段；任何本地教学模拟通过都不代表真实业务结果、JSON/CSV 清洗或文件批处理已完成。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="输入到输出实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实脚本、文件、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个结构路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="输入到输出教学场景" @change="publish"><option value="raw-input">输入与原始值</option><option value="state-flow">变量接力</option><option value="decision-path">判断路径</option><option value="flow-trace">流程痕迹</option><option value="output-target">输出契约</option><option value="qualified-flow-map">合格标注</option></select></label>
                    ...
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="执行流程图教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="执行流程图实验覆盖"><span :class="{ done: structureCoverage.entryInput }">输入原始值</span><span :class="{ done: structureCoverage.assignmentConversion }">变量接力</span><span :class="{ done: structureCoverage.branchCondition }">判断路径</span><span :class="{ done: structureCoverage.loopIteration }">流程痕迹</span><span :class="{ done: structureCoverage.functionOutput }">输出契约</span><span :class="{ done: structureCoverage.qualifiedAnnotation }">合格标注</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W8D2 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
