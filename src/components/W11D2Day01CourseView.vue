...
type CallScenario = 'tool-unsupported' | 'tool-missing-argument' | 'tool-permission-denied' | 'tool-confirmation-missing' | 'tool-execution-failed' | 'tool-success'
...
type W11D2Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W11D2Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W11D2Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '工具调用六路径门槛观察台', domDraft: '', domEdits: 0, ageInput: 'tool-unsupported', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W11D2Evidence>): W11D2Evidence {
...
const evidence = ref<W11D2Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W11D2Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W11D2Evidence }
...
const currentScenario = computed<CallScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['tool-unsupported', 'tool-missing-argument', 'tool-permission-denied', 'tool-confirmation-missing', 'tool-execution-failed', 'tool-success'].includes(value) ? value as CallScenario : 'tool-unsupported'
})
const currentRun = computed(() => {
  const scenario = currentScenario.value
  const common = { request: 'action_candidate', intent: 'create_return_case', audit: 'runId_appended' }
  if (scenario === 'tool-missing-argument') return { ...common, label: '工具参数缺失', registry: 'registered:v2', proposal: 'create_return_case(reasonCode)', validation: 'ARGUMENT_MISSING:orderId', authorization: 'not_started', confirmation: 'not_started', executor: 'executorCalled=false', receipt: 'request_missing_field', firstFork: '参数校验' }
  if (scenario === 'tool-permission-denied') return { ...common, label: '工具权限拒绝', registry: 'registered:v2', proposal: 'create_return_case(orderId,reasonCode)', validation: 'schema_valid', authorization: 'AUTHORIZATION_DENIED:not_owner', confirmation: 'not_started', executor: 'executorCalled=false', receipt: 'deny_and_handoff', firstFork: '身份权限' }
  if (scenario === 'tool-confirmation-missing') return { ...common, label: '缺少人工确认', registry: 'registered:v2', proposal: 'create_return_case(orderId,reasonCode)', validation: 'schema_valid', authorization: 'authorized', confirmation: 'CONFIRMATION_REQUIRED', executor: 'executorCalled=false', receipt: 'show_preview', firstFork: '风险确认' }
  if (scenario === 'tool-execution-failed') return { ...common, label: '工具执行失败／结果不确定', registry: 'registered:v2', proposal: 'create_return_case(orderId,reasonCode)', validation: 'schema_valid', authorization: 'authorized', confirmation: 'confirmed', executor: 'executorCalled=true', receipt: 'TOOL_TIMEOUT_UNKNOWN+callId', firstFork: '执行器结果' }
  if (scenario === 'tool-success') return { ...common, label: '工具教学成功', registry: 'registered:v2', proposal: 'create_return_case(orderId,reasonCode)', validation: 'schema_valid', authorization: 'authorized', confirmation: 'confirmed', executor: 'executorCalled=true', receipt: 'SIM_CASE_CREATED+callId', firstFork: '无失败分叉' }
  return { ...common, label: '工具未注册', registry: 'UNSUPPORTED_TOOL', proposal: 'rejected', validation: 'not_started', authorization: 'not_started', confirmation: 'not_started', executor: 'executorCalled=false', receipt: 'choose_supported_tool', firstFork: '工具目录' }
})
const observerCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    unsupported: commands.some((item) => item.includes('tool-unsupported')),
    missingArgument: commands.some((item) => item.includes('tool-missing-argument')),
    permissionDenied: commands.some((item) => item.includes('tool-permission-denied')),
    confirmationMissing: commands.some((item) => item.includes('tool-confirmation-missing')),
    executionFailed: commands.some((item) => item.includes('tool-execution-failed')),
    toolSuccess: commands.some((item) => item.includes('tool-success')),
  }
})
function runCallObserver() {
  const run = currentRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `run-call-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentScenario.value}`, output: `${run.firstFork} → ${run.executor} → ${run.receipt}`, tone: currentScenario.value === 'tool-success' ? 'success' : 'normal' })
  evidence.value.guidedLab.observations['latest-call-run'] = `${runId} · ${run.label} · firstFork=${run.firstFork} · ${run.executor} · receipt=${run.receipt}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，不代表真实模型、工具、权限或业务副作用。`; publish()
}
function resetCallObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '工具调用观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(observerCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六类工具调用变式、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '工具调用实操证据已追加保存；它只证明本地教学模拟。'
}
...
function completeDeliverable() {
  ...
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = 'AI 调用图已作为待审核成果追加保存。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W11D2</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 11 · DAY 2 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>意图／registry</span><i>→</i><span>proposal</span><i>→</i><span>validation</span><i>→</i><span>authorization／confirmation</span><i>→</i><span>executor／receipt</span><p>模型只提出动作候选；参数、权限和确认逐层决定是否进入执行器，结果回执再约束交付。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="工具调用实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个工具调用条件</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="工具调用教学场景" @change="publish"><option value="tool-unsupported">工具未注册</option><option value="tool-missing-argument">工具参数缺失</option><option value="tool-permission-denied">工具权限拒绝</option><option value="tool-confirmation-missing">缺少人工确认</option><option value="tool-execution-failed">工具执行失败／结果不确定</option><option value="tool-success">工具教学成功</option></select></label>
                    <dl class="stage-ledger"><div><dt>Request</dt><dd>{{ currentRun.request }}</dd></div><div><dt>Intent</dt><dd>{{ currentRun.intent }}</dd></div><div><dt>Registry</dt><dd>{{ currentRun.registry }}</dd></div><div><dt>Proposal</dt><dd>{{ currentRun.proposal }}</dd></div><div><dt>Validation</dt><dd>{{ currentRun.validation }}</dd></div><div><dt>Authorization</dt><dd>{{ currentRun.authorization }}</dd></div><div><dt>Confirmation</dt><dd>{{ currentRun.confirmation }}</dd></div><div><dt>Executor</dt><dd>{{ currentRun.executor }}</dd></div><div><dt>Receipt</dt><dd>{{ currentRun.receipt }}</dd></div><div><dt>Audit</dt><dd>{{ currentRun.audit }}</dd></div></dl>
                    <p class="first-fork"><b>首次分叉：</b>{{ currentRun.firstFork }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runCallObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetCallObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="工具调用教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="工具调用实验覆盖"><span :class="{ done: observerCoverage.unsupported }">工具未注册</span><span :class="{ done: observerCoverage.missingArgument }">参数缺失</span><span :class="{ done: observerCoverage.permissionDenied }">权限拒绝</span><span :class="{ done: observerCoverage.confirmationMissing }">缺少确认</span><span :class="{ done: observerCoverage.executionFailed }">执行失败／未知</span><span :class="{ done: observerCoverage.toolSuccess }">教学成功</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W11D2 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
