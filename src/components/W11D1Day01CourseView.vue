...
type SystemScenario = 'complete' | 'missing-context' | 'capability-unavailable' | 'policy-blocked'
...
type W11D1Evidence = DailyLessonEvidenceState & {
  frameworkChapters: Record<Day01ChapterId, ChapterEvidence>
}

const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W11D1Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W11D1Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: 'AI 系统链分层观察器', domDraft: '', domEdits: 0, ageInput: 'complete', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W11D1Evidence>): W11D1Evidence {
  ...
  return {
    ...base, ...source, schemaVersion: 2, lessonId: props.lesson.id,
...
const evidence = ref<W11D1Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W11D1Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W11D1Evidence }
...
const currentScenario = computed<SystemScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['complete', 'missing-context', 'capability-unavailable', 'policy-blocked'].includes(value) ? value as SystemScenario : 'complete'
})
const currentRun = computed(() => {
  const scenario = currentScenario.value
  const common = { request: 'received', audit: 'appended' }
  if (scenario === 'missing-context') return { ...common, label: '上下文缺失', context: 'missing_payment_channel', model: 'not_called', capability: 'not_requested', policy: 'clarify', delivery: 'ask_user', firstFork: '上下文组装' }
  if (scenario === 'capability-unavailable') return { ...common, label: '能力不可用', context: 'ready', model: 'candidate_ready', capability: 'unavailable', policy: 'degrade', delivery: 'human_handoff', firstFork: '能力槽位' }
  if (scenario === 'policy-blocked') return { ...common, label: '规则阻断', context: 'ready', model: 'candidate_ready', capability: 'action_pending', policy: 'blocked_high_risk', delivery: 'human_handoff', firstFork: '业务规则' }
  return { ...common, label: '完整链', context: 'ready', model: 'candidate_ready', capability: 'not_required', policy: 'allowed_explanation_only', delivery: 'answer', firstFork: '无失败分叉' }
})
const observerCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return { complete: commands.some((item) => item.includes('complete')), missingContext: commands.some((item) => item.includes('missing-context')), capability: commands.some((item) => item.includes('capability-unavailable')), policy: commands.some((item) => item.includes('policy-blocked')) }
})
function runSystemObserver() {
  const run = currentRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `run-ai-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentScenario.value}`, output: `${run.firstFork} → ${run.delivery}`, tone: currentScenario.value === 'complete' ? 'success' : 'normal' })
  evidence.value.guidedLab.observations['latest-system-run'] = `${runId} · ${run.label} · firstFork=${run.firstFork} · delivery=${run.delivery}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，不代表真实模型或业务系统。`; publish()
}
function resetSystemObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '教学观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(observerCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、四类系统变式、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '系统链实操证据已追加保存；它只证明本地教学模拟。'
}
...
function completeDeliverable() {
  ...
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = 'AI 系统链路说明已作为待审核成果追加保存。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W11D1</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 11 · DAY 1 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>产品请求</span><i>→</i><span>上下文</span><i>→</i><span>模型候选</span><i>→</i><span>能力槽位</span><i>→</i><span>业务规则</span><i>→</i><span>交付</span><i>→</i><span>审计</span><p>RAG 与工具今天只定位在“能力槽位”；具体检索机制、权限、真实动作和副作用不在本课首教。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel"><div class="lab-title"><div><span>教学模拟</span><h3>{{ lesson.guidedLab.title }}</h3><p>{{ lesson.guidedLab.goal }}</p></div><small>{{ lesson.guidedLab.safety }}</small></div><label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="AI 系统链实验预测" @input="publish" /></label>
                  <div class="system-console"><p class="simulation-label">教学模拟 · 不调用真实模型、RAG、工具、账号、Token、公司数据或生产系统</p><label><span>只改变一个系统条件</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="AI 系统链教学场景" @change="publish"><option value="complete">完整链</option><option value="missing-context">上下文缺失</option><option value="capability-unavailable">能力不可用</option><option value="policy-blocked">规则阻断</option></select></label><dl class="stage-ledger"><div><dt>Request</dt><dd>{{ currentRun.request }}</dd></div><div><dt>Context</dt><dd>{{ currentRun.context }}</dd></div><div><dt>Model</dt><dd>{{ currentRun.model }}</dd></div><div><dt>Capability</dt><dd>{{ currentRun.capability }}</dd></div><div><dt>Policy</dt><dd>{{ currentRun.policy }}</dd></div><div><dt>Delivery</dt><dd>{{ currentRun.delivery }}</dd></div><div><dt>Audit</dt><dd>{{ currentRun.audit }}</dd></div></dl><p class="first-fork"><b>首次分叉：</b>{{ currentRun.firstFork }}</p><div class="console-actions"><button type="button" :disabled="readonly" @click="runSystemObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetSystemObserver">全部清空并重做</button></div><ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="AI 系统链教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol><div class="simulation-coverage" aria-label="AI 系统链实验覆盖"><span :class="{ done: observerCoverage.complete }">完整链</span><span :class="{ done: observerCoverage.missingContext }">上下文缺失</span><span :class="{ done: observerCoverage.capability }">能力不可用</span><span :class="{ done: observerCoverage.policy }">规则阻断</span></div></div>
                  <ol class="lab-steps"><li v-for="item in guidedSteps" :key="item.index"><label><input v-model="evidence.guidedLab.stepComplete[String(item.index)]" type="checkbox" :disabled="readonly" @change="publish"><span><b>{{ item.step.title }}</b>{{ item.step.action }}</span></label><p>观察：{{ item.step.observe }}</p><small>能证明：{{ item.step.proves }}；不能证明：{{ item.step.cannotProve }}</small></li></ol><div class="record-grid"><label v-for="item in guidedRecords" :key="item.index"><span>{{ item.prompt }}</span><textarea v-model="evidence.guidedLab.records[String(item.index)]" rows="3" :readonly="readonly" @input="publish" /></label></div><div class="check-list"><label v-for="(item, index) in lesson.guidedLab.passCriteria" :key="item"><input v-model="evidence.guidedLab.passChecks[String(index)]" type="checkbox" :disabled="readonly" @change="publish">{{ item }}</label></div><div class="panel-actions"><button type="button" :disabled="readonly" @click="completeGuidedLab">保存引导实验尝试</button><button type="button" class="secondary" :disabled="readonly" @click="resetSystemObserver">全部清空并重做</button></div></div>
              </template>

              <template v-if="chapter.number === 6">
                <div class="boundary-table"><h3>看到什么，最多判断到哪里</h3><dl><div><dt>看到模型候选</dt><dd>生成阶段有输出；不能证明内容真实、规则允许或动作完成。</dd></div><div><dt>看到能力不可用</dt><dd>本次模拟能力槽位返回失败；不能指定真实 RAG、工具或内部根因。</dd></div><div><dt>看到规则阻断</dt><dd>本次模拟命中确定性门槛；不能证明真实策略或权限已经配置。</dd></div><div><dt>看到人工接管</dt><dd>系统选择兜底路径；不能证明人工已经处理或问题已解决。</dd></div></dl></div>
                <div class="independent-panel"><h3>{{ lesson.independentLab.title }}</h3><p>{{ lesson.independentLab.scenario }}</p><label><span>改变一个条件</span><select v-model="evidence.independentLab.changedCondition" :disabled="readonly" @change="publish"><option value="">请选择</option><option v-for="item in lesson.independentLab.changedConditions" :key="item" :value="item">{{ item }}</option></select></label><label><span>先预测</span><small>{{ lesson.independentLab.predictionPrompt }}</small><textarea v-model="evidence.independentLab.prediction" rows="3" :readonly="readonly" @input="publish" /></label><label><span>操作计划</span><textarea v-model="evidence.independentLab.plan" rows="3" :readonly="readonly" aria-label="独立 AI 系统变式操作计划" @input="publish" /></label><label><span>原始证据</span><textarea v-model="evidence.independentLab.evidence" rows="3" :readonly="readonly" aria-label="独立 AI 系统变式原始证据" @input="publish" /></label><label><span>观察结果</span><textarea v-model="evidence.independentLab.result" rows="3" :readonly="readonly" aria-label="独立 AI 系统变式观察结果" @input="publish" /></label><label><span>四句有限结论</span><textarea v-model="evidence.independentLab.conclusion" rows="4" :readonly="readonly" aria-label="独立 AI 系统变式有限结论" @input="publish" /></label><div class="check-list"><label v-for="(item, index) in lesson.independentLab.passCriteria" :key="item"><input v-model="evidence.independentLab.passChecks[String(index)]" type="checkbox" :disabled="readonly" @change="publish">{{ item }}</label></div><div class="panel-actions"><button type="button" :disabled="readonly" @click="completeIndependentLab">保存独立变式尝试</button><button type="button" class="secondary" :disabled="readonly" @click="resetIndependentLab">清空并重做</button></div></div>
              </template>

              <template v-if="chapter.number === 7">
                <div class="practice-workshop"><h3>针对性练习</h3><p>先作答，再看逐项解释；模型候选、系统决定与真实动作不能混写。</p><article v-for="(exercise, index) in exerciseSet" :key="exercise.id"><header><span>{{ index + 1 }}</span><div><h4>{{ exercise.prompt }}</h4><small>{{ exercise.categories.join(' · ') }}</small></div></header><div class="answer-options"><label v-for="(option, optionIndex) in exercise.options" :key="option.label"><input v-model="exerciseState(exercise).selectedIndex" type="radio" :name="exercise.id" :value="optionIndex" :disabled="readonly || exerciseState(exercise).submitted" @change="publish"><span>{{ String.fromCharCode(65 + optionIndex) }}</span>{{ option.label }}</label></div><div v-if="exerciseState(exercise).submitted" :class="['exercise-feedback', exerciseState(exercise).selfAssessment]"><b>{{ exerciseState(exercise).selfAssessment === 'pass' ? '当前通过' : '对照后修正' }}</b><p>{{ exercise.referenceAnswer }}</p><ol><li v-for="reason in exercise.reasoning" :key="reason">{{ reason }}</li></ol><p><b>逐项解释：</b>{{ exercise.options?.map((option) => `${option.label}：${option.rationale}；${option.couldBeTrueWhen}`).join('；') }}</p><p><b>常见错因：</b>{{ exercise.commonErrors.map((item) => `${item.error}：${item.reason}`).join('；') }}</p></div><div class="panel-actions"><button v-if="!exerciseState(exercise).submitted" type="button" :disabled="readonly" @click="submitExercise(exercise)">提交本题</button><button type="button" class="secondary" :disabled="readonly" @click="resetExercise(exercise)">清空并重做</button></div></article></div>
        ...
        <aside class="w1-route" aria-label="W11D1 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
