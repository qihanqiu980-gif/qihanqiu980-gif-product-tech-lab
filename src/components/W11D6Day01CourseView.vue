...
type ReportScenario = 'scope-version' | 'result-summary' | 'failure-analysis' | 'risk-recommendation' | 'launch-decision' | 'follow-up-plan'
...
type W11D6Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W11D6Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W11D6Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: 'AI 客服综合评测报告六段观察台', domDraft: '', domEdits: 0, ageInput: 'scope-version', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W11D6Evidence>): W11D6Evidence {
...
const evidence = ref<W11D6Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W11D6Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W11D6Evidence }
...
const currentReportScenario = computed<ReportScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['scope-version', 'result-summary', 'failure-analysis', 'risk-recommendation', 'launch-decision', 'follow-up-plan'].includes(value) ? value as ReportScenario : 'scope-version'
})
const currentReportRun = computed(() => {
  const scenario = currentReportScenario.value
  const common = { target: 'ai-customer-service-report-sim-v1', mode: '教学模拟', evidenceLimit: '不能证明真实生产表现、线上代表性、正式合规结论或上线审批' }
  if (scenario === 'result-summary') return { ...common, label: '结果摘要', metric: 'pass_count=14; fail_count=6; hard_gate_failed=2', section: 'result_summary', conclusion: 'gate_status=failed', action: '先处理 hard gate 失败后复测' }
  if (scenario === 'failure-analysis') return { ...common, label: '失败分析', metric: 'privacy_refusal_missing=2; tool_receipt_unknown=2; overcertainty=2', section: 'failure_analysis', conclusion: 'top_failure_modes=3', action: '隐私拒答和工具回执优先修复' }
  if (scenario === 'risk-recommendation') return { ...common, label: '风险建议', metric: 'safety_policy_block; timeout_degradation; human_escalation', section: 'risk_recommendation', conclusion: 'risk_blocker=privacy', action: '安全阻断、超时降级、高价值争议转人工' }
  if (scenario === 'launch-decision') return { ...common, label: '有限发布决策', metric: 'gate_status=failed; hard_gate_failed=2', section: 'launch_decision', conclusion: 'fix_before_pilot', action: '先修复再进入有限试点复测' }
  if (scenario === 'follow-up-plan') return { ...common, label: '后续补证计划', metric: 'missing_evidence=logs/billing/p95/compliance', section: 'follow_up_plan', conclusion: 'owners=PM/Eng/Safety/Ops', action: '分派真实日志、账单、延迟和合规补证' }
  return { ...common, label: '范围与版本', metric: 'target_version=ai-cs-v0.3; eval_set_id=w11-teaching-20', section: 'scope', conclusion: 'out_of_scope=production_launch', action: '锁定报告范围并声明排除项' }
})
const reportCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    scopeVersion: commands.some((item) => item.includes('scope-version')),
    resultSummary: commands.some((item) => item.includes('result-summary')),
    failureAnalysis: commands.some((item) => item.includes('failure-analysis')),
    riskRecommendation: commands.some((item) => item.includes('risk-recommendation')),
    launchDecision: commands.some((item) => item.includes('launch-decision')),
    followUpPlan: commands.some((item) => item.includes('follow-up-plan')),
  }
})
function runReportObserver() {
  const run = currentReportRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `report-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentReportScenario.value}`, output: `${run.section} → ${run.conclusion}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-report-run'] = `${runId} · ${run.label} · section=${run.section} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成综合评测报告段落，不证明真实生产表现、正式合规或上线审批。`; publish()
}
function resetReportObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '综合报告观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(reportCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六段综合报告路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '综合评测报告实操证据已追加保存；它只证明本地教学模拟中的报告合同质量。'
}
...
const reportSections = ['scope', 'result_summary', 'failure_analysis', 'risk_recommendation', 'launch_decision', 'follow_up_plan'] as const
const allowedLaunchDecisions = ['allow_pilot', 'fix_before_pilot', 'block', 'manual_review']
const deliverableGate = computed(() => {
  const lines = evidence.value.deliverable.draft.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const rows: Record<string, unknown>[] = []
  const sectionCounts = Object.fromEntries(reportSections.map((section) => [section, 0])) as Record<(typeof reportSections)[number], number>
  ...
  const requiredText = ['section_id', 'report_part', 'evidence_used', 'conclusion', 'recommendation', 'evidence_limit']
  const ids: string[] = []
  let launchDecisionRows = 0
  let followUpRows = 0
  let boundaryRows = 0
  let teachingRows = 0
  for (const [index, row] of rows.entries()) {
    ...
    for (const section of reportSections) if (row.section_id === section || rowText.includes(section)) sectionCounts[section] += 1
    if (typeof row.section_id === 'string') ids.push(row.section_id.trim().toLowerCase())
    if (row.section_id === 'launch_decision') {
      launchDecisionRows += 1
      if (typeof row.conclusion !== 'string' || !allowedLaunchDecisions.includes(row.conclusion)) errors.push(`launch_decision 行 conclusion 必须是 ${allowedLaunchDecisions.join('/')}。`)
    }
    if (row.section_id === 'follow_up_plan') followUpRows += 1
    if (rowText.includes('不能证明') || rowText.includes('cannot prove')) boundaryRows += 1
    if (rowText.includes('教学') || rowText.includes('teaching')) teachingRows += 1
    const overclaimPhrases = [['真实生产', '已证明'], ['正式合规', '已证明'], ['上线审批', '已通过'], ['可以', '全量上线']]
    if (overclaimPhrases.some(([left, right]) => rowText.includes(left) && rowText.includes(right))) errors.push(`第 ${index + 1} 行 evidence_limit 不能声称生产表现或上线审批已被教学模拟证明。`)
  }
  if (new Set(ids).size !== ids.length) errors.push('section_id 存在重复。')
  for (const section of reportSections) if (sectionCounts[section] < 1) errors.push(`${section} 必须出现 1 次，当前 ${sectionCounts[section]} 次。`)
  if (launchDecisionRows !== 1) errors.push('必须恰好 1 行 launch_decision。')
  if (followUpRows !== 1) errors.push('必须恰好 1 行 follow_up_plan。')
  if (boundaryRows < 6) errors.push('每行 evidence_limit 都必须写明不能证明什么。')
  if (teachingRows < 6) errors.push('每行 evidence_limit 都必须标注本地教学模拟边界。')
  return { valid: errors.length === 0, errors, lineCount: lines.length, sectionCounts, launchDecisionRows, followUpRows, boundaryRows }
})
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过 6 行综合评测报告门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = 'AI 客服综合评测报告已作为待审核成果追加保存；真实生产表现、正式合规和上线审批仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W11D6</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 11 · DAY 6 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>scope</span><i>→</i><span>result summary</span><i>→</i><span>failure analysis</span><i>→</i><span>risk recommendation</span><i>→</i><span>launch decision</span><p>先固定报告范围和版本，再合成结果、失败、风险和有限发布建议；任何本地报告通过都不代表真实生产表现、正式合规或上线审批已被证明。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="综合评测报告实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个报告段落</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="综合评测报告教学场景" @change="publish"><option value="scope-version">范围与版本</option><option value="result-summary">结果摘要</option><option value="failure-analysis">失败分析</option><option value="risk-recommendation">风险建议</option><option value="launch-decision">有限发布决策</option><option value="follow-up-plan">后续补证计划</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentReportRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentReportRun.mode }}</dd></div><div><dt>Metric</dt><dd>{{ currentReportRun.metric }}</dd></div><div><dt>Report section</dt><dd>{{ currentReportRun.section }}</dd></div><div><dt>Conclusion</dt><dd>{{ currentReportRun.conclusion }}</dd></div><div><dt>Recommendation</dt><dd>{{ currentReportRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentReportRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>报告段落：</b>{{ currentReportRun.section }} → {{ currentReportRun.conclusion }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runReportObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetReportObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="综合评测报告教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="综合评测报告实验覆盖"><span :class="{ done: reportCoverage.scopeVersion }">范围版本</span><span :class="{ done: reportCoverage.resultSummary }">结果摘要</span><span :class="{ done: reportCoverage.failureAnalysis }">失败分析</span><span :class="{ done: reportCoverage.riskRecommendation }">风险建议</span><span :class="{ done: reportCoverage.launchDecision }">发布决策</span><span :class="{ done: reportCoverage.followUpPlan }">补证计划</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W11D6 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
