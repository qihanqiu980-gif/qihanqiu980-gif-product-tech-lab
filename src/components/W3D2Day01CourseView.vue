...
type SelectOrderScenario = 'missing-from' | 'where-before-source' | 'select-column-mismatch' | 'order-without-rule' | 'limit-overclaim' | 'qualified-select-order'
...
type W3D2Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W3D2Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W3D2Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: 'SELECT 顺序六路径观察台', domDraft: '', domEdits: 0, ageInput: 'missing-from', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W3D2Evidence>): W3D2Evidence {
...
const evidence = ref<W3D2Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W3D2Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W3D2Evidence }
...
const selectOrderScenarioValues = ['missing-from', 'where-before-source', 'select-column-mismatch', 'order-without-rule', 'limit-overclaim', 'qualified-select-order'] as const
const currentSelectOrderScenario = computed<SelectOrderScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return selectOrderScenarioValues.includes(value as SelectOrderScenario) ? value as SelectOrderScenario : 'missing-from'
})
const currentSelectOrderRun = computed(() => {
  const scenario = currentSelectOrderScenario.value
  const common = { target: 'sql-execution-order.md', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的单表 SELECT 执行顺序和样本窗口，不能证明真实生产数据、聚合、GROUP BY、JOIN 或业务指标正确' }
  if (scenario === 'where-before-source') return { ...common, label: 'WHERE 早于数据源', metric: 'where_gate=drawn_before_from; base_table=unknown', field: 'execution_order', conclusion: 'source_missing_before_filter', action: '把 FROM orders 放回第一阶段，再解释 WHERE 只过滤候选行' }
  if (scenario === 'select-column-mismatch') return { ...common, label: '列清单不匹配', metric: 'select_columns=unknown_field; observed_result=column_error', field: 'select_columns', conclusion: 'column_not_traceable', action: '回到 orders 字段预览，确认输出列来自 base_table' }
  if (scenario === 'order-without-rule') return { ...common, label: '排序规则缺失', metric: 'sort_rule=missing; claim=latest_first', field: 'sort_rule', conclusion: 'order_claim_overreach', action: '补 ORDER BY 字段和方向；若未指定，只能说当前展示顺序' }
  if (scenario === 'limit-overclaim') return { ...common, label: 'LIMIT 越界', metric: 'limit_window=5; claim=all_rows_are_5', field: 'cannot_prove', conclusion: 'sample_window_overclaim', action: '把结论退回到样本窗口，等待 W3D3+ 查询和后续聚合' }
  if (scenario === 'qualified-select-order') return { ...common, label: '合格 SELECT 顺序图', metric: 'fields=12; order=FROM-WHERE-SELECT-ORDER-LIMIT; next=W3D3', field: 'sql_execution_order', conclusion: 'select_order_ready_for_w3d3', action: '保存执行顺序图，下一步只进入 W3D3 基础查询练习' }
  return { ...common, label: '缺少 FROM', metric: 'base_table=missing; selected_column=order_id', field: 'base_table', conclusion: 'missing_source_table', action: '先补 FROM orders，再解释字段和条件从哪里取得上下文' }
})
const selectOrderCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    missingFrom: commands.some((item) => item.includes('missing-from')),
    whereBeforeSource: commands.some((item) => item.includes('where-before-source')),
    selectColumnMismatch: commands.some((item) => item.includes('select-column-mismatch')),
    orderWithoutRule: commands.some((item) => item.includes('order-without-rule')),
    limitOverclaim: commands.some((item) => item.includes('limit-overclaim')),
    qualifiedSelectOrder: commands.some((item) => item.includes('qualified-select-order')),
  }
})
function runSelectOrderObserver() {
  const run = currentSelectOrderRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `w3d2-sqlite-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentSelectOrderScenario.value}`, output: `${run.field} → ${run.conclusion}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-selectOrder-run'] = `${runId} · ${run.label} · field=${run.field} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成 SELECT 顺序字段判断，不证明真实生产数据、聚合、GROUP BY、JOIN 或业务指标正确。`; publish()
}
function resetSelectOrderObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = 'SELECT 顺序观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(selectOrderCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六条 SELECT 顺序路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = 'SELECT 顺序实操证据已追加保存；它只证明本地教学模拟中的单表执行顺序可解释。'
}
...
const validationFields = ['business_question', 'base_table', 'source_rows', 'where_gate', 'select_columns', 'sort_rule', 'limit_window', 'execution_order', 'observed_result', 'can_prove', 'cannot_prove', 'next_sql_task'] as const
const deliverableGate = computed(() => {
  ...
  const requiredBoundaryPhrases = ['教学模拟', '不能证明', 'W3D3', 'FROM', 'WHERE', 'SELECT', 'ORDER BY', 'LIMIT']
  ...
  const sourceRowsReady = /source_rows[\s\S]{0,260}(FROM|候选行|orders)/i.test(text)
  const whereReady = /where_gate[\s\S]{0,320}(WHERE|status|paid|条件|行)/i.test(text)
  const selectReady = /select_columns[\s\S]{0,320}(SELECT|order_id|user_id|status|paid_at|列)/i.test(text)
  const sortReady = /sort_rule[\s\S]{0,260}(ORDER BY|paid_at|DESC|未指定|排序)/i.test(text)
  const limitReady = /limit_window[\s\S]{0,220}(LIMIT|样本|5|窗口)/i.test(text)
  const orderReady = /execution_order[\s\S]{0,360}FROM[\s\S]{0,120}WHERE[\s\S]{0,120}SELECT[\s\S]{0,120}ORDER BY[\s\S]{0,120}LIMIT/i.test(text)
  const observedReady = /observed_result[\s\S]{0,300}(教学模拟|行|列|样本|结果)/i.test(text)
  const canReady = /can_prove[\s\S]{0,360}(能证明|SELECT|执行顺序|阶段|样本窗口)/i.test(text)
  const cannotReady = /cannot_prove[\s\S]{0,560}(不能证明|真实生产|聚合|GROUP BY|JOIN|业务指标|全量)/i.test(text)
  const nextReady = /next_sql_task[\s\S]{0,180}W3D3/i.test(text)
  if (!questionReady) errors.push('business_question 必须写出具体查询问题。')
  if (!tableReady) errors.push('base_table 必须写出 orders。')
  if (!sourceRowsReady) errors.push('source_rows 必须解释 FROM 产生候选行。')
  if (!whereReady) errors.push('where_gate 必须说明 WHERE 行过滤或未设置。')
  if (!selectReady) errors.push('select_columns 必须写出 SELECT 输出列。')
  if (!sortReady) errors.push('sort_rule 必须写出 ORDER BY 或未指定排序边界。')
  if (!limitReady) errors.push('limit_window 必须写出 LIMIT 样本窗口。')
  if (!orderReady) errors.push('execution_order 必须按 FROM → WHERE → SELECT → ORDER BY → LIMIT。')
  if (!observedReady) errors.push('observed_result 必须说明本地教学模拟看到的行、列或样本。')
  if (!canReady) errors.push('can_prove 必须限定到单表 SELECT 执行顺序或结果窗口。')
  if (!cannotReady) errors.push('cannot_prove 必须写出真实生产、全量、聚合、GROUP BY、JOIN 或业务指标边界。')
  if (!nextReady) errors.push('next_sql_task 只能指向 W3D3。')
...
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过 SELECT 顺序记录字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = 'SQL 执行顺序图已作为待审核成果追加保存；真实生产数据、聚合、GROUP BY、JOIN 和业务指标仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W3D2</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 3 · DAY 2 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>business_question</span><i>→</i><span>FROM</span><i>→</i><span>WHERE</span><i>→</i><span>SELECT</span><i>→</i><span>ORDER BY</span><i>→</i><span>LIMIT</span><i>→</i><span>cannot_prove</span><p>先把问题落到 orders 数据源，再让 WHERE 选择行、SELECT 选择列、ORDER BY 决定展示顺序、LIMIT 截取样本窗口，最后写出 can_prove 和 cannot_prove。任何本地教学模拟都不能代表真实生产数据、聚合、GROUP BY、JOIN 或业务指标正确。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="SELECT 顺序 实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 只读取本地 SQLite 教学表 orders，不调用真实数据库、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个 SELECT 顺序质量路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="SELECT 顺序 教学场景" @change="publish"><option value="missing-from">缺少 FROM</option><option value="where-before-source">WHERE 早于数据源</option><option value="select-column-mismatch">列清单不匹配</option><option value="order-without-rule">排序规则缺失</option><option value="limit-overclaim">LIMIT 越界</option><option value="qualified-select-order">合格 SELECT 顺序图</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentSelectOrderRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentSelectOrderRun.mode }}</dd></div><div><dt>Field signal</dt><dd>{{ currentSelectOrderRun.metric }}</dd></div><div><dt>日志 field</dt><dd>{{ currentSelectOrderRun.field }}</dd></div><div><dt>Conclusion</dt><dd>{{ currentSelectOrderRun.conclusion }}</dd></div><div><dt>Revision action</dt><dd>{{ currentSelectOrderRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentSelectOrderRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>执行顺序字段：</b>{{ currentSelectOrderRun.field }} → {{ currentSelectOrderRun.conclusion }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runSelectOrderObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetSelectOrderObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="SELECT 顺序 教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="SELECT 顺序 实验覆盖"><span :class="{ done: selectOrderCoverage.missingFrom }">缺少 FROM</span><span :class="{ done: selectOrderCoverage.whereBeforeSource }">WHERE 早于数据源</span><span :class="{ done: selectOrderCoverage.selectColumnMismatch }">列清单不匹配</span><span :class="{ done: selectOrderCoverage.orderWithoutRule }">排序规则缺失</span><span :class="{ done: selectOrderCoverage.limitOverclaim }">LIMIT 越界</span><span :class="{ done: selectOrderCoverage.qualifiedSelectOrder }">合格顺序图</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W3D2 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
