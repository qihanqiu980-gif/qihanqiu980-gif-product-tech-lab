...
type BriefScenario = 'problem-too-broad' | 'user-missing' | 'evidence-missing' | 'metric-not-measurable' | 'scope-creep' | 'qualified-brief'
...
type W12D1Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W12D1Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W12D1Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '项目 Brief 六路径观察台', domDraft: '', domEdits: 0, ageInput: 'problem-too-broad', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W12D1Evidence>): W12D1Evidence {
...
const evidence = ref<W12D1Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W12D1Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W12D1Evidence }
...
const currentBriefScenario = computed<BriefScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['problem-too-broad', 'user-missing', 'evidence-missing', 'metric-not-measurable', 'scope-creep', 'qualified-brief'].includes(value) ? value as BriefScenario : 'problem-too-broad'
})
const currentBriefRun = computed(() => {
  const scenario = currentBriefScenario.value
  const common = { target: 'project-brief.md', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的 Brief 合同质量，不能证明真实业务价值、系统方案、验证日志、风险登记或答辩通过' }
  if (scenario === 'user-missing') return { ...common, label: '用户缺失', metric: 'target_user=missing; stakeholders=generic', section: 'target_user', conclusion: 'needs_revision', action: '补明确目标用户与暂不覆盖人群' }
  if (scenario === 'evidence-missing') return { ...common, label: '证据缺失', metric: 'evidence_used=claim_only; missing_evidence=empty', section: 'evidence_used', conclusion: 'evidence_boundary_failed', action: '拆成已有教学证据与缺失证据' }
  if (scenario === 'metric-not-measurable') return { ...common, label: '指标不可测', metric: 'success_metric=提升体验', section: 'success_metric', conclusion: 'metric_not_observable', action: '改为可观察对象、方向和最低标准' }
  if (scenario === 'scope-creep') return { ...common, label: '范围蔓延', metric: 'claims=D2-D6 completed', section: 'scope_non_goals', conclusion: 'scope_overclaim', action: '写清本日只产出项目 Brief' }
  if (scenario === 'qualified-brief') return { ...common, label: '合格 Brief', metric: 'fields=13; evidence_limit=present', section: 'brief_decision', conclusion: 'ready_for_w12d2', action: '进入 W12D2 前准备系统与数据方案问题清单' }
  return { ...common, label: '问题过宽', metric: 'user_problem=提升体验; target_user=all', section: 'user_problem', conclusion: 'problem_too_broad', action: '重写为具体用户、场景、阻碍和影响' }
})
const briefCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    problemTooBroad: commands.some((item) => item.includes('problem-too-broad')),
    userMissing: commands.some((item) => item.includes('user-missing')),
    evidenceMissing: commands.some((item) => item.includes('evidence-missing')),
    metricNotMeasurable: commands.some((item) => item.includes('metric-not-measurable')),
    scopeCreep: commands.some((item) => item.includes('scope-creep')),
    qualifiedBrief: commands.some((item) => item.includes('qualified-brief')),
  }
})
function runBriefObserver() {
  const run = currentBriefRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `brief-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentBriefScenario.value}`, output: `${run.section} → ${run.conclusion}`, tone: 'normal' })
  evidence.value.guidedLab.observations['latest-brief-run'] = `${runId} · ${run.label} · section=${run.section} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成项目 Brief 字段判断，不证明真实业务价值或后续文档已完成。`; publish()
}
function resetBriefObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '项目 Brief 观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(briefCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六条项目 Brief 路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '项目 Brief 实操证据已追加保存；它只证明本地教学模拟中的 Brief 合同质量。'
}
...
const briefFields = ['project_title', 'user_problem', 'target_user', 'stakeholders', 'evidence_used', 'missing_evidence', 'success_metric', 'scope', 'non_goals', 'assumptions', 'largest_unknown', 'next_step', 'evidence_limit'] as const
const deliverableGate = computed(() => {
  const text = evidence.value.deliverable.draft
  const lower = text.toLowerCase()
  const lines = text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const fieldPresence = Object.fromEntries(briefFields.map((field) => [field, lower.includes(field)])) as Record<(typeof briefFields)[number], boolean>
  for (const field of briefFields) if (!fieldPresence[field]) errors.push(`缺少字段：${field}。`)
  const requiredBoundaryPhrases = ['教学模拟', '不能证明', 'W12D2']
  for (const phrase of requiredBoundaryPhrases) if (!text.includes(phrase)) errors.push(`必须写明边界或下一步：${phrase}。`)
  const forbiddenClaims = [
    ['系统与数据方案', '已完成'], ['验证日志', '已完成'], ['风险登记册', '已完成'], ['作品集', '已完成'],
    ['答辩', '已通过'], ['真实业务价值', '已证明'], ['真实生产表现', '已证明'],
  ]
  for (const [subject, claim] of forbiddenClaims) {
    if (text.includes(subject) && text.includes(claim)) errors.push(`不得越界声明：${subject}${claim}。`)
  }
  const evidenceReady = /evidence_used[\s\S]{0,160}(W1D5|W11D3|W11D4|W11D5|W11D6|教学)/i.test(text)
  const missingReady = /missing_evidence[\s\S]{0,220}(真实|生产|用户|日志|工单|访谈|未完成|缺)/i.test(text)
  const metricReady = /success_metric[\s\S]{0,180}(率|时间|数量|覆盖|一致|达到|%|分钟)/i.test(text)
  const nextStepReady = /next_step[\s\S]{0,160}W12D2/i.test(text)
  if (!evidenceReady) errors.push('evidence_used 必须引用真实已有成果或明确教学模拟来源。')
  if (!missingReady) errors.push('missing_evidence 必须列出影响判断的真实缺口。')
  if (!metricReady) errors.push('success_metric 必须可观察。')
  if (!nextStepReady) errors.push('next_step 必须只指向 W12D2 准备。')
  return { valid: errors.length === 0, errors, lineCount: lines.length, fieldPresence, presentFields: briefFields.filter((field) => fieldPresence[field]).length }
})
const deliverableFieldStatus = computed(() => briefFields.map((field) => ({ field, done: deliverableGate.value.fieldPresence[field] })))
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过项目 Brief 字段门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '项目 Brief 已作为待审核成果追加保存；真实业务价值、方案有效性和作品集完成度仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W12D1</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 12 · DAY 1 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>problem</span><i>→</i><span>user</span><i>→</i><span>evidence</span><i>→</i><span>metric</span><i>→</i><span>next step</span><p>先固定值得解决的问题，再把已有证据和缺失证据分开，最后形成可观察指标和有限下一步；任何本地 Brief 通过都不代表真实业务价值、系统方案、验证日志或答辩已被证明。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="项目 Brief 实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个 Brief 质量路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="项目 Brief 教学场景" @change="publish"><option value="problem-too-broad">问题过宽</option><option value="user-missing">用户缺失</option><option value="evidence-missing">证据缺失</option><option value="metric-not-measurable">指标不可测</option><option value="scope-creep">范围蔓延</option><option value="qualified-brief">合格 Brief</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentBriefRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentBriefRun.mode }}</dd></div><div><dt>Field signal</dt><dd>{{ currentBriefRun.metric }}</dd></div><div><dt>Brief field</dt><dd>{{ currentBriefRun.section }}</dd></div><div><dt>Conclusion</dt><dd>{{ currentBriefRun.conclusion }}</dd></div><div><dt>Revision action</dt><dd>{{ currentBriefRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentBriefRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>Brief 字段：</b>{{ currentBriefRun.section }} → {{ currentBriefRun.conclusion }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runBriefObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetBriefObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="项目 Brief 教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="项目 Brief 实验覆盖"><span :class="{ done: briefCoverage.problemTooBroad }">问题过宽</span><span :class="{ done: briefCoverage.userMissing }">用户缺失</span><span :class="{ done: briefCoverage.evidenceMissing }">证据缺失</span><span :class="{ done: briefCoverage.metricNotMeasurable }">指标不可测</span><span :class="{ done: briefCoverage.scopeCreep }">范围蔓延</span><span :class="{ done: briefCoverage.qualifiedBrief }">合格 Brief</span></div>
                  </div>
