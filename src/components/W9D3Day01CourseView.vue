...
import { getW9D3FrameworkPlan } from '../course/w9d3Framework'
...
type StructureScenario = 'input-contract' | 'quality-snapshot' | 'type-coercion' | 'issue-flags' | 'split-and-log' | 'reconciliation' | 'qualified-clean-script'
...
type W9D3Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W9D3Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
const plan = (() => {
  const resolved = getW9D3FrameworkPlan(props.lesson.id)
...
function blankEvidence(): W9D3Evidence {
  return {
    schemaVersion: 2,
    ...
    expandedConceptIds: [],
    completedSections: [],
    sandbox: { activePanel: 'elements', previewHeadline: '订单清洗观察器', domDraft: '', domEdits: 0, ageInput: 'input-contract', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W9D3Evidence>): W9D3Evidence {
...
const evidence = ref<W9D3Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W9D3Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W9D3Evidence }
function publish() {
...
function resetChapterCheckpoint(chapter: Day01FrameworkChapter) {
  ...
  announcement.value = `第 ${chapter.number} 章练习与复述已清空；学习状态和历史正式提交未删除。`
  publish()
}
...
const currentStructureScenario = computed<StructureScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['input-contract', 'quality-snapshot', 'type-coercion', 'issue-flags', 'split-and-log', 'reconciliation', 'qualified-clean-script'].includes(value) ? value as StructureScenario : 'input-contract'
})
const currentStructureRun = computed(() => {
  const scenario = currentStructureScenario.value
  const common = { target: 'clean_orders.py', mode: '教学模拟', evidenceLimit: '只能证明本地教学模拟中的清洗、分流和对账路径，不能证明真实生产数据已经完全正确、口径已批准或规则无争议' }
  if (scenario === 'quality-snapshot') return { ...common, label: '质量快照', metric: 'completeness / uniqueness / validity', section: 'quality_rules', conclusion: 'quality_snapshot_ready', action: '把六维质量体检整理成规则表' }
  if (scenario === 'type-coercion') return { ...common, label: '类型转换', metric: 'raw_df -> working_df', section: 'source_file', conclusion: 'coercion_ready', action: '把原始表复制后再转成可比较类型' }
  if (scenario === 'issue-flags') return { ...common, label: '问题标记', metric: 'boolean flags', section: 'issue_flags', conclusion: 'flags_ready', action: '把缺失、重复、异常和未知状态标成布尔列' }
  if (scenario === 'split-and-log') return { ...common, label: '分流留痕', metric: 'clean + quarantine + rule_log', section: 'clean_output', conclusion: 'split_logged', action: '把可用行和隔离行分开并记录影响数' }
  if (scenario === 'reconciliation') return { ...common, label: '对账', metric: 'raw = clean + quarantine', section: 'reconciliation_summary', conclusion: 'reconciled', action: '核对总数并做抽样回查' }
  if (scenario === 'qualified-clean-script') return { ...common, label: '合格标注', metric: 'fields=11; evidence_limit=present', section: 'next_step', conclusion: 'ready_for_w9d4', action: '进入 W9D4 前先把订单清洗脚本补完整' }
  return { ...common, label: '输入契约', metric: 'orders_dirty.csv + required columns', section: 'source_file', conclusion: 'entry_boundary', action: '先写输入来源和必填列' }
})
const structureCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    entryInput: commands.some((item) => item.includes('input-contract')),
    assignmentConversion: commands.some((item) => item.includes('quality-snapshot')),
    branchCondition: commands.some((item) => item.includes('type-coercion')),
    loopIteration: commands.some((item) => item.includes('issue-flags')),
    functionOutput: commands.some((item) => item.includes('split-and-log')),
    qualifiedAnnotation: commands.some((item) => item.includes('qualified-clean-script')),
  }
})
function runStructureObserver() {
  const run = currentStructureRun.value
  evidence.value.sandbox.buttonClicks += 1
  const runId = `structure-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentStructureScenario.value}`, output: `${run.section} → ${run.conclusion}`, tone: 'normal' })
  evidence.value.guidedLab.observations.latestStructureRun = `${runId} · ${run.label} · section=${run.section} · conclusion=${run.conclusion}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只形成清洗路径判断，不证明真实生产数据或规则已经完全正确。`
  publish()
}
function resetStructureObserver() {
  ...
  announcement.value = '订单清洗观察器与当前草稿已清空；历史正式尝试未删除。'
  publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(structureCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) {
    announcement.value = '先完成预测、六条结构路径、全部步骤、记录与通过标准。'
    return
  }
  setSectionComplete('guided-lab', true)
  saveAttempt('guided-lab')
  announcement.value = '订单清洗脚本实操证据已追加保存；它只证明本地教学模拟中的路径质量。'
}
function completeIndependentLab() {
  ...
  if (![state.changedCondition, state.prediction, state.plan, state.evidence, state.result, state.conclusion].every((value) => value.trim().length >= 8) || !checksReady) {
    announcement.value = '独立变式需要完整条件、预测、计划、证据、结果、结论和自检。'
    return
  }
...
function resetIndependentLab() {
  ...
  announcement.value = '独立变式已清空；历史正式尝试未删除。'
  publish()
}
...
function submitExercise(exercise: Exercise) {
  ...
  state.selfAssessment = state.selectedIndex === exercise.answerIndex ? 'pass' : 'retry'
...
function resetExercise(exercise: Exercise) {
  ...
  announcement.value = '本题已回到未作答状态；历史正式尝试未删除。'
  publish()
}
...
const structureFields = ['script_id', 'source_file', 'quality_rules', 'issue_flags', 'clean_output', 'quarantine_output', 'rule_log', 'reconciliation_summary', 'can_prove', 'cannot_prove', 'next_step'] as const
const deliverableGate = computed(() => {
  const text = evidence.value.deliverable.draft
  const lower = text.toLowerCase()
  const lines = text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const fieldPresence = Object.fromEntries(structureFields.map((field) => [field, lower.includes(field)])) as Record<(typeof structureFields)[number], boolean>
  for (const field of structureFields) if (!fieldPresence[field]) errors.push(`缺少字段：${field}。`)
  const requiredBoundaryPhrases = ['clean_orders.py', 'orders_dirty.csv', 'orders_clean.csv', 'orders_quarantine.csv', 'quality_report.md', '教学模拟', '不能证明', 'W9D4']
  ...
  const roleReady = /source_file[\s\S]{0,220}(原始文件|orders_dirty\.csv|编码|字段|输入契约|质量)/i.test(text)
  const canProveReady = /can_prove[\s\S]{0,180}(清洗|分流|对账|规则|教学模拟|quarantine)/i.test(text)
  const cannotProveReady = /cannot_prove[\s\S]{0,220}(不能证明|真实生产|批准|完全正确|规则无争议|上线|口径)/i.test(text)
  const nextStepReady = /next_step[\s\S]{0,160}W9D4/i.test(text)
  if (!roleReady) errors.push('source_file 必须明确原始文件、编码、字段或输入契约。')
  if (!canProveReady) errors.push('can_prove 必须写出流程层能证明的有限事实。')
  if (!cannotProveReady) errors.push('cannot_prove 必须写出不能证明的真实生产或批准边界。')
  if (!nextStepReady) errors.push('next_step 必须只指向 W9D4。')
  return { valid: errors.length === 0, errors, lineCount: lines.length, fieldPresence, presentFields: structureFields.filter((field) => fieldPresence[field]).length }
})
const deliverableFieldStatus = computed(() => structureFields.map((field) => ({ field, done: deliverableGate.value.fieldPresence[field] })))
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) {
    announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过订单清洗脚本字段门禁。`
    return
  }
  setSectionComplete('deliverable', true)
  saveAttempt('deliverable')
  announcement.value = '订单清洗脚本已作为待审核成果追加保存；真实生产数据、口径批准和规则无争议仍未判定。'
}
...
function completeMemory() {
  ...
  if (!anchors || !reviews || evidence.value.memory.closedBook.trim().length < 60 || evidence.value.memory.microOperation.trim().length < 16) {
    announcement.value = '先完成全部记忆锚点、至少 60 字闭卷复述、微操作和六阶段排程。'
    return
  }
...
function resetMemory() {
  ...
  announcement.value = '闭卷复述与复习确认已清空；历史提交未删除。'
  publish()
}
...
function onScrollOwnerScroll() {
  ...
  scrollFrame = window.requestAnimationFrame(() => { scrollFrame = undefined; syncActiveChapterFromScroll() })
}
  ...
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    <aside class="w1-global-sidebar" aria-label="主要导航">
      ...
      <div class="w1-global-footer">
        <div class="w1-weight-key" aria-label="学习能力权重"><span><i></i>数据 40%</span><span><i></i>沟通 30%</span><span><i></i>评审 30%</span></div>
    ...
    <div class="w1-window">
      <header class="w1-appbar">
        ...
        <span class="w1-appbar-meta">COURSE / W9D3</span>
      </header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div>
              <p class="eyebrow">WEEK 9 · DAY 3 · 双时段课程</p>
              <h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1>
              <p class="subtitle">{{ lesson.subtitle }}</p>
            </div>
            <div class="cover-summary" aria-label="课程时间与进度">
          ...
          <details ref="mobileRoute" class="w1-mobile-route">
            ...
            <nav class="w1-route-list" aria-label="W9D3 移动端七章目录">
          ...
          <div class="course-protocol"><span>订单清洗首教</span><p>本课只用本地教学模拟把输入契约、质量六维、问题标记、clean/quarantine 分流、规则日志和对账整理进 clean_orders.py：不调用真实订单、支付、财务或分析仓库，也不把清洗脚本写成真实生产已经批准。前置不足只提示补学，不锁全文。</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>input contract</span><i>→</i><span>quality snapshot</span><i>→</i><span>issue flags</span><i>→</i><span>split and log</span><i>→</i><span>reconciliation</span><p>先固定脚本的输入契约、六维质量、问题标记、clean/quarantine 分流和对账，再把订单清洗脚本写成可复核的字段；任何本地教学模拟通过都不代表真实生产数据、规则批准或口径无争议。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="订单清洗实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实订单、支付、财务、分析仓库或生产系统</p>
                    <label><span>只改变一个结构路径</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="订单清洗教学场景" @change="publish"><option value="input-contract">输入契约</option><option value="quality-snapshot">质量快照</option><option value="type-coercion">类型转换</option><option value="issue-flags">问题标记</option><option value="split-and-log">分流留痕</option><option value="reconciliation">对账</option><option value="qualified-clean-script">合格脚本</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentStructureRun.target }}</dd></div><div><dt>Mode</dt><dd>{{ currentStructureRun.mode }}</dd></div><div><dt>Field signal</dt><dd>{{ currentStructureRun.metric }}</dd></div><div><dt>Structure field</dt><dd>{{ currentStructureRun.section }}</dd></div><div><dt>Conclusion</dt><dd>{{ currentStructureRun.conclusion }}</dd></div><div><dt>Revision action</dt><dd>{{ currentStructureRun.action }}</dd></div><div><dt>Evidence limit</dt><dd>{{ currentStructureRun.evidenceLimit }}</dd></div></dl>
                    <p class="first-fork"><b>结构字段：</b>{{ currentStructureRun.section }} → {{ currentStructureRun.conclusion }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runStructureObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetStructureObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="订单清洗脚本教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="订单清洗脚本实验覆盖"><span :class="{ done: structureCoverage.entryInput }">输入契约</span><span :class="{ done: structureCoverage.assignmentConversion }">质量快照</span><span :class="{ done: structureCoverage.branchCondition }">类型转换</span><span :class="{ done: structureCoverage.loopIteration }">问题标记</span><span :class="{ done: structureCoverage.functionOutput }">分流留痕</span><span :class="{ done: structureCoverage.qualifiedAnnotation }">合格脚本</span></div>
                  </div>
