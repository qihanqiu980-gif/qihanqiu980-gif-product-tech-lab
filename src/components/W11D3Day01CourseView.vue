...
type EvalSetScenario = 'missing-expected-action' | 'vague-oracle' | 'missing-refusal' | 'duplicate-input' | 'privacy-gate-off' | 'balanced-valid'
...
type W11D3Evidence = DailyLessonEvidenceState & {
...
const props = withDefaults(defineProps<{
  lesson: DailyCourse
  evidenceState?: DeepPartial<W11D3Evidence>
  durationMode?: 30 | 45
  readonly?: boolean
...
function blankEvidence(): W11D3Evidence {
  return {
    schemaVersion: 2,
    ...
    sandbox: { activePanel: 'elements', previewHeadline: '最小评测集六路径门槛观察台', domDraft: '', domEdits: 0, ageInput: 'missing-expected-action', buttonState: 'idle', buttonClicks: 0, consoleHistory: [] },
...
function mergeEvidence(source?: DeepPartial<W11D3Evidence>): W11D3Evidence {
...
const evidence = ref<W11D3Evidence>(mergeEvidence(props.evidenceState))
...
function cloneEvidence(): W11D3Evidence { return JSON.parse(JSON.stringify(evidence.value)) as W11D3Evidence }
...
const currentScenario = computed<EvalSetScenario>(() => {
  const value = evidence.value.sandbox.ageInput
  return ['missing-expected-action', 'vague-oracle', 'missing-refusal', 'duplicate-input', 'privacy-gate-off', 'balanced-valid'].includes(value) ? value as EvalSetScenario : 'missing-expected-action'
})
const currentRun = computed(() => {
  const scenario = currentScenario.value
  const common = { total: '20', categories: 'normal/boundary/adversarial/privacy/refusal', target: 'support-sim-v1.4' }
  if (scenario === 'vague-oracle') return { ...common, label: '预期只写“回答合理”', schema: 'passed', oracle: 'FAILED:vague_expected_action', coverage: '4/4/4/4/4', unique: 'passed', safety: 'passed', result: 'invalid', firstGate: 'behavior_oracle' }
  if (scenario === 'missing-refusal') return { ...common, label: '拒答类别为空', schema: 'passed', oracle: 'passed', coverage: '8/4/4/4/0', unique: 'passed', safety: 'passed', result: 'invalid', firstGate: 'coverage_quota' }
  if (scenario === 'duplicate-input') return { ...common, label: '重复输入只改 case_id', schema: 'passed', oracle: 'passed', coverage: '4/4/4/4/4', unique: 'FAILED:duplicate_user_input=2', safety: 'passed', result: 'invalid', firstGate: 'unique_input' }
  if (scenario === 'privacy-gate-off') return { ...common, label: '隐私样本 hard_gate=false', schema: 'passed', oracle: 'passed', coverage: '4/4/4/4/4', unique: 'passed', safety: 'FAILED:privacy_hard_gate', result: 'invalid', firstGate: 'safety_gate' }
  if (scenario === 'balanced-valid') return { ...common, label: '20 条五类平衡合格', schema: 'passed', oracle: 'passed', coverage: '4/4/4/4/4', unique: 'passed', safety: 'passed:safety_gates=12', result: 'valid:tool_cases=4', firstGate: 'none' }
  return { ...common, label: '缺少 expected_action', schema: 'FAILED:required_field', oracle: 'not_started', coverage: 'not_started', unique: 'not_started', safety: 'not_started', result: 'invalid', firstGate: 'schema_required_fields' }
})
const observerCoverage = computed(() => {
  const commands = evidence.value.sandbox.consoleHistory.map((entry) => entry.command)
  return {
    missingField: commands.some((item) => item.includes('missing-expected-action')),
    vagueOracle: commands.some((item) => item.includes('vague-oracle')),
    missingRefusal: commands.some((item) => item.includes('missing-refusal')),
    duplicateInput: commands.some((item) => item.includes('duplicate-input')),
    safetyGate: commands.some((item) => item.includes('privacy-gate-off')),
    validSet: commands.some((item) => item.includes('balanced-valid')),
  }
})
function runEvalSetObserver() {
  const run = currentRun.value; evidence.value.sandbox.buttonClicks += 1
  const runId = `eval-set-${String(evidence.value.sandbox.buttonClicks).padStart(3, '0')}`
  evidence.value.sandbox.consoleHistory.push({ command: `${runId}|${currentScenario.value}`, output: `${run.firstGate} → ${run.result}`, tone: currentScenario.value === 'balanced-valid' ? 'success' : 'normal' })
  evidence.value.guidedLab.observations['latest-eval-set-run'] = `${runId} · ${run.label} · firstGate=${run.firstGate} · result=${run.result}`
  announcement.value = `已追加 ${runId}；这是本地教学模拟，只校验样本集合合同，不运行真实模型。`; publish()
}
function resetEvalSetObserver() {
  const blank = blankEvidence(); evidence.value.sandbox = blank.sandbox; evidence.value.guidedLab = blank.guidedLab
  setSectionComplete('guided-lab', false); announcement.value = '评测集观察器与当前草稿已清空；历史正式尝试未删除。'; publish()
}
function completeGuidedLab() {
  ...
  if (evidence.value.guidedLab.prediction.trim().length < 12 || !Object.values(observerCoverage.value).every(Boolean) || !stepsReady || !recordsReady || !checksReady) { announcement.value = '先完成预测、六类集合质量路径、全部步骤、记录与通过标准。'; return }
  setSectionComplete('guided-lab', true); saveAttempt('guided-lab'); announcement.value = '集合门禁实操证据已追加保存；它只证明本地教学模拟中的样本合同质量。'
}
...
const evaluationCategories = ['normal', 'boundary', 'adversarial', 'privacy', 'refusal'] as const
const deliverableGate = computed(() => {
  const lines = evidence.value.deliverable.draft.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
  const errors: string[] = []
  const rows: Record<string, unknown>[] = []
  const categoryCounts = Object.fromEntries(evaluationCategories.map((category) => [category, 0])) as Record<(typeof evaluationCategories)[number], number>
  for (const [index, line] of lines.entries()) {
    try {
      const row = JSON.parse(line) as unknown
      if (!row || typeof row !== 'object' || Array.isArray(row)) errors.push(`第 ${index + 1} 行不是 JSON 对象。`)
      else rows.push(row as Record<string, unknown>)
    } catch { errors.push(`第 ${index + 1} 行不是可解析 JSON。`) }
  }
  if (lines.length !== 20) errors.push(`需要恰好 20 行，当前 ${lines.length} 行。`)
  const requiredText = ['case_id', 'category', 'target_version', 'user_input', 'context', 'expected_action', 'covered_stage', 'evidence_limit']
  const ids: string[] = []
  const inputs: string[] = []
  const targets = new Set<string>()
  let toolCases = 0
  for (const [index, row] of rows.entries()) {
    const missing = requiredText.filter((key) => typeof row[key] !== 'string' || !(row[key] as string).trim())
    if (missing.length) errors.push(`第 ${index + 1} 行缺少文本字段：${missing.join('、')}。`)
    const category = row.category
    if (typeof category === 'string' && evaluationCategories.includes(category as (typeof evaluationCategories)[number])) categoryCounts[category as (typeof evaluationCategories)[number]] += 1
    else errors.push(`第 ${index + 1} 行 category 不在五类合同内。`)
    for (const key of ['must_include', 'must_not_include']) {
      if (!Array.isArray(row[key]) || !(row[key] as unknown[]).length || !(row[key] as unknown[]).every((item) => typeof item === 'string' && item.trim())) errors.push(`第 ${index + 1} 行 ${key} 必须是非空字符串数组。`)
    }
    if (['adversarial', 'privacy', 'refusal'].includes(String(category)) && row.hard_gate !== true) errors.push(`第 ${index + 1} 行安全类别必须 hard_gate=true。`)
    if (typeof row.case_id === 'string') ids.push(row.case_id.trim().toLowerCase())
    if (typeof row.user_input === 'string') inputs.push(row.user_input.trim().replace(/\s+/g, '').toLowerCase())
    if (typeof row.target_version === 'string' && row.target_version.trim()) targets.add(row.target_version.trim())
    if (`${String(row.expected_action)} ${String(row.covered_stage)}`.toLowerCase().includes('tool')) toolCases += 1
  }
  if (new Set(ids).size !== ids.length) errors.push('case_id 存在重复。')
  if (new Set(inputs).size !== inputs.length) errors.push('规范化 user_input 存在重复。')
  if (targets.size > 1) errors.push('20 条样本必须绑定同一个 target_version。')
  for (const category of evaluationCategories) if (categoryCounts[category] !== 4) errors.push(`${category} 应为 4 条，当前 ${categoryCounts[category]} 条。`)
  if (toolCases < 4) errors.push(`工具阶段样本至少 4 条，当前 ${toolCases} 条。`)
  return { valid: errors.length === 0, errors, lineCount: lines.length, categoryCounts, toolCases }
})
function completeDeliverable() {
  ...
  if (deliverableContribution.value < activePath.value.deliverableMinimumContributionCharacters || !checksReady || !deliverableGate.value.valid) { announcement.value = `成果需要至少 ${activePath.value.deliverableMinimumContributionCharacters} 个有效实写字符、完成自检，并通过 20 条 JSONL 集合门禁。`; return }
  setSectionComplete('deliverable', true); saveAttempt('deliverable'); announcement.value = '20 条评测样本已作为待审核成果追加保存；模型表现仍未判定。'
}
...
<template>
  <article ref="lessonRoot" class="day01-course" :data-device="deviceMode" :aria-labelledby="`${lesson.id}-title`">
    ...
    <div class="w1-window">
      <header class="w1-appbar"><button type="button" class="w1-brand" @click="emit('navigate', 'today')"><span class="w1-brand-mark">PL</span><strong>产品技术实验室</strong></button><span class="w1-appbar-meta">COURSE / W11D3</span></header>
      <div class="w1-shell">
        <main class="w1-main">
          <header class="course-cover">
            <div><p class="eyebrow">WEEK 11 · DAY 3 · 双时段课程</p><h1 :id="`${lesson.id}-title`">{{ lesson.title }}</h1><p class="subtitle">{{ lesson.subtitle }}</p></div>
          ...
          <div class="manuscript">
            <section v-for="chapter in plan.chapters" :id="`${lesson.id.toLowerCase()}-${chapter.id}`" :key="chapter.id" class="framework-chapter" :data-framework-chapter="chapter.id" tabindex="-1">
              ...
              <div v-if="chapter.number === 4" class="understanding-map"><span>target slice／rubric</span><i>→</i><span>case schema</span><i>→</i><span>behavior oracle</span><i>→</i><span>五类分层／hard gate</span><i>→</i><span>coverage／set gate</span><p>先固定评测对象与判断语言，再写单条样本，最后用覆盖与集合门禁拒绝凑数；任何一步通过都不代表模型已经运行。</p></div>
              ...
              <template v-if="chapter.number === 5">
                ...
                <div class="lab-panel">
                  ...
                  <label class="field"><span>先预测</span><small>{{ lesson.guidedLab.predictionPrompt }}</small><textarea v-model="evidence.guidedLab.prediction" rows="3" :readonly="readonly" aria-label="评测集集合门禁实验预测" @input="publish" /></label>
                  <div class="system-console">
                    <p class="simulation-label">教学模拟 · 不调用真实模型、工具、账号、Token、公司数据或生产系统</p>
                    <label><span>只改变一个集合质量条件</span><select v-model="evidence.sandbox.ageInput" :disabled="readonly" aria-label="评测集集合质量教学场景" @change="publish"><option value="missing-expected-action">缺少 expected_action</option><option value="vague-oracle">预期只写“回答合理”</option><option value="missing-refusal">refusal 类别为空</option><option value="duplicate-input">重复输入只改 case_id</option><option value="privacy-gate-off">privacy 样本 hard_gate=false</option><option value="balanced-valid">20 条五类平衡合格</option></select></label>
                    <dl class="stage-ledger"><div><dt>Target</dt><dd>{{ currentRun.target }}</dd></div><div><dt>Rows</dt><dd>{{ currentRun.total }}</dd></div><div><dt>Categories</dt><dd>{{ currentRun.categories }}</dd></div><div><dt>Schema</dt><dd>{{ currentRun.schema }}</dd></div><div><dt>Oracle</dt><dd>{{ currentRun.oracle }}</dd></div><div><dt>Coverage</dt><dd>{{ currentRun.coverage }}</dd></div><div><dt>Unique input</dt><dd>{{ currentRun.unique }}</dd></div><div><dt>Safety gate</dt><dd>{{ currentRun.safety }}</dd></div><div><dt>Result</dt><dd>{{ currentRun.result }}</dd></div></dl>
                    <p class="first-fork"><b>首次失败门禁：</b>{{ currentRun.firstGate }}</p>
                    <div class="console-actions"><button type="button" :disabled="readonly" @click="runEvalSetObserver">运行并追加记录</button><button type="button" class="secondary" :disabled="readonly" @click="resetEvalSetObserver">全部清空并重做</button></div>
                    <ol v-if="evidence.sandbox.consoleHistory.length" class="exchange-history" aria-label="评测集教学模拟追加历史"><li v-for="(entry, index) in evidence.sandbox.consoleHistory" :key="`${entry.command}-${index}`"><code>{{ entry.command }}</code><span>{{ entry.output }}</span></li></ol>
                    <div class="simulation-coverage" aria-label="评测集实验覆盖"><span :class="{ done: observerCoverage.missingField }">字段缺失</span><span :class="{ done: observerCoverage.vagueOracle }">预期模糊</span><span :class="{ done: observerCoverage.missingRefusal }">类别缺口</span><span :class="{ done: observerCoverage.duplicateInput }">重复输入</span><span :class="{ done: observerCoverage.safetyGate }">安全门槛</span><span :class="{ done: observerCoverage.validSet }">平衡合格</span></div>
                  </div>
        ...
        <aside class="w1-route" aria-label="W11D3 七章学习路线"><div class="w1-route-head"><strong>七章学习路线</strong><span>{{ progressPercent }}%</span></div><div v-for="session in [1, 2]" :key="session" class="w1-route-group"><strong><span>时段 {{ session }}</span><span>{{ plan.chapters.filter((chapter) => chapter.session === session).length }} 章</span></strong><div class="w1-route-list"><button v-for="chapter in plan.chapters.filter((item) => item.session === session)" :key="chapter.id" type="button" class="w1-route-item" :aria-current="activeChapterId === chapter.id ? 'location' : undefined" @click="goToChapter(chapter)"><span class="w1-route-title"><span>{{ String(chapter.number).padStart(2, '0') }}</span><span>{{ chapter.title }}</span></span><span class="w1-route-states" aria-label="本章状态"><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].learn }]" :aria-label="`学习：${evidence.frameworkChapters[chapter.id].learn ? '已完成' : '未完成'}`">学习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].practicePassed }]" :aria-label="`练习：${evidence.frameworkChapters[chapter.id].practicePassed ? '已完成' : '未完成'}`">练习</span><span :class="['w1-route-state', { 'is-done': evidence.frameworkChapters[chapter.id].retellSubmitted }]" :aria-label="`复述：${evidence.frameworkChapters[chapter.id].retellSubmitted ? '已完成' : '未完成'}`">复述</span></span></button></div></div><div class="w1-route-note"><strong>掌握未判定 · {{ progressPercent }}%</strong><br>当前位置与完成状态分开；清空重做立即撤销对应高亮。</div></aside>
      </div>
    </div>
  </article>
</template>
